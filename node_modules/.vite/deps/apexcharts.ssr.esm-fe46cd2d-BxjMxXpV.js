//#region node_modules/vue3-apexcharts/dist/apexcharts.ssr.esm-fe46cd2d.js
var De = Object.defineProperty, Te = Object.defineProperties, Me = Object.getOwnPropertyDescriptors, Vt = Object.getOwnPropertySymbols, Pe = Object.prototype.hasOwnProperty, Fe = Object.prototype.propertyIsEnumerable, Xt = (A, t, e) => t in A ? De(A, t, {
	enumerable: !0,
	configurable: !0,
	writable: !0,
	value: e
}) : A[t] = e, I = (A, t) => {
	for (var e in t || (t = {})) Pe.call(t, e) && Xt(A, e, t[e]);
	if (Vt) for (var e of Vt(t)) Fe.call(t, e) && Xt(A, e, t[e]);
	return A;
...
var Lt = class Lt {
	...
	toString() {
		...
		if (this.attributes.forEach((s, i) => {
			t += ` ${i}="${s}"`;
		}), this.children.length === 0 && !this.textContent) return `<${this.nodeName}${t}/>`;
...
var Re = class {
	...
	*/
	createDOMParser() {
		return { parseFromString(t, e) {
			const s = new Lt("root");
			return s.innerHTML = t, { documentElement: s };
		} };
	}
};
var j = null, ot = null, rt = null;
var N = class {
	/**
...
var L = class ht {
	...
	static extend(t, e) {
		...
		return this.isObject(t) && this.isObject(e) && Object.keys(e).forEach((i) => {
			this.isObject(e[i]) ? i in t ? s[i] = this.extend(t[i], e[i]) : Object.assign(s, { [i]: e[i] }) : Object.assign(s, { [i]: e[i] });
		}), s;
	}
	...
	*/
	static clone(t, e = /* @__PURE__ */ new WeakMap(), s = !1) {
		...
		let i;
		if (Array.isArray(t)) if (s) i = t.slice();
		else {
			i = [], e.set(t, i);
			...
			i = {}, e.set(t, i);
			for (const a in t) Object.prototype.hasOwnProperty.call(t, a) && (i[a] = this.clone(t[a], e, !1));
		}
		return i;
	}
	/**
	...
	*/
	static shallowEqual(t, e) {
		...
		for (const a of s) if (t[a] !== e[a]) return !1;
		return !0;
	}
	static log10(t) {
	...
	*/
	...
	static getDimensions(t) {
		if (!t) return [0, 0];
		if (B.isSSR()) return [t._ssrWidth || 400, t._ssrHeight || 300];
	...
	static getBoundingClientRect(t) {
		...
		if (B.isSSR()) return N.getBoundingClientRect(t);
	...
	static hexToRgba(t = "#999999", e = .6) {
		t.substring(0, 1) !== "#" && (t = "#999999");
		let s = t.replace("#", "");
		s = s.match(new RegExp("(.{" + s.length / 3 + "})", "g"));
		for (let i = 0; i < s.length; i++) s[i] = parseInt(s[i].length === 1 ? s[i] + s[i] : s[i], 16);
	...
	static rgb2hex(t) {
		return t = t.match(/^rgba?[\s+]?\([\s+]?(\d+)[\s+]?,[\s+]?(\d+)[\s+]?,[\s+]?(\d+)[\s+]?/i), t && t.length === 4 ? "#" + ("0" + parseInt(t[1], 10).toString(16)).slice(-2) + ("0" + parseInt(t[2], 10).toString(16)).slice(-2) + ("0" + parseInt(t[3], 10).toString(16)).slice(-2) : "";
	}
	shadeRGBColor(t, e) {
	...
	static getThemeColor(t) {
		if (!ht.isCSSVariable(t) || B.isSSR()) return t;
	...
	static setELstyles(t, e) {
		for (const s in e) Object.prototype.hasOwnProperty.call(e, s) && (t.style.key = e[s]);
	}
...
var q = class {
	...
	parseDate(t) {
		if (!isNaN(Date.parse(t))) return this.getTimeStamp(t);
...
var vt = class {
	...
	xLabelFormat(t, e, s, i) {
		...
		if (a.config.xaxis.type === "datetime" && a.config.xaxis.labels.formatter === void 0 && a.config.tooltip.x.formatter === void 0) {
			...
			return o.formatDate(o.getDate(e), a.config.tooltip.x.format);
		}
		return t(e, s, i);
	}
	...
	defaultYFormatter(t, e) {
		const s = this.w;
		if (L.isNumber(t)) if (s.globals.yValueDecimal !== 0) t = t.toFixed(e.decimalsInFloat !== void 0 ? e.decimalsInFloat : s.globals.yValueDecimal);
		else {
			const i = t.toFixed(0);
			t = t == i ? i : t.toFixed(1);
		}
		...
		return e.xaxisTooltipFormatter = (s) => this.defaultGeneralFormatter(s), e.ttKeyFormatter = (s) => this.defaultGeneralFormatter(s), e.ttZFormatter = (s) => s, e.legendFormatter = (s) => this.defaultGeneralFormatter(s), t.config.xaxis.labels.formatter !== void 0 ? e.xLabelFormatter = t.config.xaxis.labels.formatter : e.xLabelFormatter = (s) => {
			if (L.isNumber(s)) {
				if (!t.config.xaxis.convertedCatToNumeric && t.config.xaxis.type === "numeric") {
					if (L.isNumber(t.config.xaxis.decimalsInFloat)) return s.toFixed(t.config.xaxis.decimalsInFloat);
					{
	...
	heatmapLabelFormatters() {
		...
		if (t.config.chart.type === "heatmap") {
			...
			const e = t.seriesData.seriesNames.reduce((s, i) => s.length > i.length ? s : i, 0);
			t.globals.yAxisScale[0].niceMax = e, t.globals.yAxisScale[0].niceMin = e;
		}
	}
};
var Ft = ({ isTimeline: A, seriesIndex: t, dataPointIndex: e, y1: s, y2: i, w: a }) => {
	var o;
	...
	if (a.config.tooltip.x.formatter === void 0) if (a.config.xaxis.type === "datetime") {
		const m = new q(a);
		g = m.formatDate(m.getDate(r), a.config.tooltip.x.format), f = m.formatDate(m.getDate(n), a.config.tooltip.x.format);
	} else g = r, f = n;
	else g = a.config.tooltip.x.formatter(r), f = a.config.tooltip.x.formatter(n);
	return {
		start: r,
		end: n,
		startVal: g,
	...
	const l = n.yLbFormatter(A.w.seriesData.series[o][r]);
	let c = "";
...
var bt = class {
	...
	candlestick() {
		return {
			...
			tooltip: {
				shared: !0,
				custom: ({ seriesIndex: t, dataPointIndex: e, w: s }) => this._getBoxTooltip(s, t, e, [
					"Open",
					"High",
					"",
					"Low",
	...
	rangeBar() {
		const t = (s) => {
			const { color: i, seriesName: a, ylabel: o, startVal: r, endVal: n } = Ft(O(I({}, s), { isTimeline: !0 }));
			return Et(O(I({}, s), {
				color: i,
				seriesName: a,
				ylabel: o,
	...
	stackedBars() {
		const t = this.bar();
		return O(I({}, t), { plotOptions: O(I({}, t.plotOptions), { bar: O(I({}, t.plotOptions.bar), {
...
var yt = class {
	...
	extendYAxis(t, e) {
		...
		return e && !o && (o = e.config.series), a && o.length !== t.yaxis.length && o.length && (t.yaxis = o.map((r, n) => {
			if (r.name || (o[n].name = `series-${n + 1}`), t.yaxis[n]) return t.yaxis[n].seriesName = o[n].name, t.yaxis[n];
			{
				const l = L.extend(s.yAxis, t.yaxis[0]);
				return l.show = !1, l;
			}
		})), a && o.length > 1 && o.length !== t.yaxis.length && console.warn("A multi-series logarithmic chart should have equal number of series and y-axes"), t;
	}
	extendAnnotations(t) {
	...
	extendYAxisAnnotations(t) {
		const e = new nt();
		return t.annotations.yaxis = L.extendArray(typeof t.annotations.yaxis < "u" ? t.annotations.yaxis : [], e.yAxisAnnotation), t;
	}
	extendXAxisAnnotations(t) {
		const e = new nt();
		return t.annotations.xaxis = L.extendArray(typeof t.annotations.xaxis < "u" ? t.annotations.xaxis : [], e.xAxisAnnotation), t;
	}
	extendPointAnnotations(t) {
		const e = new nt();
		return t.annotations.points = L.extendArray(typeof t.annotations.points < "u" ? t.annotations.points : [], e.pointAnnotation), t;
	}
	checkForDarkTheme(t) {
	...
	handleUserInputErrors(t) {
		...
		return (e.chart.type === "bar" || e.chart.type === "rangeBar") && e.tooltip.shared && e.xaxis.crosshairs.width === "barWidth" && e.series.length > 1 && (e.xaxis.crosshairs.width = "tickWidth"), (e.chart.type === "candlestick" || e.chart.type === "boxPlot") && e.yaxis[0].reversed && (console.warn(`Reversed y-axis in ${e.chart.type} chart is not supported.`), e.yaxis[0].reversed = !1), e;
	}
};
var ce = 1.618, he = [[
	1,
...
var V = class {
	...
	**/
	getStackedSeriesTotals(t = []) {
		...
		for (let i = 0; i < e.seriesData.series[e.globals.maxValsInArrayIndex].length; i++) {
			let a = 0;
			for (let o = 0; o < e.seriesData.series.length; o++) typeof e.seriesData.series[o][i] < "u" && t.indexOf(o) === -1 && (a += e.seriesData.series[o][i]);
			s.push(a);
		}
		return s;
	}
	...
	**/
			...
			if (l.seriesName) {
				...
				Array.isArray(l.seriesName) ? d = l.seriesName : d.push(l.seriesName), d.forEach((p) => {
					e.series.forEach((g, f) => {
						if (g.name === p) {
							let x = f;
							c === f || o ? !o || a.indexOf(f) > -1 ? s[c].push([c, f]) : console.warn("Series '" + g.name + "' referenced more than once in what looks like the new style. That is, when using either seriesName: [], or when there are more series than yaxes.") : (s[f].push([f, c]), x = c), h = !0, x = a.indexOf(x), x !== -1 && a.splice(x, 1);
						}
					});
				});
			}
	...
	getLargestSeries() {
		const t = this.w;
		t.globals.maxValsInArrayIndex = t.seriesData.series.map((e) => e.length).indexOf(Math.max.apply(Math, t.seriesData.series.map((e) => e.length)));
	}
	...
	**/
	...
	getSeriesTotalsXRange(t, e) {
		...
		return s.seriesData.series.map((a, o) => {
			let r = 0;
			for (let n = 0; n < a.length; n++) s.seriesData.seriesX[o][n] > t && s.seriesData.seriesX[o][n] < e && (r += a[n]);
			return r;
		});
	}
	/**
	...
	**/
	getPercentSeries() {
		...
		t.globals.seriesPercent = t.seriesData.series.map((e) => {
			const s = [];
			if (Array.isArray(e)) for (let i = 0; i < e.length; i++) {
	...
	getCalculatedRatios() {
		...
		let i = 0, a = 0, o = 0, r = 0, n = [], l = .1, c = 0;
		if (e.yRange = [], e.isMultipleYAxis) for (let h = 0; h < e.minYArr.length; h++) e.yRange.push(Math.abs(e.minYArr[h] - e.maxYArr[h])), n.push(0);
		else e.yRange.push(Math.abs(e.minY - e.maxY));
		e.xRange = Math.abs(e.maxX - e.minX), e.zRange = Math.abs(e.maxZ - e.minZ);
		for (let h = 0; h < e.yRange.length; h++) s.push(e.yRange[h] / this.w.layout.gridHeight);
		if (a = e.xRange / this.w.layout.gridWidth, i = e.yRange / this.w.layout.gridWidth, o = e.xRange / this.w.layout.gridHeight, r = e.zRange / this.w.layout.gridHeight * 16, r || (r = 1), e.minY !== Number.MIN_VALUE && Math.abs(e.minY) !== 0 && (e.hasNegs = !0), t.globals.seriesYAxisReverseMap.length > 0) {
			...
			} else n = [], n.push(h(e.minY, 0)), e.minY !== Number.MIN_VALUE && Math.abs(e.minY) !== 0 && (l = -e.minY / i, c = e.minX / a);
		} else n = [], n.push(0), l = 0, c = 0;
		return {
			yRatio: s,
			invertedYRatio: i,
			zRatio: r,
	...
	getLogValAtSeriesIndex(t, e) {
		...
		return s.config.yaxis[i] && s.config.yaxis[i].logarithmic ? this.getLogVal(s.config.yaxis[i].logBase, t, e) : t;
	}
	getBaseLog(t, e) {
...
var ut = class {
	...
	animateMarker(t, e, s, i) {
		t.attr({ opacity: 0 }).animate(e).attr({ opacity: 1 }).after(() => {
			i();
		});
	}
	...
	animatePathsGradually(t) {
		...
		let d = 0;
		h.config.chart.animations.animateGradually.enabled && (d = h.config.chart.animations.animateGradually.delay), h.config.chart.animations.dynamicAnimation.enabled && h.globals.dataChanged && h.config.chart.type !== "bar" && (d = 0), c.morphSVG(e, s, i, h.config.chart.type === "line" && !h.globals.comboCharts ? "stroke" : a, o, r, n, l * d);
	}
	showDelayedElements() {
	...
	animationCompleted(t) {
		const e = this.w;
		e.globals.animationEnded || (e.globals.animationEnded = !0, this.showDelayedElements(), typeof e.config.chart.events.animationEnd == "function" && e.config.chart.events.animationEnd(this.ctx, {
			el: t,
			w: e
		}));
	}
...
var Z = class {
	...
	applyFilter(t, e, s) {
		...
		const n = r.config.chart.dropShadow, l = s === "lighten" ? 2 : .3;
	...
	addShadow(t, e, s, i) {
		...
		if (c = Array.isArray(c) ? c[e] : c, ((a = o.config.chart.dropShadow.enabledOnSeries) == null ? void 0 : a.length) > 0 && o.config.chart.dropShadow.enabledOnSeries.indexOf(e) === -1) return t;
		t.offset({
			in: i,
			dx: l,
			dy: n,
	...
	_scaleFilterSize(t) {
		if (!t) return;
		((s) => {
			for (const i in s) Object.prototype.hasOwnProperty.call(s, i) && t.setAttribute(i, s[i]);
		})({
			width: "200%",
			height: "200%",
			x: "-50%",
...
var P = class P {
	...
	*/
	roundPathCorners(t, e) {
		...
		var n = t.split(/[,\s]/).reduce(function(S, k) {
		...
		if (n.length > 1) {
			var c = o(n[0]), h = null;
			n[n.length - 1][0] == "Z" && n[0].length > 2 && (h = [
				"L",
				c.x,
				c.y
			], n[n.length - 1] = h), l.push(n[0]);
			for (var d = 1; d < n.length; d++) {
				var p = l[l.length - 1], g = n[d], f = g == h ? n[1] : n[d + 1];
				if (f && p && p.length > 2 && g[0] == "L" && f.length > 2 && f[0] == "L") {
					var x = o(p), m = o(g), u = o(f), y = s(m, x, e), b = s(m, u, e);
					a(g, y), g.origPoint = m, l.push(g);
					var w = i(y, m, .5), v = i(m, b, .5), C = [
						"C",
						w.x,
						w.y,
						v.x,
	...
	drawPath({ d: t = "", stroke: e = "#a8a8a8", strokeWidth: s = 1, fill: i, fillOpacity: a = 1, strokeOpacity: o = 1, classes: r, strokeLinecap: n = null, strokeDashArray: l = 0 }) {
		const c = this.w;
	...
	line(t, e, s = null) {
		...
		return s === null ? i = [
			...
			t,
			e
		].join(" ") : s === "H" ? i = [" H", t].join(" ") : s === "V" && (i = [" V", e].join(" ")), i;
	}
	curve(t, e, s, i, a, o) {
		return [
			"C",
	...
	**/
	renderPaths({ j: t, realIndex: e, pathFrom: s, pathTo: i, stroke: a, strokeWidth: o, strokeLinecap: r, fill: n, animationDelay: l, initialSpeed: c, dataChangeSpeed: h, className: d, chartType: p, shouldClipToGrid: g = !0, bindEventsOnPaths: f = !0, drawShadow: x = !0 }) {
		...
		k.attr("index", e), g && (p === "bar" && !m.globals.isHorizontal || m.globals.comboCharts ? k.attr({ "clip-path": `url(#gridRectBarMask${m.globals.cuid})` }) : k.attr({ "clip-path": `url(#gridRectMask${m.globals.cuid})` })), m.config.chart.dropShadow.enabled && x && u.dropShadow(k, m.config.chart.dropShadow, e), f && (k.node.addEventListener("mouseenter", this.pathMouseEnter.bind(this, k)), k.node.addEventListener("mouseleave", this.pathMouseLeave.bind(this, k)), k.node.addEventListener("mousedown", this.pathMouseDown.bind(this, k))), k.attr({
			pathTo: i,
			pathFrom: s
		});
		const F = {
		...
		return b && !m.globals.resized && !m.globals.dataChanged ? y.animatePathsGradually(O(I({}, F), { speed: c })) : (m.globals.resized || !m.globals.dataChanged) && y.showDelayedElements(), m.globals.dataChanged && w && C && y.animatePathsGradually(O(I({}, F), { speed: h })), k;
	}
	drawPattern(t, e, s, i = "#a8a8a8", a = 0) {
		return this.w.dom.Paper.pattern(e, s, (n) => {
			t === "horizontalLines" ? n.line(0, 0, s, 0).stroke({
				color: i,
				width: a + 1
			}) : t === "verticalLines" ? n.line(0, 0, 0, e).stroke({
				color: i,
				width: a + 1
			}) : t === "slantedLines" ? n.line(0, 0, e, s).stroke({
				color: i,
				width: a
			}) : t === "squares" ? n.rect(e, s).fill("none").stroke({
				color: i,
				width: a
			}) : t === "circles" && n.circle(e).fill("none").stroke({
				color: i,
				width: a
			});
		});
	...
	drawGradient(t, e, s, i, a, o = null, r = null, n = [], l = 0) {
		...
		}), !x) t === "vertical" ? h.from(0, 0).to(0, 1) : t === "diagonal" ? h.from(0, 0).to(1, 1) : t === "horizontal" ? h.from(0, 1).to(1, 1) : t === "diagonal2" && h.from(1, 0).to(0, 1);
		else {
	...
	drawText({ x: t, y: e, text: s, textAnchor: i, fontSize: a, fontFamily: o, fontWeight: r, foreColor: n, opacity: l, maxWidth: c, cssClass: h = "", isPlainText: d = !0, dominantBaseline: p = "auto" }) {
		const g = this.w;
		typeof s > "u" && (s = "");
		let f = s;
		...
		return Array.isArray(s) ? m = g.dom.Paper.text((u) => {
			for (let y = 0; y < s.length; y++) f = s[y], c && (f = this.getTextBasedOnMaxWidth(I({ text: s[y] }, x))), y === 0 ? u.tspan(f) : u.tspan(f).newLine();
		}) : (c && (f = this.getTextBasedOnMaxWidth(I({ text: s }, x))), m = d ? g.dom.Paper.plain(s) : g.dom.Paper.text((u) => u.tspan(f))), m.attr({
			x: t,
			y: e,
			"text-anchor": i,
			"dominant-baseline": p,
	...
	pathMouseDown(t, e) {
		...
		if (t.node.getAttribute("selected") === "true") {
			...
			if (!s.config.states.active.allowMultipleDataPointsSelection && s.interact.selectedDataPoints.length > 0) {
				...
				const n = s.dom.Paper.find(".apexcharts-series path:not(.apexcharts-decoration-element)"), l = s.dom.Paper.find(".apexcharts-series circle:not(.apexcharts-decoration-element), .apexcharts-series rect:not(.apexcharts-decoration-element)"), c = (h) => {
		...
		if (r === "true") {
			...
			if (n !== "none") i.applyFilter(t, a, n.type);
		...
		} else if (s.config.states.active.filter.type !== "none") if (s.config.states.hover.filter.type !== "none" && !s.interact.isTouchDevice) {
	...
	*/
	setupEventDelegation(t, e) {
		...
		t.node.addEventListener("mouseover", (i) => {
			const a = P._findDelegateTarget(i.target, t.node, e);
			...
			if (!s) return;
			(i.relatedTarget ? P._findDelegateTarget(i.relatedTarget, t.node, e) : null) !== s && (s && s.instance && this.pathMouseLeave(s.instance, i), s = null);
	...
	static setAttrs(t, e) {
		for (const s in e) Object.prototype.hasOwnProperty.call(e, s) && t.setAttribute(s, e[s]);
	}
	...
	**/
	placeTextWithEllipsis(t, e, s) {
		if (typeof t.getComputedTextLength == "function" && (t.textContent = e, e.length > 0 && t.getComputedTextLength() >= s / 1.1)) {
			for (let i = e.length - 3; i > 0; i -= 3) if (t.getSubStringLength(0, i) <= s / 1.1) {
...
var lt = class lt {
	...
	multiply(t) {
		return new lt(this.a * t.a + this.c * t.b, this.b * t.a + this.d * t.b, this.a * t.c + this.c * t.d, this.b * t.c + this.d * t.d, this.a * t.e + this.c * t.f + this.e, this.b * t.e + this.d * t.f + this.f);
	}
	apply(t) {
		return new et(this.a * t.x + this.c * t.y + this.e, this.b * t.x + this.d * t.y + this.f);
	}
};
var Ne = class {
	constructor(t, e, s, i) {
...
var st = class {
	...
	computeColorStops(t, e) {
		const s = this.w;
		let i = null, a = null;
		for (const l of t) l >= e.threshold ? (i === null || l > i) && (i = l) : (a === null || l < a) && (a = l);
	...
	fillPath(t) {
		...
		if (f.indexOf("rgb") === -1 ? f.indexOf("#") === -1 ? y = f : f.length < 9 && (y = L.hexToRgba(f, m)) : f.indexOf("rgba") > -1 ? m = L.getOpacityFromRGBA(f) : y = L.hexToRgba(L.rgb2hex(f), m), t.opacity && (m = t.opacity), x === "pattern" && (h = this.handlePatternFill({
			...
			p && (b[this.seriesIndex] = this.computeColorStops(n.seriesData.series[this.seriesIndex], l.plotOptions.line.colors), w = "vertical"), d = this.handleGradientFill({
				type: w,
				fillConfig: t.fillConfig,
				fillColor: f,
				fillOpacity: m,
	...
	handlePatternFill({ fillConfig: t, patternFill: e, fillColor: s, fillOpacity: i, defaultColor: a }) {
		...
		return Array.isArray(o.pattern.style) ? typeof o.pattern.style[r.seriesNumber] < "u" ? e = n.drawPattern(o.pattern.style[r.seriesNumber], o.pattern.width, o.pattern.height, c, l, i) : e = a : e = n.drawPattern(o.pattern.style, o.pattern.width, o.pattern.height, c, l, i), e;
	}
	handleGradientFill({ type: t, fillColor: e, fillOpacity: s, fillConfig: i, colorStops: a, i: o }) {
		...
		if (r.gradient.gradientToColors === void 0 || r.gradient.gradientToColors.length === 0) r.gradient.shade === "dark" ? d = c.shadeColor(parseFloat(r.gradient.shadeIntensity) * -1, e.indexOf("rgb") > -1 ? L.rgb2hex(e) : e) : d = c.shadeColor(parseFloat(r.gradient.shadeIntensity), e.indexOf("rgb") > -1 ? L.rgb2hex(e) : e);
		...
		return h.indexOf("rgb") > -1 && (h = L.rgb2hex(h)), d.indexOf("rgb") > -1 && (d = L.rgb2hex(d)), l.drawGradient(t, h, d, p, g, n.size, r.gradient.stops, a, o);
	}
};
var xt = class {
	constructor(t, e) {
	...
	setGlobalMarkerSize() {
		...
		if (t.globals.markers.size = Array.isArray(t.config.markers.size) ? t.config.markers.size : [t.config.markers.size], t.globals.markers.size.length > 0) {
			if (t.globals.markers.size.length < t.seriesData.series.length + 1) for (let e = 0; e <= t.seriesData.series.length; e++) typeof t.globals.markers.size[e] > "u" && t.globals.markers.size.push(t.globals.markers.size[0]);
		} else t.globals.markers.size = t.config.series.map(() => t.config.markers.size);
	}
	plotChartMarkers({ pointsPos: t, seriesIndex: e, j: s, pSize: i, alwaysDrawMarker: a = !1, isVirtualPoint: o = !1 }) {
		const r = this.w, n = e, l = t;
		let c = null;
		const h = new P(this.w), d = r.config.markers.discrete && r.config.markers.discrete.length;
		if (Array.isArray(l.x)) for (let p = 0; p < l.x.length; p++) {
			...
			if ((r.config.chart.type === "line" || r.config.chart.type === "area") && !r.globals.comboCharts && !r.config.tooltip.intersect && (m += " no-pointer-events"), (Array.isArray(r.config.markers.size) ? r.globals.markers.size[e] > 0 : r.config.markers.size > 0) || a || d) {
				...
				r.config.series[n].data[f] && (r.config.series[n].data[f].fillColor && (y.pointFillColor = r.config.series[n].data[f].fillColor), r.config.series[n].data[f].strokeColor && (y.pointStrokeColor = r.config.series[n].data[f].strokeColor)), typeof i < "u" && (y.pSize = i), (l.x[p] < -r.globals.markers.largestSize || l.x[p] > r.layout.gridWidth + r.globals.markers.largestSize || l.y[p] < -r.globals.markers.largestSize || l.y[p] > r.layout.gridHeight + r.globals.markers.largestSize) && (y.pSize = 0), x || ((r.globals.markers.size[e] > 0 || a || d) && !c && (c = h.group({ class: a || d ? "" : "apexcharts-series-markers" }), c.attr("clip-path", `url(#gridRectMarkerMask${r.globals.cuid})`), this.setupMarkerDelegation(c)), g = h.drawMarker(l.x[p], l.y[p], y), g.attr("rel", f), g.attr("j", f), g.attr("index", e), g.node.setAttribute("default-marker-size", y.pSize), this._filters.setSelectionFilter(g, e, f), c && c.add(g));
			} else typeof r.globals.pointsArray[e] > "u" && (r.globals.pointsArray[e] = []), r.globals.pointsArray[e].push([l.x[p], l.y[p]]);
		}
		return c;
	}
	getMarkerConfig({ cssClass: t, seriesIndex: e, dataPointIndex: s = null, radius: i = null, size: a = null, strokeWidth: o = null }) {
	...
	setupMarkerDelegation(t) {
		...
		this._graphics.setupEventDelegation(t, s), t.node.addEventListener("click", (i) => {
			e.config.markers.onClick && P._findDelegateTarget(i.target, t.node, s) && e.config.markers.onClick(i);
		}), t.node.addEventListener("dblclick", (i) => {
			e.config.markers.onDblClick && P._findDelegateTarget(i.target, t.node, s) && e.config.markers.onDblClick(i);
		}), t.node.addEventListener("touchstart", (i) => {
			const a = P._findDelegateTarget(i.target, t.node, s);
			a && a.instance && this._graphics.pathMouseDown(a.instance, i);
		}, { passive: !0 });
	}
	addEvents(t) {
		const e = this.w;
		t.node.addEventListener("mouseenter", this._graphics.pathMouseEnter.bind(this.ctx, t)), t.node.addEventListener("mouseleave", this._graphics.pathMouseLeave.bind(this.ctx, t)), t.node.addEventListener("mousedown", this._graphics.pathMouseDown.bind(this.ctx, t)), t.node.addEventListener("click", e.config.markers.onClick), t.node.addEventListener("dblclick", e.config.markers.onDblClick), t.node.addEventListener("touchstart", this._graphics.pathMouseDown.bind(this.ctx, t), { passive: !0 });
	}
	getMarkerStyle(t) {
		const e = this.w, s = e.globals.markers.colors, i = e.config.markers.strokeColor || e.config.markers.strokeColors;
		return {
			pointStrokeColor: Array.isArray(i) ? i[t] : i,
			pointFillColor: Array.isArray(s) ? s[t] : s
		};
	}
};
var ge = class {
	...
	draw(t, e, s) {
		const i = this.w, a = this.graphics, o = s.realIndex, r = s.pointsPos, n = s.zRatio, l = s.elParent, c = a.group({ class: `apexcharts-series-markers apexcharts-series-${i.config.chart.type}` });
		if (c.attr("clip-path", `url(#gridRectMarkerMask${i.globals.cuid})`), this.markers.setupMarkerDelegation(c), Array.isArray(r.x)) for (let h = 0; h < r.x.length; h++) {
			let d = e + 1, p = !0;
			...
			if (g = g || 0, (x === null || typeof i.seriesData.series[o][d] > "u") && (p = !1), p) {
				const m = this.drawPoint(f, x, g, o, d, e);
				c.add(m);
			}
			l.add(c);
		}
	...
	drawPoint(t, e, s, i, a, o) {
		...
		if (r.config.series[n].data[a] && r.config.series[n].data[a].fillColor && (f = r.config.series[n].data[a].fillColor), x.attr({ fill: f }), r.config.chart.dropShadow.enabled) {
	...
	centerTextInBubble(t) {
		const e = this.w;
		return t = t + parseInt(e.config.dataLabels.style.fontSize, 10) / 4, { y: t };
	}
};
var gt = class {
	constructor(t, e = null) {
	...
	dataLabelsCorrection(t, e, s, i, a, o, r) {
		...
		e < 0 && (e = 0), e > n.layout.gridHeight + p && (e = n.layout.gridHeight + p / 2), typeof n.globals.dataLabelsRects[i] > "u" && (n.globals.dataLabelsRects[i] = []), n.globals.dataLabelsRects[i].push({
			x: t,
			y: e,
			width: d,
			height: p
	...
	drawDataLabel({ type: t, pos: e, i: s, j: i, isRangeStart: a, strokeWidth: o = 2 }) {
		...
		if (r.globals.collapsedSeriesIndices.indexOf(s) !== -1 || !l.enabled || !Array.isArray(e.x)) return p;
		p = n.group({ class: "apexcharts-data-labels" });
		for (let f = 0; f < e.x.length; f++) if (c = e.x[f] + l.offsetX, h = e.y[f] + l.offsetY + o, !isNaN(c)) {
			...
			r.config.chart.type === "bubble" ? (x = r.seriesData.seriesZ[s][d], m = u(x), h = e.y[f], h = new ge(this.w, this.ctx).centerTextInBubble(h, s, d).y) : typeof x < "u" && (m = u(x));
	...
	plotDataLabelsText(t) {
		...
		let { x: i, y: a, i: o, j: r, text: n, textAnchor: l, fontSize: c, parent: h, dataLabelsConfig: d, color: p, alwaysDrawDataLabel: g, offsetCorrection: f, className: x } = t, m = null;
		if (Array.isArray(e.config.dataLabels.enabledOnSeries) && e.config.dataLabels.enabledOnSeries.indexOf(o) < 0) return m;
		let u = {
			x: i,
			y: a,
			drawnextLabel: !0,
		...
		f && (u = this.dataLabelsCorrection(i, a, n, o, r, g, parseInt(d.style.fontSize, 10))), e.interact.zoomed || (i = u.x, a = u.y), u.textRects && (i < -20 - u.textRects.width || i > e.layout.gridWidth + u.textRects.width + 30) && (n = "");
	...
	addBackgroundToDataLabel(t, e) {
		const s = this.w, i = s.config.dataLabels.background, a = i.padding, o = i.padding / 2, r = e.width, n = e.height, c = new P(this.w).drawRect(e.x - a, e.y - o / 2, r + a * 2, n + o, i.borderRadius, s.config.chart.background === "transparent" || !s.config.chart.background ? "#fff" : s.config.chart.background, i.opacity, i.borderWidth, i.borderColor);
		return i.dropShadow.enabled && new Z(this.w).dropShadow(c, i.dropShadow), c;
	}
	dataLabelsBackground() {
		const t = this.w;
		if (t.config.chart.type === "bubble") return;
		const e = t.dom.baseEl.querySelectorAll(".apexcharts-datalabels text");
	...
	bringForward() {
		const t = this.w, e = t.dom.baseEl.querySelectorAll(".apexcharts-datalabels"), s = t.dom.baseEl.querySelector(".apexcharts-plot-series:last-child");
		for (let i = 0; i < e.length; i++) s && s.insertBefore(e[i], s.nextSibling);
	}
};
var at = class {
	...
	getLabel(t, e, s, i, a = [], o = "12px", r = !0) {
		...
		let u = {};
		n.layout.rotateXLabels && r ? u = m.getTextRects(c, parseInt(o, 10), null, `rotate(${n.config.xaxis.labels.rotate} 0 0)`, !1) : u = m.getTextRects(c, parseInt(o, 10));
	...
	checkLabelBasedOnTickamount(t, e, s) {
		...
		if (a === "dataPoints" && (a = Math.round(i.layout.gridWidth / 120)), a > s) return e;
		return t % Math.round(s / (a + 1)) === 0 || (e.text = ""), e;
	}
	checkForOverflowingLabels(t, e, s, i, a) {
		...
		if (t === 0 && o.globals.skipFirstTimelinelabel && (e.text = ""), t === s - 1 && o.globals.skipLastTimelinelabel && (e.text = ""), o.config.xaxis.labels.hideOverlappingLabels && i.length > 0) {
			...
			if (o.config.xaxis.labels.trim && o.config.xaxis.type !== "datetime") return e;
	...
	getYAxisForeColor(t, e) {
		var s;
		const i = this.w;
		return Array.isArray(t) && i.globals.yAxisScale[e] && ((s = this.theme) == null || s.pushExtraColors(t, i.globals.yAxisScale[e].result.length, !1)), t;
	}
	drawYAxisTicks(t, e, s, i, a, o, r) {
		...
		if (n.globals.isBarHorizontal ? c = 0 : n.config.chart.type === "heatmap" && (c += o / 2), i.show && e > 0) {
			...
			for (let h = e; h >= 0; h--) {
				const d = l.drawLine(t + s.offsetX - i.width + i.offsetX, c + i.offsetY, t + s.offsetX + i.offsetX, c + i.offsetY, i.color);
				r.add(d), c += o;
			}
		}
	}
...
var wt = class {
	constructor(t, e, s) {
		this.w = t, this.ctx = e, this.elgrid = s, this.axesUtils = new at(t, {
			theme: e.theme,
	...
	drawXaxis() {
		...
		s.add(i);
		let a = [];
		for (let o = 0; o < this.xaxisLabels.length; o++) a.push(this.xaxisLabels[o]);
		if (this.drawXAxisLabelAndGroup(!0, e, i, a, t.axisFlags.isXNumeric, (o, r) => r), t.labelData.hasXaxisGroups) {
			const o = t.labelData.groups;
			a = [];
			for (let n = 0; n < o.length; n++) a.push(o[n].title);
			const r = {};
			t.config.xaxis.group.style && (r.xaxisFontSize = t.config.xaxis.group.style.fontSize, r.xaxisFontFamily = t.config.xaxis.group.style.fontFamily, r.xaxisForeColors = t.config.xaxis.group.style.colors, r.fontWeight = t.config.xaxis.group.style.fontWeight, r.cssClass = t.config.xaxis.group.style.cssClass), this.drawXAxisLabelAndGroup(!1, e, i, a, !1, (n, l) => o[n].cols * l, r);
		}
		if (t.config.xaxis.title.text !== void 0) {
			const o = e.group({ class: "apexcharts-xaxis-title" }), r = e.drawText({
		...
		if (t.config.xaxis.axisBorder.show) {
			const o = t.globals.barPadForNumericAxis, r = e.drawLine(t.globals.padHorizontal + t.config.xaxis.axisBorder.offsetX - o, this.offY, this.xaxisBorderWidth + o, this.offY, t.config.xaxis.axisBorder.color, 0, this.xaxisBorderHeight);
			...
			if (c.config.xaxis.labels.show) {
				...
				if (s.add(k), k.on("click", (F) => {
					if (typeof c.config.chart.events.xAxisLabelClick == "function") {
						const T = Object.assign({}, c, { labelIndex: b });
						c.config.chart.events.xAxisLabelClick(F, this.ctx, T);
					}
				}), t) {
					const F = N.createElementNS(G, "title");
	...
	drawXaxisInversed(t) {
		...
		a.add(o);
		const r = [];
		if (e.config.yaxis[t].show) for (let g = 0; g < this.xaxisLabels.length; g++) r.push(this.xaxisLabels[g]);
		const n = e.layout.gridHeight / r.length;
		let l = -(n / 2.2);
		const c = e.formatters.yLabelFormatters[0], h = e.config.yaxis[0].labels;
		if (h.show) for (let g = 0; g <= r.length - 1; g++) {
			...
			const x = this.axesUtils.getYAxisForeColor(h.style.colors, t), m = () => Array.isArray(x) ? x[g] : x;
			let u = 0;
			...
			o.add(w), w.on("click", (C) => {
				if (typeof e.config.chart.events.xAxisLabelClick == "function") {
					const D = Object.assign({}, e, { labelIndex: g });
			...
			if (v.textContent = Array.isArray(f) ? f.join(" ") : f, w.node.appendChild(v), e.config.yaxis[t].labels.rotate !== 0) {
				...
				w.node.setAttribute("transform", `rotate(${e.config.yaxis[t].labels.rotate} 0 ${C.y})`);
			}
			l = l + n;
		}
		...
		if (p.show) {
			const g = s.drawLine(e.globals.padHorizontal + p.offsetX + d, 1 + p.offsetY, e.globals.padHorizontal + p.offsetX + d, e.layout.gridHeight + p.offsetY, p.color, 0);
			this.elgrid && this.elgrid.elGridBorders && e.config.grid.show ? this.elgrid.elGridBorders.add(g) : a.add(g);
		}
		return e.config.yaxis[0].axisTicks.show && this.axesUtils.drawYAxisTicks(d, r.length, e.config.yaxis[0].axisBorder, e.config.yaxis[0].axisTicks, 0, n, a), a;
	}
	drawXaxisTicks(t, e, s) {
		...
		if (e = e + o + i.config.xaxis.axisTicks.height, i.config.xaxis.position === "top" && (e = o - i.config.xaxis.axisTicks.height), i.config.xaxis.axisTicks.show) {
			const n = new P(this.w).drawLine(t + i.config.xaxis.axisTicks.offsetX, o + i.config.xaxis.offsetY, a + i.config.xaxis.axisTicks.offsetX, e + i.config.xaxis.offsetY, i.config.xaxis.axisTicks.color);
	...
	getXAxisTicksPositions() {
		...
		if (t.labelData.timescaleLabels.length > 0) for (let a = 0; a < s; a++) i = this.xaxisLabels[a].position, e.push(i);
		else {
			const a = s;
			for (let o = 0; o < a; o++) {
				let r = a;
	...
	xAxisLabelCorrections() {
		const t = this.w, e = new P(this.w), s = t.dom.baseEl.querySelector(".apexcharts-xaxis-texts-g"), i = t.dom.baseEl.querySelectorAll(".apexcharts-xaxis-texts-g text:not(.apexcharts-xaxis-group-label)"), a = t.dom.baseEl.querySelectorAll(".apexcharts-yaxis-inversed text"), o = t.dom.baseEl.querySelectorAll(".apexcharts-xaxis-inversed-texts-g text tspan");
		if (t.layout.rotateXLabels || t.config.xaxis.labels.rotateAlways) for (let r = 0; r < i.length; r++) {
			const n = e.rotateAroundCenter(i[r]);
			n.y = n.y - 1, n.x = n.x + 1, i[r].setAttribute("transform", `rotate(${t.config.xaxis.labels.rotate} ${n.x} ${n.y})`), i[r].setAttribute("text-anchor", "end"), s.setAttribute("transform", "translate(0, -10)");
			const l = i[r].childNodes;
			t.config.xaxis.labels.trim && Array.prototype.forEach.call(l, (c) => {
				e.placeTextWithEllipsis(c, c.textContent, t.layout.xAxisLabelsHeight - (t.config.legend.position === "bottom" ? 20 : 10));
			});
		}
		else {
		...
		if (a.length > 0) {
			...
			r.x < -20 && a[a.length - 1].parentNode.removeChild(a[a.length - 1]), n.x + n.width > t.layout.gridWidth && !t.globals.isBarHorizontal && a[0].parentNode.removeChild(a[0]);
			for (let l = 0; l < o.length; l++) e.placeTextWithEllipsis(o[l], o[l].textContent, t.config.yaxis[0].labels.maxWidth - (t.config.yaxis[0].title.text ? parseFloat(t.config.yaxis[0].title.style.fontSize) * 2 : 0) - 15);
		}
	}
};
var fe = class {
	constructor(t, e) {
		this.w = t, this.ctx = e, this.xaxisLabels = t.labelData.labels.slice(), this.axesUtils = new at(e.w, {
			theme: e.theme,
			timeScale: e.timeScale
		}), this.isRangeBar = t.rangeData.seriesRange.length && t.globals.isBarHorizontal, t.labelData.timescaleLabels.length > 0 && (this.xaxisLabels = t.labelData.timescaleLabels.slice());
	}
	drawGridArea(t = null) {
		const e = this.w, s = new P(this.w);
		t || (t = s.group({ class: "apexcharts-grid" }));
		const i = s.drawLine(e.globals.padHorizontal, 1, e.globals.padHorizontal, e.layout.gridHeight, "transparent"), a = s.drawLine(e.globals.padHorizontal, e.layout.gridHeight, e.layout.gridWidth, e.layout.gridHeight, "transparent");
	...
	createGridMask() {
		...
		const o = [
			...
			"candlestick",
			"boxPlot"
		].includes(t.config.chart.type) || t.globals.comboBarCount > 0;
		let r = 0, n = 0;
		o && t.axisFlags.isXNumeric && !t.globals.isBarHorizontal && (r = Math.max(t.config.grid.padding.left, e.barPadForNumericAxis), n = Math.max(t.config.grid.padding.right, e.barPadForNumericAxis)), t.dom.elGridRect = s.drawRect(-i / 2 - 2, -i / 2 - 2, t.layout.gridWidth + i + 4, t.layout.gridHeight + i + 4, 0, "#fff"), t.dom.elGridRectBar = s.drawRect(-i / 2 - r - 2, -i / 2 - 2, t.layout.gridWidth + i + n + r + 4, t.layout.gridHeight + i + 4, 0, "#fff");
		const l = t.globals.markers.largestSize;
		t.dom.elGridRectMarker = s.drawRect(Math.min(-i / 2 - r - 2, -l), -l, t.layout.gridWidth + Math.max(i + n + r + 4, l * 2), t.layout.gridHeight + l * 2, 0, "#fff"), t.dom.elGridRectMask.appendChild(t.dom.elGridRect.node), t.dom.elGridRectBarMask.appendChild(t.dom.elGridRectBar.node), t.dom.elGridRectMarkerMask.appendChild(t.dom.elGridRectMarker.node);
	...
	_drawGridLines({ i: t, x1: e, y1: s, x2: i, y2: a, xCount: o, parent: r }) {
		...
		if ((() => !(t === 0 && n.globals.skipFirstTimelinelabel || t === o - 1 && n.globals.skipLastTimelinelabel && !n.config.xaxis.labels.formatter || n.config.chart.type === "radar"))()) {
			...
			if (n.labelData.hasXaxisGroups && n.config.xaxis.tickPlacement === "between") {
				...
				if (d) {
					let p = 0;
					for (let g = 0; p < t && g < d.length; g++) p += d[g].cols;
					p === t && (c = n.layout.xAxisLabelsHeight * .6);
				}
			}
	...
	_drawGridLine({ i: t, x1: e, y1: s, x2: i, y2: a, xCount: o, parent: r }) {
		const n = this.w, l = r.node.classList.contains("apexcharts-gridlines-horizontal"), c = n.globals.barPadForNumericAxis, h = s === 0 && a === 0 || e === 0 && i === 0 || s === n.layout.gridHeight && a === n.layout.gridHeight || n.globals.isBarHorizontal && (t === 0 || t === o - 1), p = new P(this.w).drawLine(e - (l ? c : 0), s, i + (l ? c : 0), a, n.config.grid.borderColor, n.config.grid.strokeDashArray);
	...
	_drawGridBandRect({ c: t, x1: e, y1: s, x2: i, y2: a, type: o }) {
		const r = this.w, n = new P(this.w), l = r.globals.barPadForNumericAxis, c = r.config.grid[o].colors[t], h = n.drawRect(e - (o === "row" ? l : 0), s, i + (o === "row" ? l * 2 : 0), a, 0, c, r.config.grid[o].opacity);
	...
	_drawXYLines({ xCount: t, tickAmount: e }) {
		const s = this.w, i = ({ xC: o, x1: r, y1: n, x2: l, y2: c }) => {
			for (let h = 0; h < o; h++) r = this.xaxisLabels[h].position, l = this.xaxisLabels[h].position, this._drawGridLines({
				i: h,
				x1: r,
				y1: n,
				x2: l,
			...
			for (let h = 0; h < o + (s.axisFlags.isXNumeric ? 0 : 1); h++) h === 0 && o === 1 && s.globals.dataPoints === 1 && (r = s.layout.gridWidth / 2, l = r), this._drawGridLines({
				i: h,
				x1: r,
				y1: n,
				x2: l,
		...
		if (s.config.grid.xaxis.lines.show || s.config.xaxis.axisTicks.show) {
			...
			s.labelData.timescaleLabels.length ? i({
				xC: t,
				x1: o,
				y1: r,
				x2: n,
				y2: l
			}) : (s.axisFlags.isXNumeric && (t = s.globals.xAxisScale.result.length), a({
				xC: t,
				x1: o,
				y1: r,
				x2: n,
		...
		if (s.config.grid.yaxis.lines.show) {
			...
			for (let h = 0; h < c + (this.isRangeBar ? 1 : 0); h++) this._drawGridLine({
				i: h,
	...
	_drawInvertedXYLines({ xCount: t }) {
		...
		if (e.config.grid.xaxis.lines.show || e.config.xaxis.axisTicks.show) {
			...
			for (let r = 0; r < t + 1; r++) e.config.grid.xaxis.lines.show && this._drawGridLine({
				i: r,
				xCount: t + 1,
				x1: s,
				y1: i,
		...
		if (e.config.grid.yaxis.lines.show) {
			...
			for (let r = 0; r < e.globals.dataPoints + 1; r++) this._drawGridLine({
				i: r,
	...
	renderGrid() {
		...
		this.elg = o.group({ class: "apexcharts-grid" }), this.elgridLinesH = o.group({ class: "apexcharts-gridlines-horizontal" }), this.elgridLinesV = o.group({ class: "apexcharts-gridlines-vertical" }), this.elGridBorders = o.group({ class: "apexcharts-grid-borders" }), this.elg.add(this.elgridLinesH), this.elg.add(this.elgridLinesV), i.config.grid.show || (this.elgridLinesV.hide(), this.elgridLinesH.hide(), this.elGridBorders.hide());
		let r = 0;
		for (; r < a.seriesYAxisMap.length && a.ignoreYAxisIndexes.includes(r);) r++;
		r === a.seriesYAxisMap.length && (r = 0);
		let n = a.yAxisScale[r].result.length - 1, l;
		return !a.isBarHorizontal || this.isRangeBar ? (l = this.xaxisLabels.length, this.isRangeBar && (n = i.labelData.labels.length, i.config.xaxis.tickAmount && i.config.xaxis.labels.formatter && (l = i.config.xaxis.tickAmount), ((s = (e = (t = a.yAxisScale) == null ? void 0 : t[r]) == null ? void 0 : e.result) == null ? void 0 : s.length) > 0 && i.config.xaxis.type !== "datetime" && (l = a.yAxisScale[r].result.length - 1)), this._drawXYLines({
			xCount: l,
			tickAmount: n
		})) : (l = n, n = a.xTickAmount, this._drawInvertedXYLines({
			xCount: l,
			tickAmount: n
		})), this.drawGridBands(l, n), {
			el: this.elg,
	...
	drawGridBands(t, e) {
		...
		const o = this.w, r = (n, l, c, h, d, p) => {
			for (let g = 0, f = 0; g < l; g++, f++) f >= o.config.grid[n].colors.length && (f = 0), this._drawGridBandRect({
				c: f,
				x1: c,
				y1: h,
				x2: d,
				y2: p,
				type: n
			}), h += o.layout.gridHeight / e;
		};
		if (((s = o.config.grid.row.colors) == null ? void 0 : s.length) > 0 && r("row", e, 0, 0, o.layout.gridWidth, o.layout.gridHeight / e), ((i = o.config.grid.column.colors) == null ? void 0 : i.length) > 0) {
			...
			for (let p = 0, g = 0; p < t; p++, g++) g >= o.config.grid.column.colors.length && (g = 0), o.config.xaxis.type === "datetime" && (l = this.xaxisLabels[p].position, h = (((a = this.xaxisLabels[p + 1]) == null ? void 0 : a.position) || o.layout.gridWidth) - this.xaxisLabels[p].position), this._drawGridBandRect({
				c: g,
				x1: l,
				y1: c,
				x2: h,
...
var pe = class {
	...
	niceScale(t, e, s = 0) {
		...
		let h = r.stepSize !== void 0 && r.stepSize !== null, d = r.tickAmount !== void 0 && r.tickAmount !== null, p = d ? r.tickAmount : Rt[Math.min(Math.round(n / 2), Rt.length - 1)];
		...
		let v = Math.ceil(y / w);
		if (v = he[o.yValueDecimal === 0 ? 0 : 1][v], y = v * w, u = y, o.isBarHorizontal && r.stepSize && r.type !== "datetime" ? (u = r.stepSize, h = !0) : h && (u = r.stepSize), h && r.forceNiceScale) u *= Math.pow(10, b - Math.floor(Math.log10(u)));
		if (l && c) {
			let T = x / f;
			if (d) if (h) if (L.mod(x, u) != 0) {
				const M = L.getGCD(u, T);
				T / M < 10 ? u = M : u = T;
			} else L.mod(u, T) == 0 ? u = T : (T = u, d = !1);
			else u = T;
			else if (h) L.mod(x, u) == 0 ? T = u : u = T;
			else if (L.mod(x, u) == 0) T = u;
			else {
		...
		if (!d && !(l || c) && (f = Math.ceil((x - 1e-11) / (u + 1e-11)), f > 16 && L.getPrimeFactors(f).length < 2 && f++), !d && r.forceNiceScale && o.yValueDecimal === 0 && f > x && (f = x, u = Math.round(x / f)), f > n && (!(d || h) || r.forceNiceScale)) {
			...
			let E = f;
			t: for (var C = 0; C < M; C++) for (var D = 0; D <= M - C; D++) {
				const X = Math.min(D + C, M);
				let R = E, Y = 1;
				for (var S = D; S <= X; S++) Y *= T[S];
				if (R /= Y, R < n) {
					E = R;
					break t;
				}
	...
	linearScale(t, e, s = 10, i = 0, a = void 0) {
		...
		const n = 2;
		if (a !== 0 && isFinite(a)) {
			const h = Math.max(n, -Math.floor(Math.log10(Math.abs(a))) + n), d = Math.pow(10, h);
			a = Math.round((a + Number.EPSILON) * d) / d;
		}
		s === Number.MAX_VALUE && (s = 5, a = 1);
		let l = t;
		for (; s >= 0;) r.push(l), l = L.preciseAddition(l, a), s -= 1;
		return {
			result: r,
			niceMin: r[0],
			niceMax: r[r.length - 1]
	...
	logarithmicScale(t, e, s) {
		...
		for (let c = 0, h = o; c < n; c++, h += l) i.push(Math.pow(s, h));
	...
	_adjustTicksForSmallRange(t, e, s) {
		...
		if (typeof e < "u" && this.w.config.yaxis[e].labels.formatter && this.w.config.yaxis[e].tickAmount === void 0) {
			const a = Number(this.w.config.yaxis[e].labels.formatter(1));
	...
	setXScale(t, e) {
		...
		if (e === -Number.MAX_VALUE || !L.isNumber(e)) i.xAxisScale = this.linearScale(0, 10, 10);
		else {
			const a = i.xTickAmount;
			i.xAxisScale = this.linearScale(t, e, a, 0, s.config.xaxis.max === void 0 ? s.config.xaxis.stepSize : void 0);
		}
		return i.xAxisScale;
	}
	scaleMultipleYAxes() {
		...
		e.allSeriesCollapsed = !0, e.barGroups = [], s.forEach((o, r) => {
				...
				if (t.chart.stacked) {
					...
					for (let y = 0; y < o.length; y++) {
						...
						t.series[b].group ? g = t.series[b].group : g = "axis-".concat(r), !(e.collapsedSeriesIndices.indexOf(b) < 0 && e.ancillaryCollapsedSeriesIndices.indexOf(b) < 0) || (e.allSeriesCollapsed = !1, n.forEach((v, C) => {
							if (t.series[b].group === v) for (let D = 0; D < this.w.seriesData.series[b].length; D++) {
					...
					p || (p = t.chart.type), p === "bar" || p === "column" ? n.forEach((y, b) => {
						...
						h = Math.min(h, Math.min.apply(null, x[b])), d = Math.max(d, Math.max.apply(null, x[b]));
					}), l = h, c = d), l === Number.MIN_VALUE && c === Number.MIN_VALUE && (c = -Number.MAX_VALUE);
				} else for (let f = 0; f < o.length; f++) {
					const x = o[f];
			...
			} else this.setYScaleForIndex(r, 0, -Number.MAX_VALUE);
		});
	}
};
var It = class {
	...
	getMinYMaxY(t, e = Number.MAX_VALUE, s = -Number.MAX_VALUE, i = null) {
		...
		for (let u = t; u < i; u++) {
			...
			this.w.labelData.categoryLabels.length && (h.dataPoints = this.w.labelData.categoryLabels.filter((v) => typeof v < "u").length), this.w.labelData.labels.length && c.xaxis.type !== "datetime" && this.w.seriesData.series.reduce((v, C) => v + C.length, 0) !== 0 && (h.dataPoints = Math.max(h.dataPoints, this.w.labelData.labels.length));
			let b = 0, w = g[u].length - 1;
			if (m) {
				if (c.xaxis.min) for (; b < w && this.w.seriesData.seriesX[u][b] < c.xaxis.min; b++);
				if (c.xaxis.max) for (; w > b && this.w.seriesData.seriesX[u][w] > c.xaxis.max; w--);
			}
			for (let v = b; v <= w && v < this.w.seriesData.series[u].length; v++) {
				...
				if (C !== null && L.isNumber(C)) {
					...
					}), s = d, C = L.noExponents(C), L.isFloat(C) && (h.yValueDecimal = Math.max(h.yValueDecimal, C.toString().split(".")[1].length)), p > ((n = f[u]) == null ? void 0 : n[v]) && ((l = f[u]) == null ? void 0 : l[v]) < 0 && (p = f[u][v]);
				} else h.hasNullValues = !0;
			}
	...
	setYRange() {
		...
		if (t.isMultipleYAxis) {
			s = Number.MAX_VALUE;
			for (let a = 0; a < this.w.seriesData.series.length; a++) i = this.getMinYMaxY(a), t.minYArr[a] = i.lowestY, t.maxYArr[a] = i.highestY, s = Math.min(s, i.lowestY);
		}
		return i = this.getMinYMaxY(0, s, null, this.w.seriesData.series.length), e.chart.type === "bar" ? (t.minY = i.minY, t.maxY = i.maxY) : (t.minY = i.lowestY, t.maxY = i.highestY), s = i.lowestY, e.chart.stacked && this._setStackedMinMax(), e.chart.type === "line" || e.chart.type === "area" || e.chart.type === "scatter" || e.chart.type === "candlestick" || e.chart.type === "boxPlot" || e.chart.type === "rangeBar" && !t.isBarHorizontal ? t.minY === Number.MIN_VALUE && s !== -Number.MAX_VALUE && s !== t.maxY && (t.minY = s) : t.minY = t.minY !== Number.MIN_VALUE ? Math.min(i.minY, t.minY) : i.minY, e.yaxis.forEach((a, o) => {
			a.max !== void 0 && (typeof a.max == "number" ? t.maxYArr[o] = a.max : typeof a.max == "function" && (t.maxYArr[o] = a.max(t.isMultipleYAxis ? t.maxYArr[o] : t.maxY)), t.maxY = t.maxYArr[o]), a.min !== void 0 && (typeof a.min == "number" ? t.minYArr[o] = a.min : typeof a.min == "function" && (t.minYArr[o] = a.min(t.isMultipleYAxis ? t.minYArr[o] === Number.MIN_VALUE ? 0 : t.minYArr[o] : t.minY)), t.minY = t.minYArr[o]);
	...
	setXRange() {
		const t = this.w.globals, e = this.w.config, s = e.xaxis.type === "numeric" || e.xaxis.type === "datetime" || e.xaxis.type === "category" && !this.w.axisFlags.noLabelsProvided || this.w.axisFlags.noLabelsProvided || this.w.axisFlags.isXNumeric, i = () => {
			for (let a = 0; a < this.w.seriesData.series.length; a++) if (this.w.labelData.labels[a]) for (let o = 0; o < this.w.labelData.labels[a].length; o++) this.w.labelData.labels[a][o] !== null && L.isNumber(this.w.labelData.labels[a][o]) && (t.maxX = Math.max(t.maxX, this.w.labelData.labels[a][o]), t.initialMaxX = Math.max(t.maxX, this.w.labelData.labels[a][o]), t.minX = Math.min(t.minX, this.w.labelData.labels[a][o]), t.initialMinX = Math.min(t.minX, this.w.labelData.labels[a][o]));
		};
		if (this.w.axisFlags.isXNumeric && i(), this.w.axisFlags.noLabelsProvided && e.xaxis.categories.length === 0 && (t.maxX = this.w.labelData.labels[this.w.labelData.labels.length - 1], t.initialMaxX = this.w.labelData.labels[this.w.labelData.labels.length - 1], t.minX = 1, t.initialMinX = 1), this.w.axisFlags.isXNumeric || this.w.axisFlags.noLabelsProvided || this.w.axisFlags.dataFormatXNumeric) {
			let a = 10;
			if (e.xaxis.tickAmount === void 0) a = Math.round(t.svgWidth / 150), e.xaxis.type === "numeric" && t.dataPoints < 30 && (a = t.dataPoints - 1), a > t.dataPoints && t.dataPoints !== 0 && (a = t.dataPoints - 1);
			...
			if (t.xTickAmount = a, e.xaxis.max !== void 0 && typeof e.xaxis.max == "number" && (t.maxX = e.xaxis.max), e.xaxis.min !== void 0 && typeof e.xaxis.min == "number" && (t.minX = e.xaxis.min), e.xaxis.range !== void 0 && (t.minX = t.maxX - e.xaxis.range), t.minX !== Number.MAX_VALUE && t.maxX !== -Number.MAX_VALUE) if (e.xaxis.convertedCatToNumeric && !this.w.axisFlags.dataFormatXNumeric) {
				const o = [];
			...
			else t.xAxisScale = this.scales.linearScale(0, a, a, 0, e.xaxis.stepSize), this.w.axisFlags.noLabelsProvided && this.w.labelData.labels.length > 0 && (t.xAxisScale = this.scales.linearScale(1, this.w.labelData.labels.length, a - 1, 0, e.xaxis.stepSize), this.w.seriesData.seriesX = this.w.labelData.labels.slice());
	...
	setZRange() {
		...
		if (this.w.axisFlags.isDataXYZ) {
			for (let e = 0; e < this.w.seriesData.series.length; e++) if (typeof this.w.seriesData.seriesZ[e] < "u") for (let s = 0; s < this.w.seriesData.seriesZ[e].length; s++) this.w.seriesData.seriesZ[e][s] !== null && L.isNumber(this.w.seriesData.seriesZ[e][s]) && (t.maxZ = Math.max(t.maxZ, this.w.seriesData.seriesZ[e][s]), t.minZ = Math.min(t.minZ, this.w.seriesData.seriesZ[e][s]));
		}
	}
	_handleSingleDataPoint() {
		...
		if (t.minX === t.maxX) {
			...
			} else (e.xaxis.type === "numeric" || e.xaxis.type === "category" && !this.w.axisFlags.noLabelsProvided) && (t.minX = t.minX - 2, t.initialMinX = t.minX, t.maxX = t.maxX + 2, t.initialMaxX = t.maxX);
		}
	}
	_getMinXDiff() {
		const t = this.w.globals;
		this.w.axisFlags.isXNumeric && this.w.seriesData.seriesX.forEach((e) => {
			if (e.length) {
				e.length === 1 && e.push(this.w.seriesData.seriesX[t.maxValsInArrayIndex][this.w.seriesData.seriesX[t.maxValsInArrayIndex].length - 1]);
				const s = e.slice();
				...
				}), (t.dataPoints === 1 || t.minXDiff === Number.MAX_VALUE) && (t.minXDiff = .5);
			}
		});
	}
	_setStackedMinMax() {
		...
		const s = {}, i = {};
		e.forEach((a) => {
			s[a] = [], i[a] = [], this.w.config.series.map((r, n) => a.indexOf(this.w.seriesData.seriesNames[n]) > -1 ? n : null).filter((r) => r !== null).forEach((r) => {
				var n, l, c, h;
				for (let d = 0; d < this.w.seriesData.series[t.maxValsInArrayIndex].length; d++) typeof s[a][d] > "u" && (s[a][d] = 0, i[a][d] = 0), (this.w.config.chart.stacked && !t.comboCharts || this.w.config.chart.stacked && t.comboCharts && (!this.w.config.chart.stackOnlyBar || ((l = (n = this.w.config.series) == null ? void 0 : n[r]) == null ? void 0 : l.type) === "bar" || ((h = (c = this.w.config.series) == null ? void 0 : c[r]) == null ? void 0 : h.type) === "column")) && this.w.seriesData.series[r][d] !== null && L.isNumber(this.w.seriesData.series[r][d]) && (this.w.seriesData.series[r][d] > 0 ? s[a][d] += parseFloat(this.w.seriesData.series[r][d]) + 1e-4 : i[a][d] += parseFloat(this.w.seriesData.series[r][d]));
			});
...
var _t = class {
	constructor(t, { theme: e = null, timeScale: s = null } = {}, i) {
		this.w = t, this.elgrid = i, this.xaxisFontSize = t.config.xaxis.labels.style.fontSize, this.axisFontFamily = t.config.xaxis.labels.style.fontFamily, this.xaxisForeColors = t.config.xaxis.labels.style.colors, this.isCategoryBarHorizontal = t.config.chart.type === "bar" && t.config.plotOptions.bar.horizontal, this.xAxisoffX = t.config.xaxis.position === "bottom" ? t.layout.gridHeight : 0, this.drawnLabels = [], this.axesUtils = new at(t, {
			theme: e,
			timeScale: s
		});
	}
	drawYaxis(t) {
		const e = this.w, s = new P(this.w), i = e.config.yaxis[t].labels.style, { fontSize: a, fontFamily: o, fontWeight: r } = i, n = s.group({
		...
		n.add(l);
		const c = e.globals.yAxisScale[t].result.length - 1, h = e.layout.gridHeight / c, d = e.formatters.yLabelFormatters[t], p = this.axesUtils.checkForReversedLabels(t, e.globals.yAxisScale[t].result.slice());
		if (e.config.yaxis[t].labels.show) {
			...
			for (let x = c; x >= 0; x--) {
				...
				const y = this.getTextAnchor(e.config.yaxis[t].labels.align, e.config.yaxis[t].opposite), b = this.axesUtils.getYAxisForeColor(i.colors, t), w = Array.isArray(b) ? b[x] : b, v = Array.from(e.dom.baseEl.querySelectorAll(`.apexcharts-yaxis[rel='${t}'] .apexcharts-yaxis-label tspan`)).map((D) => D.textContent), C = s.drawText({
					x: u,
					y: g,
				...
				l.add(C), this.addTooltip(C, m), f === null && (f = C), e.config.yaxis[t].labels.rotate !== 0 && this.rotateLabel(s, C, f, e.config.yaxis[t].labels.rotate), g += h;
			}
		}
	...
	addTooltip(t, e) {
		const s = N.createElementNS(G, "title");
	...
	addAxisBorder(t, e, s, i, a) {
		...
		if (o.config.yaxis[s].opposite && (n = -31 - r.offsetX), r.show) {
			const l = t.drawLine(n, o.layout.translateY + r.offsetY - 2, n, o.layout.gridHeight + o.layout.translateY + r.offsetY + 2, r.color, 0, r.width);
			e.add(l);
		}
		o.config.yaxis[s].axisTicks.show && this.axesUtils.drawYAxisTicks(n, i, r, o.config.yaxis[s].axisTicks, s, a, e);
	}
	drawYaxisInversed(t) {
		const e = this.w, s = new P(this.w), i = s.group({ class: "apexcharts-xaxis apexcharts-yaxis-inversed" }), a = s.group({
			class: "apexcharts-xaxis-texts-g",
			transform: `translate(${e.layout.translateXAxisX}, ${e.layout.translateXAxisY})`
		});
		i.add(a);
		let o = e.globals.yAxisScale[t].result.length - 1;
		const r = e.layout.gridWidth / o + .1;
		let n = r + e.config.xaxis.labels.offsetX;
		const l = e.formatters.xLabelFormatter;
		let c = this.axesUtils.checkForReversedLabels(t, e.globals.yAxisScale[t].result.slice());
		const h = e.labelData.timescaleLabels;
		if (h.length > 0 && (this.xaxisLabels = h.slice(), c = h.slice(), o = c.length), e.config.xaxis.labels.show) for (let d = h.length ? 0 : o; h.length ? d < h.length : d >= 0; h.length ? d++ : d--) {
			let p = l(c[d], d, e), g = e.layout.gridWidth + e.globals.padHorizontal - (n - r + e.config.xaxis.labels.offsetX);
			if (h.length) {
				const x = this.axesUtils.getLabel(c, h, g, d, this.drawnLabels, this.xaxisFontSize);
	...
	inversedYAxisBorder(t) {
		...
		if (i.show) {
			...
			const o = s.drawLine(e.globals.padHorizontal + a + i.offsetX, this.xAxisoffX, e.layout.gridWidth, this.xAxisoffX, i.color, 0, i.height);
	...
	inversedYAxisTitleText(t) {
		...
		if (e.config.xaxis.title.text !== void 0) {
			const i = s.group({ class: "apexcharts-xaxis-title apexcharts-yaxis-title-inversed" }), a = s.drawText({
	...
	yAxisTitleRotate(t, e) {
		const s = this.w, i = new P(this.w), a = s.dom.baseEl.querySelector(`.apexcharts-yaxis[rel='${t}'] .apexcharts-yaxis-texts-g`), o = a ? a.getBoundingClientRect() : {
			width: 0,
			height: 0
		}, r = s.dom.baseEl.querySelector(`.apexcharts-yaxis[rel='${t}'] .apexcharts-yaxis-title text`), n = r ? r.getBoundingClientRect() : {
			width: 0,
			height: 0
		};
		if (r) {
			const l = this.xPaddingForYAxisTitle(t, o, n, e);
			r.setAttribute("x", l.xPos - (e ? 10 : 0));
			const c = i.rotateAroundCenter(r);
			r.setAttribute("transform", `rotate(${e ? s.config.yaxis[t].title.rotate * -1 : s.config.yaxis[t].title.rotate} ${c.x} ${c.y})`);
		}
	}
	xPaddingForYAxisTitle(t, e, s, i) {
		...
		return a.config.yaxis[t].title.text === void 0 || t < 0 ? {
			xPos: o,
			padd: 0
		} : (i ? o = e.width + a.config.yaxis[t].title.offsetX + s.width / 2 + r / 2 : (o = e.width * -1 + a.config.yaxis[t].title.offsetX + r / 2 + s.width / 2, a.globals.isBarHorizontal && (r = 25, o = e.width * -1 - a.config.yaxis[t].title.offsetX - r)), {
			xPos: o,
			padd: r
		});
	}
	...
	setYAxisTextAlignments() {
		const t = this.w;
		Array.from(t.dom.baseEl.getElementsByClassName("apexcharts-yaxis")).forEach((s, i) => {
			const a = t.config.yaxis[i];
			if (a && !a.floating && a.labels.align !== void 0) {
				const o = t.dom.baseEl.querySelector(`.apexcharts-yaxis[rel='${i}'] .apexcharts-yaxis-texts-g`), r = Array.from(t.dom.baseEl.querySelectorAll(`.apexcharts-yaxis[rel='${i}'] .apexcharts-yaxis-label`)), n = o.getBoundingClientRect();
				r.forEach((l) => {
					l.setAttribute("text-anchor", a.labels.align);
				}), a.labels.align === "left" && !a.opposite ? o.setAttribute("transform", `translate(-${n.width}, 0)`) : a.labels.align === "center" ? o.setAttribute("transform", `translate(${n.width / 2 * (a.opposite ? 1 : -1)}, 0)`) : a.labels.align === "right" && a.opposite && o.setAttribute("transform", `translate(${n.width}, 0)`);
			}
		});
	}
};
	...
	setupEventHandlers() {
		...
		this.ctx.eventList.forEach((i) => {
			s.addEventListener(i, (a) => {
				...
				a.type === "keydown" ? t.config.chart.accessibility.enabled && t.config.chart.accessibility.keyboard.enabled && (e.ctx.keyboardNavigation && e.ctx.keyboardNavigation.handleKey(a), typeof t.config.chart.events.keyDown == "function" && t.config.chart.events.keyDown(a, e, n), e.ctx.events.fireEvent("keydown", [
					a,
					e,
					n
				])) : a.type === "keyup" ? t.config.chart.accessibility.enabled && t.config.chart.accessibility.keyboard.enabled && (typeof t.config.chart.events.keyUp == "function" && t.config.chart.events.keyUp(a, e, n), e.ctx.events.fireEvent("keyup", [
					a,
					e,
					n
				])) : a.type === "mousemove" || a.type === "touchmove" ? typeof t.config.chart.events.mouseMove == "function" && t.config.chart.events.mouseMove(a, e, n) : a.type === "mouseleave" || a.type === "touchleave" ? typeof t.config.chart.events.mouseLeave == "function" && t.config.chart.events.mouseLeave(a, e, n) : (a.type === "mouseup" && a.which === 1 || a.type === "touchend") && (typeof t.config.chart.events.click == "function" && t.config.chart.events.click(a, e, n), e.ctx.events.fireEvent("click", [
					a,
					e,
					n
				]));
...
var Oe = class {
	...
	drawAxis(t, e) {
		const s = this.w.globals, i = this.w.config, a = new wt(this.w, this.ctx, e), o = new _t(this.w, {
...
var Bt = class {
	...
	drawXCrosshairs() {
		...
		if (t.config.xaxis.crosshairs.show) {
			o === "gradient" && (y = e.drawGradient("vertical", r, n, l, c, null, h, null));
			let b = e.drawRect();
	...
	drawYCrosshairs() {
		...
		if (t.config.yaxis[0].crosshairs.show) {
			const o = e.drawLine(-i, 0, t.layout.gridWidth + i, 0, s.stroke.color, s.stroke.dashArray, s.stroke.width);
			o.attr({ class: "apexcharts-ycrosshairs" }), t.dom.elGraphical.add(o);
		}
		const a = e.drawLine(-i, 0, t.layout.gridWidth + i, 0, s.stroke.color, 0, 0);
		a.attr({ class: "apexcharts-ycrosshairs-hidden" }), t.dom.elGraphical.add(a);
	}
};
var _e = class {
	constructor(t) {
	...
	checkResponsiveConfig(t) {
		...
		i.sort((r, n) => r.breakpoint > n.breakpoint ? 1 : n.breakpoint > r.breakpoint ? -1 : 0).reverse();
		const a = new yt({}), o = (r = {}) => {
			...
			if (l > n) {
				...
				const h = V.extendArrayProps(a, c, e);
				r = L.extend(h, r), r = L.extend(e.config, r), this.overrideResponsiveOptions(r);
			} else for (let c = 0; c < i.length; c++) l < i[c].breakpoint && (r = V.extendArrayProps(a, i[c].options, e), r = L.extend(e.config, r), this.overrideResponsiveOptions(r));
		};
		if (t) {
...
var U = class U {
	...
	getSeriesByName(t) {
		return this.w.dom.baseEl.querySelector(`.apexcharts-inner .apexcharts-series[seriesName='${L.escapeString(t)}']`);
	}
	isSeriesHidden(t) {
	...
	static addCollapsedClassToSeries(t, e, s) {
		function i(a) {
			for (let o = 0; o < a.length; o++) a[o].index === s && e.node.classList.add("apexcharts-series-collapsed");
		}
	...
	emptyCollapsedSeries(t) {
		const e = this.w;
		for (let s = 0; s < t.length; s++) e.globals.collapsedSeriesIndices.indexOf(s) > -1 && (t[s].data = []);
		return t;
	}
	highlightSeries(t) {
		...
		o || (o = e.dom.baseEl.querySelectorAll(".apexcharts-series, .apexcharts-datalabels, .apexcharts-yaxis"), e.globals.cachedSelectors[a] = o);
		let r = null, n = null, l = null;
		if (e.globals.axisCharts || e.config.chart.type === "radialBar") if (e.globals.axisCharts) {
			r = e.dom.baseEl.querySelector(`.apexcharts-series[data\\:realIndex='${i}']`), n = e.dom.baseEl.querySelector(`.apexcharts-datalabels[data\\:realIndex='${i}']`);
			const c = e.globals.seriesYAxisReverseMap[i];
			l = e.dom.baseEl.querySelector(`.apexcharts-yaxis[rel='${c}']`);
		} else r = e.dom.baseEl.querySelector(`.apexcharts-series[rel='${i + 1}']`);
		else r = e.dom.baseEl.querySelector(`.apexcharts-series[rel='${i + 1}'] path`);
		for (let c = 0; c < o.length; c++) o[c].classList.add(this.legendInactiveClass);
		if (r) e.globals.axisCharts || r.parentNode.classList.remove(this.legendInactiveClass), r.classList.remove(this.legendInactiveClass), n !== null && n.classList.remove(this.legendInactiveClass), l !== null && l.classList.remove(this.legendInactiveClass);
		else for (let c = 0; c < o.length; c++) o[c].classList.remove(this.legendInactiveClass);
	}
	toggleSeriesOnHover(t, e) {
		const s = this.w;
		e || (e = t.target);
		const i = s.dom.baseEl.querySelectorAll(".apexcharts-series, .apexcharts-datalabels, .apexcharts-yaxis");
		if (t.type === "mousemove") {
			const a = parseInt(e.getAttribute("rel"), 10) - 1;
			this.highlightSeries(s.seriesData.seriesNames[a]);
		} else if (t.type === "mouseout") for (let a = 0; a < i.length; a++) i[a].classList.remove(this.legendInactiveClass);
	}
	highlightRangeInSeries(t, e) {
		const s = this.w, i = s.dom.baseEl.getElementsByClassName("apexcharts-heatmap-rect"), a = (r) => {
			for (let n = 0; n < i.length; n++) i[n].classList[r](this.legendInactiveClass);
		}, o = (r, n) => {
		...
		if (t.type === "mousemove") {
			...
			a("add");
			const n = s.config.plotOptions.heatmap.colorScale.ranges, l = n[r];
			o(l, n.reduce((h, d) => Math.max(h, d.to), 0));
		} else t.type === "mouseout" && a("remove");
	}
	getActiveConfigSeriesIndex(t = "asc", e = []) {
		...
		if (s.config.series.length > 1) {
			...
			for (let o = t === "asc" ? 0 : a.length - 1; t === "asc" ? o < a.length : o >= 0; t === "asc" ? o++ : o--) if (a[o] !== -1) {
				i = a[o];
				break;
			}
		}
	...
	getPreviousPaths() {
		...
		function e(o, r, n) {
			...
			for (let h = 0; h < l.length; h++) if (l[h].hasAttribute("pathTo")) {
				const d = l[h].getAttribute("pathTo");
				c.paths.push({ d });
			}
			t.globals.previousPaths.push(c);
		}
		const s = (o) => t.dom.baseEl.querySelectorAll(`.apexcharts-${o}-series .apexcharts-series`);
		[
			"line",
			"area",
			"bar",
			...
			const r = s(o);
			for (let n = 0; n < r.length; n++) e(r, n, o);
		});
		const a = t.dom.baseEl.querySelectorAll(`.apexcharts-${t.config.chart.type} .apexcharts-series`);
		if (a.length > 0) for (let o = 0; o < a.length; o++) {
			const r = t.dom.baseEl.querySelectorAll(`.apexcharts-${t.config.chart.type} .apexcharts-series[data\\:realIndex='${o}'] rect`), n = [];
	...
	setNullSeriesToZeroValues(t) {
		const e = this.w;
		for (let s = 0; s < t.length; s++) if (t[s].length === 0) for (let i = 0; i < t[e.globals.maxValsInArrayIndex].length; i++) t[s].push(0);
		return t;
	}
	hasAllSeriesEqualX() {
		let t = !0;
		const e = this.w, s = this.filteredSeriesX();
		for (let i = 0; i < s.length - 1; i++) if (s[i][0] !== s[i + 1][0]) {
			t = !1;
			break;
		}
...
var We = class {
	...
	setDefaultColors() {
		...
		e.dom.elWrap.classList.add(`apexcharts-theme-${e.config.theme.mode || "light"}`);
		...
		e.globals.colors = this.getColors(a), this.applySeriesColors(e.seriesData.seriesColors, e.globals.colors), e.config.theme.monochrome.enabled && (e.globals.colors = this.getMonochromeColors(e.config.theme.monochrome, e.seriesData.series, s));
...
var $e = class {
	...
	**/
	getDatalabelsRect() {
		...
		const s = L.getLargestStringFromArr(e), i = new P(this.w), a = t.config.dataLabels.style, o = i.getTextRects(s, parseInt(a.fontSize), a.fontFamily);
		return {
			width: o.width * 1.05,
			height: o.height
		};
	...
	getLargestStringFromMultiArr(t, e) {
		...
		if (s.axisFlags.isMultiLineX) {
			const a = e.map((n) => Array.isArray(n) ? n.length : 1), o = Math.max(...a);
			i = e[a.indexOf(o)];
		}
		return i;
	}
...
var Ve = class {
	...
	**/
	getxAxisLabelsCoords() {
		...
		if (t.labelData.timescaleLabels.length > 0) {
			...
			let a = L.getLargestStringFromArr(e), o = this.dCtx.dimHelpers.getLargestStringFromMultiArr(a, e);
			t.globals.isBarHorizontal && (a = t.globals.yAxisScale[0].result.reduce((d, p) => d.length > p.length ? d : p, 0), o = a);
			...
			let c = l.getTextRects(a, t.config.xaxis.labels.style.fontSize), h = c;
			if (a !== o && (h = l.getTextRects(o, t.config.xaxis.labels.style.fontSize)), s = {
				...
				if (!t.globals.isBarHorizontal) {
					...
					const d = (p) => l.getTextRects(p, t.config.xaxis.labels.style.fontSize, t.config.xaxis.labels.style.fontFamily, `rotate(${t.config.xaxis.labels.rotate} 0 0)`, !1);
	...
	*/
	getxAxisGroupLabelsCoords() {
		var t;
		const e = this.w;
		if (!e.labelData.hasXaxisGroups) return {
			width: 0,
			height: 0
		};
		...
		let a;
		const o = L.getLargestStringFromArr(i), r = this.dCtx.dimHelpers.getLargestStringFromMultiArr(o, i), n = new P(this.w), l = n.getTextRects(o, s);
		let c = l;
	...
	**/
	getxAxisTitleCoords() {
		...
		let e = 0, s = 0;
		if (t.config.xaxis.title.text !== void 0) {
			const a = new P(this.w).getTextRects(t.config.xaxis.title.text, t.config.xaxis.title.style.fontSize);
	...
	getxAxisTimeScaleLabelsCoords() {
		...
		const e = this.dCtx.timescaleLabels.map((r) => r.value), s = e.reduce((r, n) => typeof r > "u" ? (console.error("You have possibly supplied invalid Date format. Please supply a valid JavaScript Date"), 0) : r.length > n.length ? r : n, 0), a = new P(this.w).getTextRects(s, t.config.xaxis.labels.style.fontSize);
	...
	additionalPaddingXLabels(t) {
		...
		const r = e.config.yaxis[0].opposite && e.globals.isBarHorizontal, n = (h) => s.collapsedSeriesIndices.indexOf(h) !== -1, l = (h) => {
			...
			} else a === "datetime" ? this.dCtx.gridPad.right < o && !e.layout.rotateXLabels && (s.skipLastTimelinelabel = !0) : a !== "datetime" && this.dCtx.gridPad.right < o / 2 - this.dCtx.yAxisWidthRight && !e.layout.rotateXLabels && !e.config.xaxis.labels.trim && (this.dCtx.xPadRight = o / 2 + 1);
		}, c = (h, d) => {
...
var Ue = class {
		...
		return t.config.yaxis.map((r, n) => {
			...
			if (!o.isYAxisHidden(n) && r.labels.show && r.labels.minWidth !== void 0 && (h = r.labels.minWidth), !o.isYAxisHidden(n) && r.labels.show && c.result.length) {
				...
				if ((typeof g > "u" || g.length === 0) && (g = c.niceMax), String(g).length === 1 && (g = g + ".0", f = g), t.globals.isBarHorizontal) {
					...
					g = L.getLargestStringFromArr(b), g = d(g, {
						seriesIndex: n,
						dataPointIndex: -1,
						w: t
					}), f = this.dCtx.dimHelpers.getLargestStringFromMultiArr(g, b);
				}
				const x = new P(this.w), m = "rotate(".concat(r.labels.rotate, " 0 0)"), u = x.getTextRects(g, r.labels.style.fontSize, r.labels.style.fontFamily, m, !1);
				let y = u;
				g !== f && (y = x.getTextRects(f, r.labels.style.fontSize, r.labels.style.fontFamily, m, !1)), i.push({
	...
	**/
	getyAxisTitleCoords() {
		...
		return t.config.yaxis.map((s) => {
			if (s.show && s.title.text !== void 0) {
				const i = new P(this.w), a = "rotate(".concat(s.title.rotate, " 0 0)"), o = i.getTextRects(s.title.text, s.title.style.fontSize, s.title.style.fontFamily, a, !1);
				e.push({
					width: o.width,
					height: o.height
				});
	...
	getTotalYAxisWidth() {
		const t = this.w;
		let e = 0, s = 0, i = 0;
		const a = t.globals.yAxisScale.length > 1 ? 10 : 0, o = new at(this.w, {
			theme: this.dCtx.theme,
			timeScale: this.dCtx.timeScale
		}), r = function(l) {
...
var qe = class {
	...
	gridPadForColumnsInNumericAxis(t) {
		...
		if (i.noData || i.collapsedSeries.length + i.ancillaryCollapsedSeries.length === s.series.length) return 0;
		const a = (h) => [
			"bar",
			"rangeBar",
			"candlestick",
			"boxPlot"
		].includes(h), o = s.chart.type;
	...
	setGridXPosForDualYAxis(t, e) {
		const { w: s } = this, i = new at(this.w, {
			theme: this.dCtx.theme,
			timeScale: this.dCtx.timeScale
		});
	...
	**/
	plotCoords() {
		...
		this.isSparkline && ((t.config.markers.discrete.length > 0 || t.config.markers.size > 0) && Object.entries(this.gridPad).forEach(([a, o]) => {
			this.gridPad[a] = Math.max(o, this.w.globals.markers.largestSize / 1.5);
		...
		return t.layout.gridWidth = t.layout.gridWidth - i * 2, t.layout.translateX = t.layout.translateX + this.gridPad.left + this.xPadLeft + (i > 0 ? i : 0), t.layout.translateY = t.layout.translateY + this.gridPad.top, { layout: {
	...
	setDimensionsForAxisCharts() {
		...
		this.dimGrid.setGridXPosForDualYAxis(i, s), new _t(this.w, {
			theme: this.theme,
...
var je = class {
	...
	calculateTimeScaleTicks(t, e) {
		const s = this.w;
		if (s.globals.allSeriesCollapsed) return s.labelData.labels = [], s.labelData.timescaleLabels = [], [];
		...
		const F = this.timeScaleArray.map((M) => {
			...
			return M.unit === "month" ? O(I({}, E), {
				day: 1,
				value: M.value + 1
			}) : M.unit === "day" || M.unit === "hour" ? O(I({}, E), { value: M.value }) : M.unit === "minute" ? O(I({}, E), {
				value: M.value,
				minute: M.value
			}) : M.unit === "second" ? O(I({}, E), {
				value: M.value,
		...
		return F.filter((M) => {
			...
			if (this.tickInterval === "hours" || this.tickInterval === "minutes_fives" || this.tickInterval === "seconds_tens" || this.tickInterval === "seconds_fives") {
				if (!z) return !0;
			} else if ((R % E === 0 || Y) && !z) return !0;
		});
	}
	recalcDimensionsBasedOnFormat(t) {
		const e = this.w, s = this.formatDates(t), i = this.removeOverlappingTS(s);
	...
	generateYearScale({ firstVal: t, currentMonth: e, currentYear: s, daysWidthOnXAxis: i, numberOfYears: a }) {
		let o = t.minYear, r = 0;
		const n = new q(this.w), l = "year";
		if (t.minDate > 1 || t.minMonth > 0) {
			const d = n.determineRemainingDaysOfYear(t.minYear, t.minMonth, t.minDate);
		...
		} else t.minDate === 1 && t.minMonth === 0 && this.timeScaleArray.push({
			position: r,
			value: o,
			unit: l,
			year: s,
		...
		let c = o, h = r;
		for (let d = 0; d < a; d++) c++, h = n.determineDaysOfYear(c - 1) * i + h, this.timeScaleArray.push({
			position: h,
			value: c,
			unit: l,
			year: c,
	...
	generateMonthScale({ firstVal: t, currentMonthDate: e, currentMonth: s, currentYear: i, daysWidthOnXAxis: a, numberOfMonths: o }) {
		let r = s, n = 0;
	...
	generateDayScale({ firstVal: t, currentMonth: e, currentYear: s, hoursWidthOnXAxis: i, numberOfDays: a }) {
		const o = new q(this.w);
		let r = "day", n = t.minDate + 1, l = n;
		const c = (m, u, y) => {
			return m > o.determineDaysOfMonths(u + 1, y) && (u = u + 1, l = 1, r = "month", g = u), u;
		}, h = 24 - t.minHour, d = 0;
		...
		let x = p;
		for (let m = 0; m < a; m++) {
			l += 1, r = "day", f = c(l, f, this._getYear(s, f, d));
	...
	generateHourScale({ firstVal: t, currentDate: e, currentMonth: s, currentYear: i, minutesWidthOnXAxis: a, numberOfHours: o }) {
		const r = new q(this.w), n = 0;
		let l = "hour";
		const c = (b, w) => {
			return b > r.determineDaysOfMonths(w + 1, i) && (x = 1, w = w + 1), {
				month: w,
				date: x
			};
		}, h = (b, w) => {
			return b > r.determineDaysOfMonths(w + 1, i) && (w = w + 1), w;
	...
	generateMinuteScale({ currentMillisecond: t, currentSecond: e, currentMinute: s, currentHour: i, currentDate: a, currentMonth: o, currentYear: r, minutesWidthOnXAxis: n, secondsWidthOnXAxis: l, numberOfMinutes: c }) {
		const d = "minute", g = (60 - e - t / 1e3) * l;
		let f = s + 1;
		const x = a, m = o, u = r;
		let y = i, b = g;
		for (let w = 0; w < c; w++) f >= 60 && (f = 0, y += 1, y === 24 && (y = 0)), this.timeScaleArray.push({
			position: b,
			value: f,
			unit: d,
			hour: y,
	...
	generateSecondScale({ currentMillisecond: t, currentSecond: e, currentMinute: s, currentHour: i, currentDate: a, currentMonth: o, currentYear: r, secondsWidthOnXAxis: n, numberOfSeconds: l }) {
		const h = "second", p = (1e3 - t) / 1e3 * n;
		let g = e + 1, f = s;
		const x = a, m = o, u = r;
		let y = i, b = p;
		for (let w = 0; w < l; w++) g >= 60 && (f++, g = 0, f >= 60 && (y++, f = 0, y === 24 && (y = 0))), this.timeScaleArray.push({
			position: b,
			value: g,
			unit: h,
			hour: y,
	...
	formatDates(t) {
		...
		return t.map((i) => {
			...
			} else a = o.formatDate(n, e.config.xaxis.labels.format);
			return {
				dateString: r,
				position: i.position,
				value: a,
	...
	removeOverlappingTS(t) {
		...
		let s = !1, i;
		t.length > 0 && t[0].value && t.every((r) => r.value.length === t[0].value.length) && (s = !0, i = e.getTextRects(t[0].value).width);
		let a = 0, o = t.map((r, n) => {
...
function Q(A) {
	const t = xe[A];
	if (!t) throw new Error(`ApexCharts: chart type "${A}" is not registered. Import it via ApexCharts.use() or use the full apexcharts bundle.
...
var Je = class {
	...
	setupElements() {
		const { globals: t, config: e } = this.w, s = e.chart.type;
		t.axisCharts = [
			"line",
			"area",
			"bar",
			...
			"scatter",
			"bubble"
		].includes(s), t.isBarHorizontal = [
			"bar",
			"rangeBar",
			"boxPlot"
		].includes(s) && e.plotOptions.bar.horizontal, t.chartClass = `.apexcharts${t.chartID}`, this.w.dom.baseEl = this.el, this.w.dom.elWrap = N.createElementNS("http://www.w3.org/1999/xhtml", "div"), P.setAttrs(this.w.dom.elWrap, {
		...
		if (this.w.dom.Paper = o().addTo(this.w.dom.elWrap), this.w.dom.Paper.attr({
		...
		}), this.w.dom.Paper.node.style.background = e.theme.mode === "dark" && !e.chart.background ? "#343A3F" : e.theme.mode === "light" && !e.chart.background ? "#fff" : e.chart.background, this.setSVGDimensions(), this.w.dom.elLegendForeign = N.createElementNS(G, "foreignObject"), P.setAttrs(this.w.dom.elLegendForeign, {
			x: 0,
			y: 0,
			width: t.svgWidth,
			height: t.svgHeight
		}), this.w.dom.elLegendWrap = N.createElementNS("http://www.w3.org/1999/xhtml", "div"), this.w.dom.elLegendWrap.classList.add("apexcharts-legend"), this.w.dom.elWrap.appendChild(this.w.dom.elLegendWrap), this.w.dom.Paper.node.appendChild(this.w.dom.elLegendForeign), e.chart.accessibility.enabled) {
			...
			if (l.textContent = e.title.text || "Chart", this.w.dom.Paper.node.insertBefore(l, this.w.dom.elLegendForeign.nextSibling), e.chart.accessibility.description) {
	...
	plotChartType(t, e) {
		...
		this.w.seriesData.series.forEach((u, y) => {
			...
			r[v] ? (v === "rangeArea" ? (r[v].series.push(this.w.rangeData.seriesRangeStart[y]), r[v].seriesRangeEnd.push(this.w.rangeData.seriesRangeEnd[y])) : r[v].series.push(u), r[v].i.push(y), v === "bar" && (s.globals.columnSeries = r.bar)) : [
				"heatmap",
				"treemap",
				"pie",
				"donut",
				...
				"radialBar",
				"radar"
			].includes(v) ? l = v : console.warn(`You have specified an unrecognized series type (${v}).`), n !== v && v !== "scatter" && c++;
		}), c > 0 && (l && console.warn(`Chart or series type ${l} cannot appear with other chart or series types.`), r.bar.series.length > 0 && a.plotOptions.bar.horizontal && (c -= r.bar.series.length, r.bar = {
			series: [],
			i: []
		...
		const d = r.line.series.length > 0 || r.area.series.length > 0 || r.scatter.series.length > 0 || r.bubble.series.length > 0 || r.rangeArea.series.length > 0 || !o.comboCharts && [
			"line",
			"area",
			"scatter",
			"bubble",
			"rangeArea"
		].includes(a.chart.type) ? new (Q("line"))(i.w, i, e) : null, g = r.candlestick.series.length > 0 || r.boxPlot.series.length > 0 || !o.comboCharts && ["candlestick", "boxPlot"].includes(a.chart.type) ? new (Q("candlestick"))(i.w, i, e) : null;
		i.pie = !o.comboCharts && [
			"pie",
			"donut",
			"polarArea"
		].includes(a.chart.type) ? new (Q("pie"))(i.w, i) : null;
		i.rangeBar = r.rangeBar.series.length > 0 || !o.comboCharts && a.chart.type === "rangeBar" ? new (Q("rangeBar"))(i.w, i, e) : null;
		let m = [];
		if (o.comboCharts) {
			const u = new V(this.w);
			if (r.area.series.length > 0 && m.push(...u.drawSeriesByGroup(r.area, o.areaGroups, "area", d)), r.bar.series.length > 0) if (a.chart.stacked) {
				const y = new (Q("barStacked"))(i.w, i, e);
				m.push(y.draw(r.bar.series, r.bar.i));
			} else i.bar = new (Q("bar"))(i.w, i, e), m.push(i.bar.draw(r.bar.series, r.bar.i));
			if (r.rangeArea.series.length > 0 && m.push(d.draw(r.rangeArea.series, "rangeArea", r.rangeArea.i, r.rangeArea.seriesRangeEnd)), r.line.series.length > 0 && m.push(...u.drawSeriesByGroup(r.line, o.lineGroups, "line", d)), r.candlestick.series.length > 0 && m.push(g.draw(r.candlestick.series, "candlestick", r.candlestick.i)), r.boxPlot.series.length > 0 && m.push(g.draw(r.boxPlot.series, "boxPlot", r.boxPlot.i)), r.rangeBar.series.length > 0 && m.push(i.rangeBar.draw(r.rangeBar.series, r.rangeBar.i)), r.scatter.series.length > 0) {
				const y = new (Q("line"))(i.w, i, e, !0);
				m.push(y.draw(r.scatter.series, "scatter", r.scatter.i));
			}
			if (r.bubble.series.length > 0) {
				const y = new (Q("line"))(i.w, i, e, !0);
				m.push(y.draw(r.bubble.series, "bubble", r.bubble.i));
			}
		} else {
			...
			switch (u) {
				...
				case "rangeBar":
					...
					break;
				case "rangeArea":
					m = d.draw(this.w.rangeData.seriesRangeStart, "rangeArea", void 0, this.w.rangeData.seriesRangeEnd);
					break;
				case "heatmap":
				...
				case "radar":
					...
					break;
				default: m = d.draw(this.w.seriesData.series);
			}
		}
		return m;
	}
	setSVGDimensions() {
		...
		if (t.svgHeight !== "auto" && t.svgHeight !== "") if (a === "%") t.svgHeight = L.getDimensions(this.el.parentNode)[1] * parseInt(e.chart.height, 10) / 100;
	...
	shiftGraphPosition() {
		...
		P.setAttrs(this.w.dom.elGraphical.node, { transform: `translate(${s}, ${e})` });
	}
	resizeNonAxisCharts() {
		var t, e;
		const { w: s } = this;
		...
		const o = s.dom.baseEl.querySelector(".apexcharts-radialbar, .apexcharts-pie");
		...
		const n = Math.ceil(r + this.w.layout.translateY + i + a);
	...
	updateSourceChart(t) {
		this.ctx.w.interact.selection = void 0, this.ctx.updateHelpers._updateOptions({ chart: { selection: { xaxis: {
	...
	setupBrushHandler() {
		...
		if (e.config.chart.brush.enabled && typeof e.config.chart.events.selection != "function") {
			...
			s.forEach((i) => {
				...
				s.forEach((o) => {
					t.constructor.getChartByID(o).ctx.updateHelpers._updateOptions({ xaxis: {
						min: a.xaxis.min,
						max: a.xaxis.max
					} }, !1, !1, !1, !1);
				});
	...
	getAccessibleChartLabel() {
		...
		let s = "";
		if (e.chart.accessibility && e.chart.accessibility.description) s = e.chart.accessibility.description;
		else if (e.title.text) {
			const i = e.chart.type;
			s = `${e.title.text}. ${i} chart`, e.subtitle.text && (s += `. ${e.subtitle.text}`);
		} else s = `${e.chart.type} chart with ${t.seriesData.series.length || (e.series ? e.series.length : 0)} data series`;
		return s;
	}
};
var Wt = class {
	constructor(t, { resetGlobals: e = () => {}, isMultipleY: s = () => {} } = {}) {
	...
	handleFormatXY(t, e) {
		...
		for (let l = 0; l < n.length; l++) {
			...
			if (d || g) if (d || s.xaxis.convertedCatToNumeric) {
			...
			} else s.xaxis.type === "datetime" ? this.twoDSeriesX.push(a.parseDate(h.toString())) : (this.w.axisFlags.dataFormatXNumeric = !0, this.w.axisFlags.isXNumeric = !0, this.twoDSeriesX.push(parseFloat(h)));
			else p ? (this.fallbackToCategory = !0, this.twoDSeriesX.push(h)) : (this.w.axisFlags.isXNumeric = !0, this.w.axisFlags.dataFormatXNumeric = !0, this.twoDSeriesX.push(h));
		}
	}
	handleRangeData(t, e) {
		let s = {};
		return this.isFormat2DArray() ? s = this.handleRangeDataFormat("array", t, e) : this.isFormatXY() && (s = this.handleRangeDataFormat("xy", t, e)), this.w.rangeData.seriesRangeStart[e] = s.start === void 0 ? [] : s.start, this.w.rangeData.seriesRangeEnd[e] = s.end === void 0 ? [] : s.end, this.w.rangeData.seriesRange[e] = s.rangeUniques, this.w.rangeData.seriesRange.forEach((i) => {
			i && i.forEach((a) => {
				...
				if (!(r <= 1)) for (let n = 0; n < r; n++) {
					const l = o[n], c = l.y1, h = l.y2;
					for (let d = n + 1; d < r; d++) {
						const p = o[d], g = p.y1;
						c <= p.y2 && g <= h && (a.overlaps.indexOf(l.rangeName) < 0 && a.overlaps.push(l.rangeName), a.overlaps.indexOf(p.rangeName) < 0 && a.overlaps.push(p.rangeName));
					}
				}
			});
		}), s;
	...
	handleRangeDataFormat(t, e, s) {
		...
		}), t === "array") for (let n = 0; n < e[s].data.length; n++) Array.isArray(e[s].data[n]) ? (i.push(e[s].data[n][1][0]), a.push(e[s].data[n][1][1])) : (i.push(e[s].data[n]), a.push(e[s].data[n]));
		else if (t === "xy") for (let n = 0; n < e[s].data.length; n++) {
	...
	parseDataAxisCharts(t) {
		...
		const n = () => {
			for (let l = 0; l < a.length; l++) if (typeof a[l] == "string") if (i.isValidDate(a[l])) this.twoDSeriesX.push(i.parseDate(a[l]));
			else throw new Error("You have provided invalid Date format. Please provide a valid JavaScript Date");
			else this.twoDSeriesX.push(a[l]);
		};
	...
	parseDataNonAxisCharts(t) {
		const e = this.w.config, s = Array.isArray(t) && t.every((o) => typeof o == "number") && e.labels.length > 0, i = Array.isArray(t) && t.some((o) => o && typeof o == "object" && o.data || o && typeof o == "object" && o.parsing);
		if (s && i && console.warn("ApexCharts: Both old format (numeric series + labels) and new format (series objects with data/parsing) detected. Using old format for backward compatibility."), s) {
			this.w.seriesData.series = t.slice(), this.w.seriesData.seriesNames = e.labels.slice();
			for (let o = 0; o < this.w.seriesData.series.length; o++) this.w.seriesData.seriesNames[o] === void 0 && this.w.seriesData.seriesNames.push("series-" + (o + 1));
			return this.w;
		}
		if (Array.isArray(t) && t.every((o) => typeof o == "number")) {
			this.w.seriesData.series = t.slice(), this.w.seriesData.seriesNames = [];
			for (let o = 0; o < this.w.seriesData.series.length; o++) this.w.seriesData.seriesNames.push(e.labels[o] || `series-${o + 1}`);
			return this.w;
		}
		...
		this.w.seriesData.series = a.values, this.w.seriesData.seriesNames = a.labels, e.chart.type === "radialBar" && (this.w.seriesData.series = this.w.seriesData.series.map((o) => {
			...
			return r > 100 && console.warn(`ApexCharts: RadialBar value ${r} > 100, consider using percentage values (0-100)`), r;
		}));
		for (let o = 0; o < this.w.seriesData.series.length; o++) this.w.seriesData.seriesNames[o] === void 0 && this.w.seriesData.seriesNames.push("series-" + (o + 1));
		return this.w;
	}
	...
	*/
	...
	extractPieDataFromSeries(t) {
		const e = [], s = [];
		if (!Array.isArray(t)) return console.warn("ApexCharts: Expected array for series data"), {
			values: [],
			labels: []
		};
		if (t.length === 0) return console.warn("ApexCharts: Empty series array"), {
			values: [],
			labels: []
		};
		const i = t[0];
		if (typeof i == "object" && i !== null && i.data) this.extractPieDataFromSeriesObjects(t, e, s);
		else return console.warn("ApexCharts: Unsupported series format for pie/donut/radialBar. Expected series objects with data property."), {
			values: [],
			labels: []
		};
		return {
	...
	extractPieDataFromSeriesObjects(t, e, s) {
		t.forEach((i, a) => {
			...
			i.data.forEach((o) => {
				typeof o == "object" && o !== null ? o.x !== void 0 && o.y !== void 0 ? (s.push(String(o.x)), e.push(L.parseNumber(o.y))) : console.warn("ApexCharts: Invalid data point format for pie chart. Expected {x, y} format:", o) : console.warn("ApexCharts: Expected object data point, got:", typeof o);
			});
		});
	}
	...
	*/
	handleExternalLabelsData(t) {
		...
		}), this.w.labelData.labels = Array.from(new Set(this.w.labelData.labels.map(JSON.stringify)), JSON.parse)), e.xaxis.convertedCatToNumeric && (new bt(e).convertCatToNumericXaxis(e, this.w.seriesData.seriesX[0]), this._generateExternalLabels(t))) : this._generateExternalLabels(t);
	}
	_generateExternalLabels(t) {
		...
		if (e.axisCharts) {
			if (this.w.seriesData.series.length > 0) if (this.isFormatXY()) {
				const a = s.series.map((r) => r.data.filter((n, l, c) => c.findIndex((h) => h.x === n.x) === l)), o = a.reduce((r, n, l, c) => c[r].length > n.length ? r : l, 0);
				for (let r = 0; r < a[o].length; r++) i.push(r + 1);
			} else for (let a = 0; a < this.w.seriesData.series[e.maxValsInArrayIndex].length; a++) i.push(a + 1);
	...
	parseRawDataIfNeeded(t) {
		...
		if (this.w.axisFlags.dataWasParsed || !i && !t.some((o) => o.parsing)) return t;
		const a = t.map((o, r) => {
			var n, l, c;
			...
			if (!h.x && !h.y) return o;
			const d = o.data[0];
			if (typeof d == "object" && d !== null && (Object.prototype.hasOwnProperty.call(d, "x") || Object.prototype.hasOwnProperty.call(d, "y")) || Array.isArray(d)) return o;
			if (!h.x || !h.y || Array.isArray(h.y) && h.y.length === 0) return console.warn(`ApexCharts: Series ${r} has parsing config but missing x or y field specification`), o;
			const p = o.data.map((g, f) => {
				if (typeof g != "object" || g === null) return console.warn(`ApexCharts: Series ${r}, data point ${f} is not an object, skipping parsing`), g;
				const x = this.getNestedValue(g, h.x);
				let m, u;
				if (Array.isArray(h.y)) {
					const b = h.y.map((w) => this.getNestedValue(g, w));
					this.w.config.chart.type === "bubble" && b.length === 2 ? m = b[0] : m = b;
				} else m = this.getNestedValue(g, h.y);
				h.z && (u = this.getNestedValue(g, h.z)), x === void 0 && console.warn(`ApexCharts: Series ${r}, data point ${f} missing field '${h.x}'`), m === void 0 && console.warn(`ApexCharts: Series ${r}, data point ${f} missing field '${h.y}'`);
				const y = {
					x,
					y: m
				};
	...
	*/
	...
	parseData(t) {
		...
		if (t = this.parseRawDataIfNeeded(t), s.series = t, i.initialSeries = L.clone(t), this.excludeCollapsedSeriesInYAxis(), this.fallbackToCategory = !1, this.resetGlobals(), this.isMultipleY(), i.axisCharts ? (this.parseDataAxisCharts(t), this.coreUtils.getLargestSeries()) : this.parseDataNonAxisCharts(t), s.chart.stacked) {
			...
			this.w.seriesData.series = o.setNullSeriesToZeroValues(this.w.seriesData.series);
		}
		...
		for (let o = 0; o < a.length; o++) if (Array.isArray(a[o])) {
...
var Qe = class {
	...
	*/
	...
	toggleDataPointSelection(t, e) {
		...
		if (s.globals.axisCharts ? i = s.dom.Paper.findOne(`${a} path[j='${e}'], ${a} circle[j='${e}'], ${a} rect[j='${e}']`) : typeof e > "u" && (i = s.dom.Paper.findOne(`${a} path[j='${t}']`), (s.config.chart.type === "pie" || s.config.chart.type === "polarArea" || s.config.chart.type === "donut") && this.ctx.pie.pieClicked(t)), i) new P(this.w).pathMouseDown(i, null);
		else return console.warn("toggleDataPointSelection: Element not found"), null;
...
var be = class {
	...
	*/
	getNearestValues({ hoverArea: t, elGrid: e, clientX: s, clientY: i }) {
		...
		if (a.axisFlags.isXNumeric) {
			...
			u = this.closestInMultiArray(C, D, y, b), m = u.index, f = u.j, m !== null && a.globals.hasNullValues && (y = a.globals.seriesXvalues[m], u = this.closestInArray(C, y), f = u.j);
		}
	...
	getFirstActiveXArray(t) {
		...
		for (let a = 0; a < i.length; a++) if (i[a] !== -1 && e.globals.collapsedSeriesIndices.indexOf(a) === -1 && e.globals.ancillaryCollapsedSeriesIndices.indexOf(a) === -1) {
			s = i[a];
			break;
		}
		return s;
	...
	*/
	isXoverlap(t) {
		...
		if (i.length > 0) for (let a = 0; a < i.length - 1; a++) typeof i[a][t] < "u" && typeof i[a + 1][t] < "u" && i[a][t] !== i[a + 1][t] && s.push("unEqual");
		return s.length === 0;
	}
	isInitialSeriesSameLen() {
		var t, e, s;
		let i = !0;
		const a = ((t = this.w.globals.initialSeries) == null ? void 0 : t.filter((o, r) => {
			var n;
			return !((n = this.w.globals.collapsedSeriesIndices) != null && n.includes(r));
		})) || [];
		for (let o = 0; o < a.length - 1; o++) {
			if (!((e = a[o]) != null && e.data) || !((s = a[o + 1]) != null && s.data)) return !0;
	...
	getElMarkers(t) {
		return typeof t == "number" ? this.w.dom.baseEl.querySelectorAll(`.apexcharts-series[data\\:realIndex='${t}'] .apexcharts-series-markers-wrap > *`) : this.w.dom.baseEl.querySelectorAll(".apexcharts-series-markers-wrap > *");
	}
	getAllMarkers(t = !1) {
		let e = this.w.dom.baseEl.querySelectorAll(".apexcharts-series-markers-wrap");
		e = [...e], t && (e = e.filter((i) => {
	...
	toggleAllTooltipSeriesGroups(t) {
		...
		s.allTooltipSeriesGroups.length === 0 && (s.allTooltipSeriesGroups = e.dom.baseEl.querySelectorAll(".apexcharts-tooltip-series-group"));
		const i = s.allTooltipSeriesGroups;
		for (let a = 0; a < i.length; a++) t === "enable" ? (i[a].classList.add("apexcharts-active"), i[a].style.display = e.config.tooltip.items.display) : (i[a].classList.remove("apexcharts-active"), i[a].style.display = "none");
	}
};
var ts = class {
	constructor(t) {
	...
	drawSeriesTexts({ shared: t = !0, ttItems: e, i: s = 0, j: i = null, y1: a, y2: o, e: r }) {
		const n = this.w;
		n.config.tooltip.custom !== void 0 ? this.handleCustomTooltip({
			i: s,
			j: i,
			y1: a,
			y2: o,
	...
	printLabels({ i: t, j: e, values: s, ttItems: i, shared: a, e: o }) {
		...
		for (let m = 0, u = n.seriesData.series.length - 1; m < n.seriesData.series.length; m++, u--) {
			...
			if (n.globals.axisCharts) {
				...
				if (a) y = this.getFormatters(b), f = this.getSeriesName({
					fn: y.yLbTitleFormatter,
					index: b,
					seriesIndex: t,
					j: e
	...
	DOMHandling({ t, j: e, ttItems: s, values: i, seriesName: a, shared: o, pColor: r }) {
		...
		let f = null;
		f = s[t].children, n.config.tooltip.fillSeriesColor && (s[t].style.backgroundColor = r, f[0].style.display = "none"), l.showTooltipTitle && (l.tooltipTitle === null && (l.tooltipTitle = n.dom.baseEl.querySelector(".apexcharts-tooltip-title")), l.tooltipTitle.innerHTML = d), l.isXAxisTooltipEnabled && (l.xaxisTooltipText.innerHTML = p !== "" ? p : d);
		const x = s[t].querySelector(".apexcharts-tooltip-text-y-label");
		x && (x.innerHTML = a || "");
		...
		const u = s[t].querySelector(".apexcharts-tooltip-text-goals-label"), y = s[t].querySelector(".apexcharts-tooltip-text-goals-value");
		...
		} else u.innerHTML = "", y.innerHTML = "";
		if (g !== null) {
			const b = s[t].querySelector(".apexcharts-tooltip-text-z-label");
			b.innerHTML = n.config.tooltip.z.title;
			const w = s[t].querySelector(".apexcharts-tooltip-text-z-value");
			w.innerHTML = typeof g < "u" ? g : "";
		}
		if (o && f[0]) {
			if (n.config.tooltip.hideEmptySeries) {
				const b = s[t].querySelector(".apexcharts-tooltip-marker"), w = s[t].querySelector(".apexcharts-tooltip-text");
		...
		} else Array.isArray(l.tConfig.enabledOnSeries) && l.tConfig.enabledOnSeries.indexOf(t) === -1 && (f[0].parentNode.style.display = "none");
	}
	toggleActiveInactiveSeries(t, e) {
		...
		if (t) this.tooltipUtil.toggleAllTooltipSeriesGroups("enable");
			...
			const i = s.dom.baseEl.querySelector(`.apexcharts-tooltip-series-group-${e}`);
	...
	getValuesToPrint({ i: t, j: e }) {
		...
		if (e === null) n = s.seriesData.series[t];
		else if (s.axisFlags.isXNumeric && s.config.chart.type !== "treemap") {
			if (a = i[t][e], i[t].length === 0) a = i[this.tooltipUtil.getFirstActiveXArray(i)][e];
		} else new Wt(this.w).isFormatXY() ? a = typeof s.config.series[t].data[e] < "u" ? s.config.series[t].data[e].x : "" : a = typeof s.labelData.labels[e] < "u" ? s.labelData.labels[e] : "";
		const h = a;
		return s.axisFlags.isXNumeric && s.config.xaxis.type === "datetime" ? a = new vt(this.w).xLabelFormat(s.formatters.ttKeyFormatter, h, h, {
			i: void 0,
			dateFormatter: new q(this.w).formatDate,
			w: this.w
		}) : s.globals.isBarHorizontal ? a = s.formatters.yLabelFormatters[0](h, l) : a = s.formatters.xLabelFormatter(h, l), s.config.tooltip.x.formatter !== void 0 && (a = s.formatters.ttKeyFormatter(h, l)), s.seriesData.seriesZ.length > 0 && s.seriesData.seriesZ[t].length > 0 && (r = c(s.seriesData.seriesZ[t][e], s)), typeof s.config.xaxis.tooltip.formatter == "function" ? o = s.formatters.xaxisTooltipFormatter(h, l) : o = a, {
...
var ye = class {
	...
	*/
	...
	moveYAxisTooltip(t) {
		const e = this.w, s = this.ttCtx;
		s.yaxisTTEls === null && (s.yaxisTTEls = e.dom.baseEl.querySelectorAll(".apexcharts-yaxistooltip"));
		const i = parseInt(s.ycrosshairsHidden.getAttribute("y1"), 10);
	...
	*/
	moveTooltip(t, e, s = null) {
		...
		} else i.globals.isBarHorizontal || r.ttHeight / 2 + c > i.layout.gridHeight && (c = i.layout.gridHeight - r.ttHeight + i.layout.translateY);
	...
	moveMarkers(t, e) {
		...
		if (s.globals.markers.size[t] > 0) {
			const a = s.dom.baseEl.querySelectorAll(` .apexcharts-series[data\\:realIndex='${t}'] .apexcharts-marker`);
			for (let o = 0; o < a.length; o++) parseInt(a[o].getAttribute("rel"), 10) === e && (i.marker.resetPointsSize(), i.marker.enlargeCurrentPoint(e, a[o]));
	...
	moveDynamicPointOnHover(t, e) {
		...
		if (g && (g === "column" || g === "candlestick" || g === "boxPlot")) return;
		l = (i = (s = d[e]) == null ? void 0 : s[t]) == null ? void 0 : i[0], c = ((o = (a = d[e]) == null ? void 0 : a[t]) == null ? void 0 : o[1]) || 0;
		const f = r.dom.baseEl.querySelector(`.apexcharts-series[data\\:realIndex='${e}'] .apexcharts-series-markers path`);
	...
	moveDynamicPointsOnHover(t) {
		...
		if ((e = n[r]) != null && e[t] && (a = n[r][t][0], o = n[r][t][1]), isNaN(a)) return;
		const d = s.tooltipUtil.getAllMarkers();
		if (d.length) for (let p = 0; p < i.seriesData.series.length; p++) {
			const g = n[p];
	...
	moveStickyTooltipOverBars(t, e) {
		...
		let r = s.dom.baseEl.querySelector(`.apexcharts-bar-series .apexcharts-series[rel='${o}'] path[j='${t}'], .apexcharts-candlestick-series .apexcharts-series[rel='${o}'] path[j='${t}'], .apexcharts-boxPlot-series .apexcharts-series[rel='${o}'] path[j='${t}'], .apexcharts-rangebar-series .apexcharts-series[rel='${o}'] path[j='${t}']`);
		!r && typeof e == "number" && (r = s.dom.baseEl.querySelector(`.apexcharts-bar-series .apexcharts-series[data\\:realIndex='${e}'] path[j='${t}'],
		...
		s.axisFlags.isXNumeric ? (r && !p && (n = n - (a % 2 !== 0 ? c / 2 : 0)), r && p && (n = n - c / 2)) : s.globals.isBarHorizontal || (n = i.xAxisTicksPositions[t - 1] + i.dataPointsDividedWidth / 2, isNaN(n) && (n = i.xAxisTicksPositions[t] - i.dataPointsDividedWidth / 2)), s.globals.isBarHorizontal ? l = l - i.tooltipRect.ttHeight : s.config.tooltip.followCursor ? l = i.e.clientY - d.top - i.tooltipRect.ttHeight / 2 : l + i.tooltipRect.ttHeight + 15 > s.layout.gridHeight && (l = s.layout.gridHeight), s.globals.isBarHorizontal || this.moveXCrosshairs(n), i.fixedTooltip || this.moveTooltip(n, l || s.layout.gridHeight);
	}
};
var es = class {
	constructor(t) {
	...
	drawDynamicPoints() {
		...
		for (let a = 0; a < i.length; a++) {
			const o = i[a].querySelector(".apexcharts-series-markers-wrap");
			if (o !== null) {
	...
	enlargePoints(t) {
		const e = this.w, s = this, i = this.ttCtx, a = t, o = e.dom.baseEl.querySelectorAll(".apexcharts-series:not(.apexcharts-series-collapsed) .apexcharts-marker");
	...
	resetPointsSize() {
		const e = this.w.dom.baseEl.querySelectorAll(".apexcharts-series:not(.apexcharts-series-collapsed) .apexcharts-marker");
		for (let s = 0; s < e.length; s++) {
...
var ss = class {
	...
	*/
	handleMarkerTooltip({ e: t, opt: e, x: s, y: i }) {
		...
		if (t.target.classList.contains("apexcharts-marker")) {
			...
			if (n = parseInt(e.paths.getAttribute("rel"), 10), r = parseInt(e.paths.parentNode.parentNode.parentNode.getAttribute("rel"), 10) - 1, o.intersect) {
	...
	*/
	handleBarTooltip({ e: t, opt: e }) {
		...
		if (h.j === null && h.barHeight === 0 && h.barWidth === 0) return;
		l = h.i;
		const d = h.j;
...
var is = class {
	...
	**/
	drawXaxisTooltip() {
		...
		e.isXAxisTooltipEnabled && t.dom.baseEl.querySelector(".apexcharts-xaxistooltip") === null && (e.xaxisTooltip = N.createElementNS("http://www.w3.org/1999/xhtml", "div"), e.xaxisTooltip.setAttribute("class", i + " apexcharts-theme-" + t.config.tooltip.theme), a.appendChild(e.xaxisTooltip), e.xaxisTooltipText = N.createElementNS("http://www.w3.org/1999/xhtml", "div"), e.xaxisTooltipText.classList.add("apexcharts-xaxistooltip-text"), e.xaxisTooltipText.style.fontFamily = t.config.xaxis.tooltip.style.fontFamily || t.config.chart.fontFamily, e.xaxisTooltipText.style.fontSize = t.config.xaxis.tooltip.style.fontSize, e.xaxisTooltip.appendChild(e.xaxisTooltipText));
	}
	/**
	* This method adds the secondary tooltip which appears below x axis
	* @memberof Tooltip
	**/
	drawYaxisTooltip() {
		...
		for (let s = 0; s < t.config.yaxis.length; s++) {
			...
			t.dom.baseEl.querySelector(`.apexcharts-yaxistooltip apexcharts-yaxistooltip-${s}`) === null && (e.yaxisTooltip = N.createElementNS("http://www.w3.org/1999/xhtml", "div"), e.yaxisTooltip.setAttribute("class", a + " apexcharts-theme-" + t.config.tooltip.theme), o.appendChild(e.yaxisTooltip), s === 0 && (e.yaxisTooltipText = []), e.yaxisTooltipText[s] = N.createElementNS("http://www.w3.org/1999/xhtml", "div"), e.yaxisTooltipText[s].classList.add("apexcharts-yaxistooltip-text"), e.yaxisTooltip.appendChild(e.yaxisTooltipText[s]));
		}
	}
	/**
	* @memberof Tooltip
	**/
	setXCrosshairWidth() {
		...
		if (e.xcrosshairsWidth = parseInt(t.config.xaxis.crosshairs.width, 10), t.globals.comboCharts) {
			...
			if (i !== null && t.config.xaxis.crosshairs.width === "barWidth") e.xcrosshairsWidth = parseFloat(i.getAttribute("barWidth"));
			...
			const i = t.dom.baseEl.querySelector(".apexcharts-bar-area");
			if (i !== null) e.xcrosshairsWidth = parseFloat(i.getAttribute("barWidth"));
	...
	handleYCrosshair() {
		const t = this.w, e = this.ttCtx;
		e.ycrosshairs = t.dom.baseEl.querySelector(".apexcharts-ycrosshairs"), e.ycrosshairsHidden = t.dom.baseEl.querySelector(".apexcharts-ycrosshairs-hidden");
	}
...
var as = class {
	...
	getCachedDimensions() {
		...
		if (t.globals.dimensionCache.tooltip) {
			...
			if (Date.now() - s.lastUpdate < 1e3) return {
				ttWidth: s.width,
				ttHeight: s.height
			};
		}
	...
	drawTooltip(t) {
		...
		const s = N.createElementNS("http://www.w3.org/1999/xhtml", "div");
		...
		if ((e.globals.comboCharts || this.tConfig.intersect || e.config.chart.type === "rangeBar") && !this.tConfig.shared && (this.showOnIntersect = !0), (e.config.markers.size === 0 || e.globals.markers.largestSize === 0) && this.marker.drawDynamicPoints(this), e.globals.collapsedSeries.length === e.seriesData.series.length) return;
		this.dataPointsDividedHeight = e.layout.gridHeight / e.globals.dataPoints, this.dataPointsDividedWidth = e.layout.gridWidth / e.globals.dataPoints, this.showTooltipTitle && (this.tooltipTitle = N.createElementNS("http://www.w3.org/1999/xhtml", "div"), this.tooltipTitle.classList.add("apexcharts-tooltip-title"), this.tooltipTitle.style.fontFamily = this.tConfig.style.fontFamily || e.config.chart.fontFamily, this.tooltipTitle.style.fontSize = this.tConfig.style.fontSize, s.appendChild(this.tooltipTitle));
		let i = e.seriesData.series.length;
		(e.globals.xyCharts || e.globals.comboCharts) && this.tConfig.shared && (this.showOnIntersect ? i = 1 : i = e.seriesData.series.length), this.legendLabels = e.dom.baseEl.querySelectorAll(".apexcharts-legend-text"), this.ttItems = this.createTTElements(i), this.addSVGEvents(), this.setupDimensionCache();
	}
	createTTElements(t) {
		const e = this.w, s = [], i = this.getElTooltip();
		for (let a = 0; a < t; a++) {
			const o = N.createElementNS("http://www.w3.org/1999/xhtml", "div");
			o.classList.add("apexcharts-tooltip-series-group", `apexcharts-tooltip-series-group-${a}`), o.style.order = e.config.tooltip.inverseOrder ? t - a : a + 1;
			const r = N.createElementNS("http://www.w3.org/1999/xhtml", "span");
			...
			const c = N.createElementNS("http://www.w3.org/1999/xhtml", "div");
			c.classList.add("apexcharts-tooltip-text"), c.style.fontFamily = this.tConfig.style.fontFamily || e.config.chart.fontFamily, c.style.fontSize = this.tConfig.style.fontSize, [
				"y",
				"goals",
				"z"
			].forEach((h) => {
				const d = N.createElementNS("http://www.w3.org/1999/xhtml", "div");
				d.classList.add(`apexcharts-tooltip-${h}-group`);
				const p = N.createElementNS("http://www.w3.org/1999/xhtml", "span");
				p.classList.add(`apexcharts-tooltip-text-${h}-label`), d.appendChild(p);
				const g = N.createElementNS("http://www.w3.org/1999/xhtml", "span");
	...
	addSVGEvents() {
		...
		let h;
		if (t.globals.axisCharts && (a ? h = t.dom.baseEl.querySelectorAll(".apexcharts-series[data\\:longestSeries='true'] .apexcharts-marker") : i ? h = t.dom.baseEl.querySelectorAll(".apexcharts-series .apexcharts-bar-area, .apexcharts-series .apexcharts-candlestick-area, .apexcharts-series .apexcharts-boxPlot-area, .apexcharts-series .apexcharts-rangebar-area") : (e === "heatmap" || e === "treemap") && (h = t.dom.baseEl.querySelectorAll(".apexcharts-series .apexcharts-heatmap, .apexcharts-series .apexcharts-treemap")), h && h.length)) for (let p = 0; p < h.length; p++) n.push(h[p].getAttribute("cy")), l.push(h[p].getAttribute("cx"));
		if (t.globals.xyCharts && !this.showOnIntersect || t.globals.comboCharts && !this.showOnIntersect || i && this.tooltipUtil.hasBars() && this.tConfig.shared) this.addPathsEventListeners([o], c);
		...
		if (this.showOnIntersect) {
			const p = t.dom.baseEl.querySelectorAll(".apexcharts-line-series .apexcharts-marker, .apexcharts-area-series .apexcharts-marker");
	...
	addDatapointEventsListeners(t) {
		const s = this.w.dom.baseEl.querySelectorAll(".apexcharts-series-markers .apexcharts-marker, .apexcharts-bar-area, .apexcharts-candlestick-area, .apexcharts-boxPlot-area, .apexcharts-rangebar-area");
	...
	addPathsEventListeners(t, e) {
		...
		for (let i = 0; i < t.length; i++) {
			...
			[
				...
				"mouseout",
				"touchend"
			].map((r) => t[i].addEventListener(r, s.onSeriesHover.bind(s, a), {
				capture: !1,
				passive: !0
			}));
		}
	...
	seriesHover(t, e) {
		...
		let s = [];
		const i = this.w;
		i.config.chart.group && (s = this.ctx.getGroupedCharts()), !(i.globals.axisCharts && (i.globals.minX === -Infinity && i.globals.maxX === Infinity || i.globals.dataPoints === 0)) && (s.length ? s.forEach((a) => {
	...
	axisChartsTooltips({ e: t, opt: e }) {
		...
		if (t.type === "mousemove" || t.type === "touchmove" || t.type === "mouseup") {
			if (s.globals.collapsedSeries.length + s.globals.ancillaryCollapsedSeries.length === s.seriesData.series.length) return;
			c !== null && c.classList.add("apexcharts-active");
			const p = this.yaxisTooltips.filter((g) => g === !0);
			if (this.ycrosshairs !== null && p.length && this.ycrosshairs.classList.add("apexcharts-active"), d && !this.showOnIntersect || h.length > 1) this.handleStickyTooltip(t, r, n, e);
			...
			if (this.yaxisTooltips.length) for (let g = 0; g < s.config.yaxis.length; g++) this.axesTooltip.drawYaxisTooltipText(g, n, this.xyRatios);
		...
		} else (t.type === "mouseout" || t.type === "touchend") && this.handleMouseOut(e);
	}
	nonAxisChartsTooltips({ e: t, opt: e, tooltipRect: s }) {
		...
		if (t.type === "mousemove" || t.type === "touchmove") {
			...
			} else n = i.interact.clientX - r.left - s.ttWidth / 2, l = i.interact.clientY - r.top - s.ttHeight - 10;
			if (o.style.left = n + "px", o.style.top = l + "px", i.config.legend.tooltipHoverFormatter) {
				const h = i.config.legend.tooltipHoverFormatter, d = a - 1, g = h(this.legendLabels[d].getAttribute("data:default-text"), {
					seriesIndex: d,
					dataPointIndex: d,
					w: i
				});
		...
		} else (t.type === "mouseout" || t.type === "touchend") && (o.classList.remove("apexcharts-active"), i.dom.baseEl.classList.remove("apexcharts-tooltip-active"), i.config.legend.tooltipHoverFormatter && this.legendLabels.forEach((n) => {
	...
	handleStickyTooltip(t, e, s, i) {
		...
		if (n !== null) this.handleStickyCapturedSeries(t, n, i, r);
		else if (this.tooltipUtil.isXoverlap(r) || a.globals.isBarHorizontal) {
			const c = a.seriesData.series.findIndex((h, d) => !a.globals.collapsedSeriesIndices.includes(d));
	...
	handleStickyCapturedSeries(t, e, s, i) {
		...
		if (typeof a.seriesData.series[e][i] < "u") this.tConfig.shared && this.tooltipUtil.isXoverlap(i) && this.tooltipUtil.isInitialSeriesSameLen() ? this.create(t, this, e, i, s.ttItems) : this.create(t, this, e, i, s.ttItems, !1);
		else if (this.tooltipUtil.isXoverlap(i)) {
			const o = a.seriesData.series.findIndex((r, n) => !a.globals.collapsedSeriesIndices.includes(n));
			this.create(t, this, o, i, s.ttItems);
		}
	}
	deactivateHoverFilter() {
		const t = this.w, e = new P(this.w, this.ctx), s = t.dom.Paper.find(".apexcharts-bar-area");
		for (let i = 0; i < s.length; i++) e.pathMouseLeave(s[i]);
	}
	handleMouseOut(t) {
		...
		if (e.dom.baseEl.classList.remove("apexcharts-tooltip-active"), t.tooltipEl.classList.remove("apexcharts-active"), e.config.chart.accessibility.enabled && e.config.chart.accessibility.announcements.enabled && t.tooltipEl.setAttribute("aria-hidden", "true"), this.deactivateHoverFilter(), e.config.chart.type !== "bubble" && this.marker.resetPointsSize(), s !== null && s.classList.remove("apexcharts-active"), this.ycrosshairs !== null && this.ycrosshairs.classList.remove("apexcharts-active"), this.isXAxisTooltipEnabled && this.xaxisTooltip.classList.remove("apexcharts-active"), this.yaxisTooltips.length) {
			this.yaxisTTEls === null && (this.yaxisTTEls = e.dom.baseEl.querySelectorAll(".apexcharts-yaxistooltip"));
	...
	create(t, e, s, i, a, o = null) {
		...
		if (C.config.legend.tooltipHoverFormatter) {
			...
			for (let X = 0; X < E.length; X++) {
				const R = E[X], Y = parseInt(R.getAttribute("i"), 10), z = decodeURIComponent(R.getAttribute("data:default-text")), H = M(z, {
					seriesIndex: o ? Y : s,
					dataPointIndex: i,
					w: C
				});
				if (o) R.innerHTML = C.globals.collapsedSeriesIndices.indexOf(Y) < 0 ? H : z;
				else if (R.innerHTML = Y === s ? H : z, s === Y) break;
			}
		}
		const T = I(I({
			ttItems: a,
			i: s,
			j: i
		}, typeof ((c = (l = (n = (r = C.rangeData.seriesRange) == null ? void 0 : r[s]) == null ? void 0 : n[i]) == null ? void 0 : l.y[0]) == null ? void 0 : c.y1) < "u" && { y1: (g = (p = (d = (h = C.rangeData.seriesRange) == null ? void 0 : h[s]) == null ? void 0 : d[i]) == null ? void 0 : p.y[0]) == null ? void 0 : g.y1 }), typeof ((u = (m = (x = (f = C.rangeData.seriesRange) == null ? void 0 : f[s]) == null ? void 0 : x[i]) == null ? void 0 : m.y[0]) == null ? void 0 : u.y2) < "u" && { y2: (v = (w = (b = (y = C.rangeData.seriesRange) == null ? void 0 : y[s]) == null ? void 0 : b[i]) == null ? void 0 : w.y[0]) == null ? void 0 : v.y2 });
		if (o) {
			if (D.tooltipLabels.drawSeriesTexts(O(I({}, T), { shared: this.showOnIntersect ? !1 : this.tConfig.shared })), S) F();
				...
				for (let R = 0; R < E.length; R++) M.pathMouseEnter(E[R]);
			}
		} else D.tooltipLabels.drawSeriesTexts(I({ shared: !1 }, T)), this.tooltipUtil.hasBars() && D.tooltipPosition.moveStickyTooltipOverBars(i, s), S && D.tooltipPosition.moveMarkers(s, i);
	}
};
var tt = class dt {
	constructor(t) {
	...
	attr(t, e) {
		if (typeof t == "string" && e === void 0) return this.node.getAttribute(t);
	...
	css(t) {
		for (const e in t) this.node.style[e] = t[e];
		return this;
	}
	fill(t) {
	...
	find(t) {
		return Array.from(this.node.querySelectorAll(t)).map((e) => e.instance || new dt(e));
	}
	findOne(t) {
	...
	on(t, e) {
		...
		return this._listeners.push({
			...
			eventType: s,
			handler: e
		}), this.node.addEventListener(s, e), this;
	}
	off(t, e) {
		if (!t && !e) this._listeners.forEach((s) => {
...
var cs = class extends tt {
	constructor(t, e, s, i) {
		...
		}), typeof i == "function") i(new At(this.node));
		let o = t.node.querySelector("defs");
		o || (o = N.createElementNS(G, "defs"), t.node.appendChild(o)), o.appendChild(this.node);
	}
	url() {
...
var At = class At extends tt {
	line(t, e, s, i) {
		const a = this._make("line");
		return t !== void 0 && a.attr({
			x1: t,
			y1: e,
			x2: s,
			y2: i
	...
	image(t, e) {
		...
		if (this.node.appendChild(s), typeof e == "function") {
			const a = new Image();
			a.onload = function() {
				i.size(a.width, a.height), e.call(i, {
					width: a.width,
					height: a.height
				});
			}, a.src = t;
...
var ps = class {
	...
	_primitive(t, e) {
		...
		for (const i in e) s.setAttribute(i, e[i]);
...
function qt(A) {
	...
	let i;
	const a = [], o = [];
	for (; (i = e.exec(A)) !== null;) a.push(i[1]), o.push(i.index);
	for (let r = 0; r < a.length; r++) {
		const n = o[r] + a[r].length, l = r + 1 < o.length ? o[r + 1] : A.length, c = A.substring(n, l), h = [];
		let d;
		for (s.lastIndex = 0; (d = s.exec(c)) !== null;) h.push(parseFloat(d[0]));
		const p = a[r].toUpperCase();
		if (p === "Z") t.push(["Z"]);
		else if (p === "M" || p === "L" || p === "T") for (let g = 0; g < h.length; g += 2) t.push([
			p,
			h[g],
			h[g + 1]
		]);
		else if (p === "H") for (let g = 0; g < h.length; g++) t.push([p, h[g]]);
		else if (p === "V") for (let g = 0; g < h.length; g++) t.push([p, h[g]]);
		else if (p === "C") for (let g = 0; g < h.length; g += 6) t.push([
			p,
			h[g],
			h[g + 1],
			h[g + 2],
		...
		else if (p === "S" || p === "Q") for (let g = 0; g < h.length; g += 4) t.push([
			p,
			h[g],
			h[g + 1],
			h[g + 2],
		...
		else if (p === "A") for (let g = 0; g < h.length; g += 7) t.push([
			p,
			h[g],
			h[g + 1],
			h[g + 2],
...
function Zt(A) {
	let t = Infinity, e = Infinity, s = -Infinity, i = -Infinity;
	return A.forEach((a) => {
		for (let o = 1; o < a.length; o += 2) if (o + 1 <= a.length) {
			const r = a[o], n = a[o + 1];
...
function Kt(A) {
	var t = A.length;
	return this.pos = [A[t - 2], A[t - 1]], "SCQT".indexOf(A[0]) != -1 && (this.reflection = [2 * this.pos[0] - A[t - 4], 2 * this.pos[1] - A[t - 3]]), A;
}
function Jt(A) {
	var t = [A];
	switch (A[0]) {
		case "M": return this.pos = this.start = [A[1], A[2]], t;
		case "L":
...
function Qt(A, t) {
	if (t === !1) return !1;
	for (var e = t, s = A.length; e < s; ++e) if (A[e][0] == "M") return e;
	return !1;
}
function ms(A, t) {
	...
	if (e === 0 || s === 0 || l.x === c.x && l.y === c.y) return [[
		"C",
		l.x,
		l.y,
		c.x,
	...
	for (h = new et((l.x - c.x) / 2, (l.y - c.y) / 2).transform(new lt().rotate(i)), d = h.x * h.x / (e * e) + h.y * h.y / (s * s), d > 1 && (d = Math.sqrt(d), e = d * e, s = d * s), p = new lt().rotate(i).scale(1 / e, 1 / s).rotate(-i), l = l.transform(p), c = c.transform(p), g = [c.x - l.x, c.y - l.y], x = g[0] * g[0] + g[1] * g[1], f = Math.sqrt(x), g[0] /= f, g[1] /= f, m = x < 4 ? Math.sqrt(1 - x / 4) : 0, a === o && (m *= -1), u = new et((c.x + l.x) / 2 + m * -g[1], (c.y + l.y) / 2 + m * g[0]), y = new et(l.x - u.x, l.y - u.y), b = new et(c.x - u.x, c.y - u.y), w = Math.acos(y.x / Math.sqrt(y.x * y.x + y.y * y.y)), y.y < 0 && (w *= -1), v = Math.acos(b.x / Math.sqrt(b.x * b.x + b.y * b.y)), b.y < 0 && (v *= -1), o && w > v && (v += 2 * Math.PI), !o && w < v && (v -= 2 * Math.PI), D = Math.ceil(Math.abs(w - v) * 2 / Math.PI), k = [], F = w, C = (v - w) / D, S = 4 * Math.tan(C / 4) / 3, X = 0; X <= D; X++) M = Math.cos(F), T = Math.sin(F), E = new et(u.x + M, u.y + T), k[X] = [
		new et(E.x + S * T, E.y - S * M),
		E,
		new et(E.x - S * T, E.y + S * M)
	], F += C;
	for (k[0][0] = k[0][1].clone(), k[k.length - 1][2] = k[k.length - 1][1].clone(), p = new lt().rotate(i).scale(e, s).rotate(-i), X = 0, R = k.length; X < R; X++) k[X][0] = k[X][0].transform(p), k[X][1] = k[X][1].transform(p), k[X][2] = k[X][2].transform(p);
	for (X = 1, R = k.length; X < R; X++) E = k[X - 1][2], z = E.x, H = E.y, E = k[X][0], _ = E.x, $ = E.y, E = k[X][1], r = E.x, n = E.y, Y.push([
		"C",
		z,
		H,
		_,
...
function bs(A, t, e, s, i, a) {
	for (var o = A.slice(t, e || void 0), r = s.slice(i, a || void 0), n = 0, l = {
		pos: [0, 0],
		start: [0, 0]
	}, c = {
		pos: [0, 0],
		start: [0, 0]
	}; o[n] = jt.call(l, o[n]), r[n] = jt.call(c, r[n]), o[n][0] != r[n][0] || o[n][0] == "M" || o[n][0] == "A" && (o[n][4] != r[n][4] || o[n][5] != r[n][5]) ? (Array.prototype.splice.apply(o, [n, 1].concat(Jt.call(l, o[n]))), Array.prototype.splice.apply(r, [n, 1].concat(Jt.call(c, r[n])))) : (o[n] = Kt.call(l, o[n]), r[n] = Kt.call(c, r[n])), !(++n == o.length && n == r.length);) n == o.length && o.push([
		"C",
		l.pos[0],
		l.pos[1],
		l.pos[0],
...
function ys(A, t) {
	for (var e = qt(A), s = qt(t), i = 0, a = 0, o = !1, r = !1, n; !(i === !1 && a === !1);) {
		if (o = Qt(e, i === !1 ? !1 : i + 1), r = Qt(s, a === !1 ? !1 : a + 1), i === !1) {
			const l = Zt(n.start);
		...
		n = bs(e, i, o, s, a, r), e = e.slice(0, i).concat(n.start, o === !1 ? [] : e.slice(o)), s = s.slice(0, a).concat(n.dest, r === !1 ? [] : s.slice(r)), i = o === !1 ? !1 : i + n.start.length, a = r === !1 ? !1 : a + n.dest.length;
	}
	return {
		start: e,
		dest: s
...
function te(A) {
	...
	const t = A.match(/rgba?\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*(?:,\s*([\d.]+))?\s*\)/);
	return t ? [
		+t[1],
		+t[2],
		+t[3],
...
var Gt = class Gt {
	...
	_execute(t) {
		...
		const i = () => {
			const a = {}, o = {}, r = {};
			if (this._attrTarget) for (const h of Object.keys(this._attrTarget)) {
				const d = e.attr(h);
				a[h] = d;
			...
			const l = performance.now(), c = (h) => {
				...
				if (this._attrTarget) if (p >= 1) e.attr(this._attrTarget);
				else {
					const f = {};
					for (const x of Object.keys(this._attrTarget)) if (o[x] && r[x]) f[x] = As(o[x], r[x], g);
					else {
...
function Ss(A) {
	A.prototype.draggable = function(t) {
		if (t === !1) return this._dragCleanup && (this._dragCleanup(), this._dragCleanup = null), this;
		const e = this, s = t || {}, i = (a) => {
			...
			const f = (m) => {
				...
				const S = {
					...
					x2: w + C,
					y2: v + D
				}, k = new CustomEvent("dragmove", { detail: {
					handler: { move: function(F, T) {
...
function ks(A) {
	A.prototype.select = function(t) {
		if (t === !1) return this._selectCleanup && (this._selectCleanup(), this._selectCleanup = null), this;
		...
		n.forEach((c, h) => {
			const d = new At(document.createElementNS(G, "g"));
		...
		if (t === !1) return this._resizeCleanup && (this._resizeCleanup(), this._resizeCleanup = null), this;
		...
		const i = [], a = (o) => {
			...
			const n = r.group.node, l = (c) => {
				...
				const x = parseFloat(e.attr("x")) || 0, m = parseFloat(e.attr("width")) || 0, u = (b) => {
					...
					f && (v = v / f.a);
					let C = x, D = m;
					o === "l" ? (C = x + v, D = m - v) : o === "r" && (D = m + v), D < 0 && (D = 0), e.attr({
						x: C,
						width: D
					}), e._updateSelectPositions && e._updateSelectPositions();
					const S = new CustomEvent("resize", { detail: { el: e } });
					...
					const b = new CustomEvent("resize", { detail: { el: e } });
...
ks(tt);
function Tt() {
	const t = new At(N.createElementNS(G, "svg"));
...
var we = class zt {
	...
	*/
	static registerFeatures(t) {
		for (const [e, s] of Object.entries(t)) zt._featureRegistry.set(e, s);
	}
	constructor(t) {
...
var ee = class {
	...
	clearDomElements({ isUpdating: t }) {
		...
		if (B.isBrowser()) {
			...
			}), this.ctx.el !== null) for (; this.ctx.el.firstChild;) this.ctx.el.removeChild(this.ctx.el.firstChild);
...
var K = class K {
	...
	*/
	render() {
		return new Promise((t, e) => {
			...
			} else e(/* @__PURE__ */ new Error("Element not found"));
		});
	}
	create(t, e) {
		var s;
		...
		if (o.noData = !1, o.animationEnded = !1, !L.elementExists(this.el) || (this.responsive.checkResponsiveConfig(e), i.config.xaxis.convertedCatToNumeric && new bt(i.config).convertCatToNumericXaxis(i.config, this.ctx), this.core.setupElements(), i.config.chart.type === "treemap" && (i.config.grid.show = !1, i.config.yaxis[0].show = !1), o.svgWidth === 0)) return o.animationEnded = !0, null;
		let r = t;
		t.forEach((u, y) => {
			u.hidden && (r = this.legend.legendHelpers.getSeriesAfterCollapsing({ realIndex: y }));
		});
	...
	mount(t = null) {
		...
		return new Promise((i, a) => {
			var o, r, n, l, c, h, d, p;
			if (e.el === null) return a(/* @__PURE__ */ new Error("Not enough data to display or target element not found"));
			...
			if (e.annotations = f ? new f(e.w, {
				theme: e.theme,
				timeScale: e.timeScale
			}) : null, (o = e.annotations) == null || o.drawImageAnnos(), (r = e.annotations) == null || r.drawTextAnnos(), s.config.grid.position === "back" && (g && s.dom.elGraphical.add(g.el), (n = g == null ? void 0 : g.elGridBorders) != null && n.node && s.dom.elGraphical.add(g.elGridBorders)), Array.isArray(t.elGraph)) for (let u = 0; u < t.elGraph.length; u++) s.dom.elGraphical.add(t.elGraph[u]);
			...
			const x = new wt(this.w, this.ctx, g), m = new _t(this.w, {
				theme: this.theme,
				timeScale: this.timeScale
			}, g);
			if (g !== null && (x.xAxisLabelCorrections(g.xAxisTickWidth), m.setYAxisTextAlignments(), s.config.yaxis.map((u, y) => {
				...
				if (B.isBrowser() && s.config.tooltip.enabled && !s.globals.noData && e.w.globals.tooltip.drawTooltip(t.xyRatios), s.config.chart.accessibility.enabled && s.config.chart.accessibility.keyboard.enabled && s.config.chart.accessibility.keyboard.navigation.enabled && ((h = e.keyboardNavigation) == null || h.init()), B.isBrowser() && s.globals.axisCharts && (s.axisFlags.isXNumeric || s.config.xaxis.convertedCatToNumeric || s.axisFlags.isRangeBar)) (s.config.chart.zoom.enabled || s.config.chart.selection && s.config.chart.selection.enabled || s.config.chart.pan && s.config.chart.pan.enabled) && ((d = e.zoomPanSelection) == null || d.init({ xyRatios: t.xyRatios }));
				else {
	...
	*/
	updateOptions(t, e = !1, s = !0, i = !0, a = !0) {
		...
		if (o.interact.selection = void 0, this.lastUpdateOptions) {
			if (L.shallowEqual(this.lastUpdateOptions, t)) return this;
	...
	*/
	updateSeries(t = [], e = !0, s = !0) {
		return this.data.resetParsingFlags(), this.series.resetSeries(!1), this.updateHelpers.revertDefaultAxisMinMax(), this.updateHelpers._updateSeries(t, e, s);
	}
	/**
	...
	*/
	appendSeries(t, e = !0, s = !0) {
		...
		return i.push(t), this.series.resetSeries(!1), this.updateHelpers.revertDefaultAxisMinMax(), this.updateHelpers._updateSeries(i, e, s);
	}
	/**
	...
	*/
	appendData(t, e = !0) {
		...
		for (let a = 0; a < i.length; a++) if (t[a] !== null && typeof t[a] < "u") for (let o = 0; o < t[a].data.length; o++) i[a].data.push(t[a].data[o]);
		return s.w.config.series = i, e && (s.w.globals.initialSeries = L.clone(s.w.config.series)), this.update();
	}
	update(t) {
		return new Promise((e, s) => {
			if (this.lastUpdateOptions && JSON.stringify(this.lastUpdateOptions) === JSON.stringify(t)) return e(this);
	...
	*/
	static initOnLoad() {
		...
		for (let e = 0; e < t.length; e++) {
			const s = t[e];
			new K(s, JSON.parse(t[e].getAttribute("data-options"))).render();
		}
	}
	/**
...
var Ae = class {
	...
	svgUrl() {
		return new Promise((t) => {
			this.getSvgString().then((e) => {
				const s = new Blob([e], { type: "image/svg+xml;charset=utf-8" });
		...
		return B.isSSR() ? Promise.resolve({ imgURI: "" }) : new Promise((e) => {
			...
			r.fillStyle = o, r.fillRect(0, 0, a.width * i, a.height * i), this.getSvgString(i).then((n) => {
				...
				c.crossOrigin = "anonymous", c.onload = () => {
					if (r.drawImage(c, 0, 0), a.msToBlob) e({ blob: a.msToBlob() });
					else e({ imgURI: a.toDataURL("image/png") });
				}, c.src = l;
			});
		});
	}
	exportToSVG() {
		this.svgUrl().then((t) => {
			this.triggerDownload(t, this.w.config.chart.toolbar.export.svg.filename, ".svg");
		});
	}
	exportToPng() {
		...
		this.dataURI(s).then(({ imgURI: i, blob: a }) => {
			a ? navigator.msSaveOrOpenBlob(a, this.w.globals.chartID + ".png") : this.triggerDownload(i, this.w.config.chart.toolbar.export.png.filename, ".png");
		});
	}
	exportToCSV({ series: t, fileName: e, columnDelimiter: s = ",", lineDelimiter: i = `
` }) {
		const a = this.w;
		t || (t = a.config.series);
		let o = [];
		const r = [];
		let n = "";
		const l = "﻿", c = a.seriesData.series.map((b, w) => a.globals.collapsedSeriesIndices.indexOf(w) === -1 ? b : []), h = (b) => typeof a.config.chart.toolbar.export.csv.categoryFormatter == "function" ? a.config.chart.toolbar.export.csv.categoryFormatter(b) : a.config.xaxis.type === "datetime" && String(b).length >= 10 ? new Date(b).toDateString() : L.isNumber(b) ? b : b.split(s).join(""), d = (b) => typeof a.config.chart.toolbar.export.csv.valueFormatter == "function" ? a.config.chart.toolbar.export.csv.valueFormatter(b) : b, p = Math.max(...t.map((b) => b.data ? b.data.length : 0)), g = new Wt(this.w), f = new at(this.w, {
			theme: this.ctx.theme,
			timeScale: this.ctx.timeScale
		}), x = (b) => {
			let w = "";
			if (!a.globals.axisCharts) w = a.config.labels[b];
			else {
				if (a.config.xaxis.type === "category" || a.config.xaxis.convertedCatToNumeric) if (a.globals.isBarHorizontal) {
				...
				} else w = f.getLabel(a.labelData.labels, a.labelData.timescaleLabels, 0, b).text;
		...
		o.push(a.config.chart.toolbar.export.csv.headerCategory), a.config.chart.type === "boxPlot" ? (o.push("minimum"), o.push("q1"), o.push("median"), o.push("q3"), o.push("maximum")) : a.config.chart.type === "candlestick" ? (o.push("open"), o.push("high"), o.push("low"), o.push("close")) : a.config.chart.type === "rangeBar" ? (o.push("minimum"), o.push("maximum")) : t.map((b, w) => {
			...
			a.globals.axisCharts && o.push(v.split(s).join("") ? v.split(s).join("") : `series-${w}`);
		...
		}), n += r.join(i), this.triggerDownload("data:text/csv; charset=utf-8," + encodeURIComponent(l + n), e || a.config.chart.toolbar.export.csv.filename, ".csv");
	}
...
var Ms = class {
	...
	toggleDataSeries(t, e) {
		...
		if (s.globals.axisCharts || s.config.chart.type === "radialBar") {
			...
			if (s.globals.risingSeries = [], s.globals.axisCharts) {
				if (i = s.dom.baseEl.querySelector(`.apexcharts-series[data\\:realIndex='${t}']`), !i) return;
				a = parseInt(i.getAttribute("data:realIndex"), 10);
			} else {
				if (i = s.dom.baseEl.querySelector(`.apexcharts-series[rel='${t + 1}']`), !i) return;
			...
			if (e ? [{
				...
				const o = s.dom.baseEl.querySelector(`.apexcharts-legend-series[rel="${t + 1}"]`);
				if (o) {
					...
					const n = o.querySelector(".apexcharts-legend-text"), l = n ? n.textContent : s.seriesData.seriesNames[t], c = r ? "hidden" : "visible";
					o.setAttribute("aria-label", `${l}, ${c}. Press Enter or Space to toggle.`);
				}
			}
		} else {
			const i = s.dom.Paper.findOne(` .apexcharts-series[rel='${t + 1}'] path`), a = s.config.chart.type;
	...
	hideSeries({ seriesEl: t, realIndex: e }) {
		const s = this.w, i = this.getSeriesAfterCollapsing({ realIndex: e }), a = t.childNodes;
		for (let o = 0; o < a.length; o++) a[o].classList.contains("apexcharts-series-markers-wrap") && (a[o].classList.contains("apexcharts-hide") ? a[o].classList.remove("apexcharts-hide") : a[o].classList.add("apexcharts-hide"));
		this.lgCtx.updateSeries(i, s.config.chart.animations.dynamicAnimation.enabled);
	}
	riseCollapsedSeries(t, e, s) {
		const i = this.w;
		let a = L.clone(i.config.series);
		if (t.length > 0) {
			for (let o = 0; o < t.length; o++) t[o].index === s && (i.globals.axisCharts ? a[s].data = t[o].data.slice() : a[s] = t[o].data, typeof a[s] != "number" && (a[s].hidden = !1), t.splice(o, 1), e.splice(o, 1), i.globals.risingSeries.push(s), o--);
			a = this._getSeriesBasedOnCollapsedState(a), this.lgCtx.updateSeries(a, i.config.chart.animations.dynamicAnimation.enabled);
		}
	}
...
var Ps = class {
	...
	createLegendMarker({ i: t, fillcolor: e }) {
		...
		if (h.height = (r + c) * 2 + "px", h.width = (r + c) * 2 + "px", h.left = n + "px", h.top = l + "px", s.config.legend.markers.customHTML) h.background = "transparent", h.color = e[t], Array.isArray(s.config.legend.markers.customHTML) ? s.config.legend.markers.customHTML[t] && (i.innerHTML = s.config.legend.markers.customHTML[t]()) : i.innerHTML = s.config.legend.markers.customHTML();
			...
			s.dom.Paper.find(".apexcharts-legend-marker.apexcharts-marker").forEach((u) => {
	...
	drawLegends() {
		...
		} else this.isBarsDistributed && (a = s.labelData.labels.slice());
		...
		s.labelData.seriesGroups.length > 1 && s.config.legend.clusterGroupedSeries && s.labelData.seriesGroups.forEach((c, h) => {
			l[h] = N.createElement("div"), l[h].classList.add("apexcharts-legend-group", `apexcharts-legend-group-${h}`), s.config.legend.clusterGroupedSeriesOrientation === "horizontal" ? s.dom.elLegendWrap.classList.add("apexcharts-legend-group-horizontal") : l[h].classList.add("apexcharts-legend-group-vertical");
		});
		for (let c = n ? a.length - 1 : 0; n ? c >= 0 : c <= a.length - 1; n ? c-- : c++) {
			...
			let d = !1, p = !1;
			if (s.globals.collapsedSeries.length > 0) for (let y = 0; y < s.globals.collapsedSeries.length; y++) s.globals.collapsedSeries[y].index === c && (d = !0);
			if (s.globals.ancillaryCollapsedSeriesIndices.length > 0) for (let y = 0; y < s.globals.ancillaryCollapsedSeriesIndices.length; y++) s.globals.ancillaryCollapsedSeriesIndices[y] === c && (p = !0);
			const g = this.createLegendMarker({
				i: c,
				fillcolor: o
			});
			P.setAttrs(g, {
			...
			if (s.config.chart.accessibility.enabled && s.config.chart.accessibility.keyboard.enabled) {
				...
				f.setAttribute("aria-label", `${y}, ${w}. Press Enter or Space to toggle.`), f.setAttribute("aria-pressed", b ? "true" : "false");
			}
		...
		s.dom.elWrap.addEventListener("click", e.onLegendClick, !0), s.config.legend.onItemHover.highlightDataSeries && s.config.legend.customLegendItems.length === 0 && (s.dom.elWrap.addEventListener("mousemove", e.onLegendHovered, !0), s.dom.elWrap.addEventListener("mouseout", e.onLegendHovered, !0)), s.config.chart.accessibility.enabled && s.config.chart.accessibility.keyboard.enabled && s.dom.elWrap.addEventListener("keydown", e.onLegendKeyDown.bind(e), !0);
	}
	setLegendWrapXY(t, e) {
		const s = this.w, i = s.dom.elLegendWrap, a = i.clientHeight;
		let o = 0, r = 0;
		if (s.config.legend.position === "bottom") r = s.globals.svgHeight - Math.min(a, s.globals.svgHeight / 2) - 5;
	...
	onLegendHovered(t) {
		...
		if (e.config.chart.type !== "heatmap" && !this.isBarsDistributed) !t.target.classList.contains("apexcharts-inactive-legend") && s && new U(this.ctx.w).toggleSeriesOnHover(t, t.target);
		else if (s) {
			...
			this.ctx.events.fireEvent("legendHover", [
				...
				i,
				this.w
			]), new U(this.ctx.w).highlightRangeInSeries(t, t.target);
		}
	}
	onLegendKeyDown(t) {
		...
		if ((t.target.classList.contains("apexcharts-legend-series") || t.target.classList.contains("apexcharts-legend-text") || t.target.classList.contains("apexcharts-legend-marker")) && (t.key === "Enter" || t.key === " ")) {
			...
			e.onLegendClick(t), a !== null && s.config.legend.onItemClick.toggleDataSeries && requestAnimationFrame(() => {
				const o = s.dom.baseEl.querySelector(`.apexcharts-legend-series[rel="${a}"]`);
				o && o.focus();
			});
		}
	}
	onLegendClick(t) {
		...
		if (!e.config.legend.customLegendItems.length && (t.target.classList.contains("apexcharts-legend-series") || t.target.classList.contains("apexcharts-legend-text") || t.target.classList.contains("apexcharts-legend-marker"))) {
			...
			typeof a == "function" && a(this.ctx, s, this.w), this.ctx.events.fireEvent("legendClick", [
				this.ctx,
				s,
				this.w
			]);
...
var Ce = class {
	...
	createToolbar() {
		...
		if (s.setAttribute("class", "apexcharts-toolbar"), s.style.top = t.config.chart.toolbar.offsetY + "px", s.style.right = -t.config.chart.toolbar.offsetX + 3 + "px", t.dom.elWrap.appendChild(s), this.elZoom = e(), this.elZoomIn = e(), this.elZoomOut = e(), this.elPan = e(), this.elSelection = e(), this.elZoomReset = e(), this.elMenuIcon = e(), this.elMenu = e(), this.elCustomIcons = [], this.t = t.config.chart.toolbar.tools, Array.isArray(this.t.customIcons)) for (let r = 0; r < this.t.customIcons.length; r++) this.elCustomIcons.push(e());
		const i = [], a = (r, n, l) => {
		...
		for (let r = 0; r < this.elCustomIcons.length; r++) i.push({
		...
		for (let r = 0; r < i.length; r++) P.setAttrs(i[r].el, {
			class: i[r].class,
			title: i[r].title,
			tabindex: "0",
			role: "button",
	...
	_createHamburgerMenu(t) {
		...
		for (let s = 0; s < e.length; s++) this.elMenuItems.push(N.createElementNS("http://www.w3.org/1999/xhtml", "div")), this.elMenuItems[s].innerHTML = e[s].title, P.setAttrs(this.elMenuItems[s], {
	...
	addToolbarEventListeners() {
		...
		for (let e = 0; e < this.t.customIcons.length; e++) this.elCustomIcons[e].addEventListener("click", this.t.customIcons[e].click.bind(this, this.ctx, this.ctx.w));
		[
			this.elZoomReset,
			this.elSelection,
			this.elZoom,
	...
	getToolbarIconsReference() {
		const t = this.w;
		this.elZoom || (this.elZoom = t.dom.baseEl.querySelector(".apexcharts-zoom-icon")), this.elPan || (this.elPan = t.dom.baseEl.querySelector(".apexcharts-pan-icon")), this.elSelection || (this.elSelection = t.dom.baseEl.querySelector(".apexcharts-selection-icon"));
	}
	...
	togglePanning() {
		this.ctx.getSyncedCharts().forEach((e) => {
			e.ctx.toolbar.toggleOtherControls(), e.w.interact.panEnabled = !e.w.interact.panEnabled, e.ctx.toolbar.elPan.classList.contains(e.ctx.toolbar.selectedClass) ? e.ctx.toolbar.elPan.classList.remove(e.ctx.toolbar.selectedClass) : e.ctx.toolbar.elPan.classList.add(e.ctx.toolbar.selectedClass), e.ctx.toolbar.elPan.setAttribute("aria-pressed", String(e.w.interact.panEnabled));
		});
	}
	toggleOtherControls() {
		const t = this.w;
		t.interact.panEnabled = !1, t.interact.zoomEnabled = !1, t.interact.selectionEnabled = !1, this.getToolbarIconsReference(), [
			this.elPan,
			this.elSelection,
			this.elZoom
		].forEach((s) => {
	...
	handleZoomOut() {
		const t = this.w;
		if (t.axisFlags.isRangeBar && (this.minX = t.globals.minY, this.maxX = t.globals.maxY), t.config.xaxis.type === "datetime" && new Date(this.minX).getUTCFullYear() < 1e3) return;
	...
	zoomUpdateOptions(t, e) {
		...
		if (s.config.xaxis.convertedCatToNumeric && (t < 1 && (t = 1, e = s.globals.dataPoints), e - t < 2)) return;
		let i = {
			min: t,
			max: e
		};
		...
		a && (i = a.xaxis);
		const o = { xaxis: i }, r = L.clone(s.globals.initialConfig.yaxis);
		s.config.chart.group || (o.yaxis = r), this.w.interact.zoomed = !0, this.ctx.updateHelpers._updateOptions(o, !1, this.w.config.chart.animations.dynamicAnimation.enabled), this.zoomCallback(i, r);
	}
	zoomCallback(t, e) {
	...
	handleZoomReset() {
		this.ctx.getSyncedCharts().forEach((e) => {
			...
			const i = e.ctx.series.emptyCollapsedSeries(L.clone(s.globals.initialSeries));
			e.updateHelpers._updateSeries(i, s.config.chart.animations.dynamicAnimation.enabled);
		});
	}
	destroy() {
...
var Ns = class extends Ce {
	...
	svgMouseEvents(t, e) {
		...
		if (e.shiftKey ? (this.shiftWasPressed = !0, i.enableZoomPanFromToolbar(o === "pan" ? "zoom" : "pan")) : this.shiftWasPressed && (i.enableZoomPanFromToolbar(o), this.shiftWasPressed = !1), !e.target) return;
	...
	executeMouseWheelZoom(t) {
		...
		if (!i) return;
		const a = (t.clientX - i.left) / i.width, o = this.minX, r = this.maxX, n = r - o, l = .5, c = 1.5;
		let h, d, p;
		if (t.deltaY < 0) {
			h = l * n;
			const f = o + a * n;
			d = f - h / 2, p = f + h / 2;
		} else h = c * n, d = o - h / 2, p = r + h / 2;
		if (!s.axisFlags.isRangeBar) {
			d = Math.max(d, s.globals.initialMinX), p = Math.min(p, s.globals.initialMaxX);
			const f = (s.globals.initialMaxX - s.globals.initialMinX) * .01;
			if (p - d < f) {
				const x = (d + p) / 2;
				d = x - f / 2, p = x + f / 2;
			}
	...
	makeSelectionRectDraggable() {
		...
		e.width > 0 && e.height > 0 && (this.selectionRect.select(!1).resize(!1), this.selectionRect.select({
			createRot: () => {},
			updateRot: () => {},
			createHandle: (s, i, a, o, r) => r === "l" || r === "r" ? s.circle(8).css({
				"stroke-width": 1,
				stroke: "#333",
				fill: "#fff"
			}) : s.circle(0),
	...
	preselectedSelection() {
		...
		if (!t.interact.zoomEnabled) {
			if (typeof t.interact.selection < "u" && t.interact.selection !== null) this.drawSelectionRect(O(I({}, t.interact.selection), {
				translateX: t.layout.translateX,
				translateY: t.layout.translateY
			}));
	...
	drawSelectionRect({ x: t, y: e, width: s, height: i, translateX: a = 0, translateY: o = 0 }) {
		...
		if (this.dragged || r.interact.selection !== null) {
			const c = { transform: "translate(" + a + ", " + o + ")" };
	...
	selectionDrawn({ context: t, zoomtype: e }) {
		...
		if (s.config.yaxis.forEach((m, u) => {
			...
			if (s.interact.zoomEnabled) {
				...
				const y = { xaxis: u };
				s.config.chart.group || (y.yaxis = m), i.ctx.updateHelpers._updateOptions(y, !1, i.w.config.chart.animations.dynamicAnimation.enabled), typeof s.config.chart.events.zoomed == "function" && o.zoomCallback(u, m);
...
var Pt = class {
	...
	setOrientations(t, e = null) {
		...
		if (t.label.orientation === "vertical") {
			const i = e !== null ? e : 0, a = s.dom.baseEl.querySelector(`.apexcharts-xaxis-annotations .apexcharts-xaxis-annotation-label[rel='${i}']`);
			if (a !== null) {
				const o = a.getBBox();
				a.setAttribute("x", parseFloat(a.getAttribute("x")) - o.height + 4);
	...
	addBackgroundToAnno(t, e) {
		...
		let { left: l, right: c, top: h, bottom: d } = e.label.style.padding;
		e.label.orientation === "vertical" && ([h, d, l, c] = [
			l,
			c,
			h,
			d
		]);
		const p = (n.left - a.left) / r - l, g = (n.top - a.top) / r - h, f = this.annoCtx.graphics.drawRect(p - s.globals.barPadForNumericAxis, g, n.width / r + l + c, n.height / r + h + d, e.label.borderRadius, e.label.style.background, 1, e.label.borderWidth, e.label.borderColor, 0);
		return e.id && f.node.classList.add(e.id), f;
	}
	annotationsBackground() {
		const t = this.w, e = (s, i, a) => {
			const o = t.dom.baseEl.querySelector(`.apexcharts-${a}-annotations .apexcharts-${a}-annotation-label[rel='${i}']`);
			if (o) {
				const r = o.parentNode, n = this.addBackgroundToAnno(o, s);
				n && (r.insertBefore(n.node, o), s.label.mouseEnter && n.node.addEventListener("mouseenter", s.label.mouseEnter.bind(this, s)), s.label.mouseLeave && n.node.addEventListener("mouseleave", s.label.mouseLeave.bind(this, s)), s.label.click && n.node.addEventListener("click", s.label.click.bind(this, s)));
			}
		};
	...
	getY1Y2(t, e) {
		...
		if (this.annoCtx.invertAxis) {
			const n = i.config.xaxis.convertedCatToNumeric ? i.labelData.categoryLabels : i.labelData.labels, l = n.indexOf(a), c = i.dom.baseEl.querySelector(`.apexcharts-yaxis-texts-g text:nth-child(${l + 1})`);
			...
			const n = i.globals.seriesYAxisMap[e.yAxisIndex][0], l = i.config.yaxis[e.yAxisIndex].logarithmic ? new V(this.w).getLogVal(i.config.yaxis[e.yAxisIndex].logBase, a, n) / i.globals.yLogRatio[n] : (a - i.globals.minYArr[n]) / (i.globals.yRange[n] / i.layout.gridHeight);
		...
		return typeof a == "string" && a.includes("px") && (o = parseFloat(a)), {
			yP: o,
			clipped: r
		};
	}
	...
	getStringX(t) {
		...
		const i = e.labelData.labels.map((o) => Array.isArray(o) ? o.join(" ") : o).indexOf(t), a = e.dom.baseEl.querySelector(`.apexcharts-xaxis-texts-g text:nth-child(${i + 1})`);
	...
	addXaxisAnnotation(t, e, s) {
		...
		if (L.isNumber(o)) {
			if (t.x2 === null || typeof t.x2 > "u") {
				if (!r) {
					const d = this.annoCtx.graphics.drawLine(o + t.offsetX, 0 + t.offsetY, o + t.offsetX, i.layout.gridHeight + t.offsetY, t.borderColor, h, t.borderWidth);
				...
				const p = this.annoCtx.graphics.drawRect(o + t.offsetX, 0 + t.offsetY, l - o, i.layout.gridHeight + t.offsetY, 0, t.fillColor, t.opacity, 1, t.borderColor, h);
			...
			if (!(r && n)) {
				const d = this.annoCtx.graphics.getTextRects(c, parseFloat(t.label.style.fontSize)), p = t.label.position === "top" ? 4 : t.label.position === "center" ? i.layout.gridHeight / 2 + (t.label.orientation === "vertical" ? d.width / 2 : 0) : i.layout.gridHeight, g = this.annoCtx.graphics.drawText({
					x: o + t.label.offsetX,
				...
				g.attr({ rel: s }), e.appendChild(g.node), this.annoCtx.helpers.setOrientations(t, s);
			}
		}
	}
	drawXAxisAnnotations() {
		const t = this.w, e = this.annoCtx.graphics.group({ class: "apexcharts-xaxis-annotations" });
...
var Hs = class {
	constructor(t) {
		this.w = t.w, this.annoCtx = t, this.helpers = new Pt(this.annoCtx), this.axesUtils = new at(this.annoCtx.w, {
	...
	addYaxisAnnotation(t, e, s) {
		...
		if (t.y2 === null || typeof t.y2 > "u") {
			if (!n) {
				h = !0;
				const p = this.annoCtx.graphics.drawLine(0 + t.offsetX, r + t.offsetY, this._getYAxisAnnotationWidth(t), r + t.offsetY, t.borderColor, a, t.borderWidth);
			...
			if (!(n && c)) {
				h = !0;
				const p = this.annoCtx.graphics.drawRect(0 + t.offsetX, l + t.offsetY, this._getYAxisAnnotationWidth(t), r - l, 0, t.fillColor, t.opacity, 1, t.borderColor, a);
				p.node.classList.add("apexcharts-annotation-rect"), p.attr("clip-path", `url(#gridRectMask${i.globals.cuid})`), e.appendChild(p.node), t.id && p.node.classList.add(t.id);
			}
		}
		if (h) {
			...
			g.attr({ rel: s }), e.appendChild(g.node);
		}
	}
	...
	drawYAxisAnnotations() {
		const t = this.w, e = this.annoCtx.graphics.group({ class: "apexcharts-yaxis-annotations" });
...
var Os = class {
	...
	addPointAnnotation(t, e, s) {
		if (this.w.globals.collapsedSeriesIndices.indexOf(t.seriesIndex) > -1) return;
		...
		if (L.isNumber(o) && !(l || r)) {
			...
			let h = this.annoCtx.graphics.drawMarker(o + t.marker.offsetX, n + t.marker.offsetY, c);
			...
			if (p.attr({ rel: s }), e.appendChild(p.node), t.customSVG.SVG) {
				const g = this.annoCtx.graphics.group({ class: "apexcharts-point-annotations-custom-svg " + t.customSVG.cssClass });
				g.attr({ transform: `translate(${o + t.customSVG.offsetX}, ${n + t.customSVG.offsetY})` }), g.node.innerHTML = t.customSVG.SVG, e.appendChild(g.node);
			}
			if (t.image.path) {
	...
	drawPointAnnotations() {
		const t = this.w, e = this.annoCtx.graphics.group({ class: "apexcharts-point-annotations" });
...
var _s = class {
	...
	drawAxesAnnotations() {
		...
		if (t.globals.axisCharts && t.globals.dataPoints) {
			const e = this.yAxisAnnotations.drawYAxisAnnotations(), s = this.xAxisAnnotations.drawXAxisAnnotations(), i = this.pointsAnnotations.drawPointAnnotations(), a = t.config.chart.animations.enabled, o = [
				e,
				s,
				i
			], r = [
			...
			for (let n = 0; n < 3; n++) t.dom.elGraphical.add(o[n]), a && !t.globals.resized && !t.globals.dataChanged && t.config.chart.type !== "scatter" && t.config.chart.type !== "bubble" && t.globals.dataPoints > 1 && r[n].classList.add("apexcharts-element-hidden"), t.globals.delayedElements.push({
				el: r[n],
				index: 0
			});
	...
	addText(t) {
		const { x: e, y: s, text: i, textAnchor: a, foreColor: o, fontSize: r, fontFamily: n, fontWeight: l, cssClass: c, backgroundColor: h, borderWidth: d, strokeDashArray: p, borderRadius: g, borderColor: f, appendTo: x = ".apexcharts-svg", paddingLeft: m = 4, paddingRight: u = 4, paddingBottom: y = 2, paddingTop: b = 2 } = t, w = this.w, v = this.graphics.drawText({
			x: e,
			y: s,
			text: i,
			textAnchor: a || "start",
		...
		const D = v.bbox();
		if (i) {
			const S = this.graphics.drawRect(D.x - m, D.y - b, D.width + m + u, D.height + y + b, g, h || "transparent", 1, d, f, p);
			C.insertBefore(S.node, v.node);
		}
	}
	addImage(t) {
		const e = this.w, { path: s, x: i = 0, y: a = 0, width: o = 20, height: r = 20, appendTo: n = ".apexcharts-svg" } = t, l = e.dom.Paper.image(s);
		l.size(o, r).move(i, a);
	...
	addAnnotationExternal({ params: t, pushToMemory: e, context: s, type: i, contextMethod: a }) {
		const o = s, r = o.w, n = r.dom.baseEl.querySelector(`.apexcharts-${i}-annotations`), l = n.childNodes.length + 1, c = new nt(), h = Object.assign({}, i === "xaxis" ? c.xAxisAnnotation : i === "yaxis" ? c.yAxisAnnotation : c.pointAnnotation), d = L.extend(h, t);
		switch (i) {
			case "xaxis":
		...
		const p = r.dom.baseEl.querySelector(`.apexcharts-${i}-annotations .apexcharts-${i}-annotation-label[rel='${l}']`), g = this.helpers.addBackgroundToAnno(p, d);
	...
	clearAnnotations(t) {
		const e = t.w;
		let s = e.dom.baseEl.querySelectorAll(".apexcharts-yaxis-annotations, .apexcharts-xaxis-annotations, .apexcharts-point-annotations");
		for (let i = e.globals.memory.methodsToExec.length - 1; i >= 0; i--) (e.globals.memory.methodsToExec[i].label === "addText" || e.globals.memory.methodsToExec[i].label === "addAnnotation") && e.globals.memory.methodsToExec.splice(i, 1);
...
var Ws = class {
	...
	*/
	...
	_onKeyDown(t) {
		if (!(!this._isNavEnabled() || !this.active)) switch (t.key) {
			case "ArrowRight":
	...
	_move(t, e) {
		...
		if (t !== 0) {
			...
			if (a && a.tConfig && a.tConfig.shared) {
				...
				if (a.tooltipUtil && a.tooltipUtil.isXoverlap(c) && a.tooltipUtil.isInitialSeriesSameLen()) return;
			}
			const o = this._getSeriesCount();
			let r = this.seriesIndex + t, n = 0;
			for (; n < o && (r < 0 && (r = i ? o - 1 : 0), r >= o && (r = i ? 0 : o - 1), !!s.globals.collapsedSeriesIndices.includes(r));) r += t, n++;
			this.seriesIndex = r;
	...
	_skipNullForward() {
		...
		if (Array.isArray(t.seriesData.series[e])) {
			for (; a < s && t.seriesData.series[e][i] === null;) i = (i + 1) % s, a++;
	...
	_skipNullBackward() {
		...
		let i = this.dataPointIndex, a = 0;
		if (Array.isArray(t.seriesData.series[e])) {
			for (; a < s && t.seriesData.series[e][i] === null;) i = (i - 1 + s) % s, a++;
			this.dataPointIndex = i;
		}
	}
	_showCurrentPoint() {
	...
	*/
	...
	_showTooltipBar(t, e, s) {
		...
		s.tooltipLabels.drawSeriesTexts(O(I(I({
			...
			i: t,
			j: e
		}, (c == null ? void 0 : c.y1) !== void 0 && { y1: c.y1 }), (c == null ? void 0 : c.y2) !== void 0 && { y2: c.y2 }), { shared: l }));
		const h = `.apexcharts-series[data\\:realIndex='${t}']`, d = n.dom.Paper.findOne(`${h} path[j='${e}'], ${h} circle[j='${e}'], ${h} rect[j='${e}']`);
		if (d && (this._leaveHoveredBar(), new P(this.w, this.ctx).pathMouseEnter(d, null), this._hoveredBarEl = d), n.globals.isBarHorizontal) {
			...
			if (p) {
				...
				let C = x + y;
				x < (s.xyRatios && s.xyRatios.baseLineInvertedY != null ? s.xyRatios.baseLineInvertedY : g.width / 2) && (C = x - b);
	...
	*/
	_showScatterBubblePoint(t, e, s) {
		...
		const a = i.querySelector(`.apexcharts-series[data\\:realIndex='${t}']`);
		if (!a) return;
	...
	_showTooltipNonAxis(t, e, s, i) {
		...
		const o = i.getBoundingClientRect(), r = o.width || s.tooltipRect.ttWidth || 0, n = o.height || s.tooltipRect.ttHeight || 0, l = a.dom.baseEl.querySelector(`.apexcharts-pie-area[j='${e}']`);
		if (l) {
	...
	_showTooltipRadialBar(t, e, s, i) {
		...
		const { ttWidth: o = 0, ttHeight: r = 0 } = s.getCachedDimensions(), n = a.dom.baseEl.querySelector(`.apexcharts-radialbar-series[data\\:realIndex='${t}'] path`);
		if (n) {
			const l = parseFloat(n.getAttribute("data:angle")) || 0, h = (a.config.plotOptions.radialBar.startAngle || 0) + l / 2, d = a.layout.gridWidth / 2, p = a.layout.gridHeight / 2, g = a.globals.radialSize || Math.min(a.layout.gridWidth, a.layout.gridHeight) / 2, f = a.seriesData.series.length, x = g / Math.max(f, 1), m = g - t * x, y = (m + (m - x)) / 2, b = L.polarToCartesian(d, p, y, h), w = b.x + (a.layout.translateX || 0), v = b.y + (a.layout.translateY || 0);
	...
	_showTooltipHeatTree(t, e, s, i, a) {
		...
		const r = i.getBoundingClientRect(), n = r.width || s.tooltipRect.ttWidth || 0, l = r.height || s.tooltipRect.ttHeight || 0, c = a === "heatmap" ? "apexcharts-heatmap-rect" : "apexcharts-treemap-rect", h = o.dom.baseEl.querySelector(`.${c}[i='${t}'][j='${e}']`);
		if (h) {
	...
	_getFocusableElement(t, e) {
		...
		return i === "pie" || i === "donut" || i === "polarArea" ? a.querySelector(`.apexcharts-pie-area[j='${e}']`) : i === "heatmap" ? a.querySelector(`.apexcharts-heatmap-rect[i='${t}'][j='${e}']`) : i === "treemap" ? a.querySelector(`.apexcharts-treemap-rect[i='${t}'][j='${e}']`) : i === "radialBar" ? a.querySelector(`.apexcharts-radialbar-series[data\\:realIndex='${t}'] path`) : i === "bar" || i === "candlestick" || i === "boxPlot" || i === "rangeBar" ? a.querySelector(`.apexcharts-series[data\\:realIndex='${t}'] path[j='${e}']`) : a.querySelector(`.apexcharts-series[data\\:realIndex='${t}'] .apexcharts-marker[rel='${e}']`) || null;
	}
	_fireClick() {
	...
	_getDataPointCount(t) {
		const e = this.w, s = e.config.chart.type;
		if (s === "pie" || s === "donut" || s === "polarArea") return e.seriesData.series.length;
	...
	*/
	_snapToVisibleRange() {
		...
		if (r >= a && r <= o) return;
		const n = i.length;
		for (let l = 0; l < n; l++) if (i[l] >= a && i[l] <= o) {
	...
	*/
	_snapToVisibleRangeInDirection(t) {
		...
		const n = a.length;
		if (t >= 0) {
			for (let l = 0; l < n; l++) if (a[l] >= o && a[l] <= r) {
				this.dataPointIndex = l;
				return;
			}
		} else for (let l = n - 1; l >= 0; l--) if (a[l] >= o && a[l] <= r) {
			this.dataPointIndex = l;
			return;
		}
	}
...
var Gs = class {
	...
	**/
	handleBarDataLabels(t) {
		const { x: e, y: s, y1: i, y2: a, i: o, j: r, realIndex: n, columnGroupIndex: l, series: c, barHeight: h, barWidth: d, barXPosition: p, barYPosition: g, visibleSeries: f } = t, x = this.w, m = new P(this.barCtx.w), u = Array.isArray(this.barCtx.strokeWidth) ? this.barCtx.strokeWidth[n] : this.barCtx.strokeWidth;
		let y, b;
		...
		if (x.config.dataLabels.enabled) {
			...
			X = m.getTextRects(x.config.dataLabels.formatter ? x.config.dataLabels.formatter(Y, O(I({}, x), {
				seriesIndex: o,
				dataPointIndex: r,
				w: x
			})) : x.formatters.yLabelFormatters[0](Y), parseFloat(k.style.fontSize));
		}
		const R = {
			x: e,
			y: s,
	...
	calculateColumnsDataLabelsPosition(t) {
		const e = this.w;
		let { i: s, j: i, realIndex: a, y: o, bcx: r, barWidth: n, barHeight: l, textRects: c, dataLabelsX: h, dataLabelsY: d, dataLabelsConfig: p, barDataLabelsConfig: g, barTotalDataLabelsConfig: f, strokeWidth: x, offX: m, offY: u } = t, y, b;
		const w = "middle", v = r;
		l = Math.abs(l);
		...
		if (e.labelData.seriesGroups.forEach((M) => {
			...
			const X = new P(this.barCtx.w).getTextRects(this.getStackedTotalDataLabel({
				realIndex: a,
				j: i
			}), p.fontSize);
	...
	calculateBarsDataLabelsPosition(t) {
		const e = this.w;
		let { x: s, i, j: a, realIndex: o, bcy: r, barHeight: n, barWidth: l, textRects: c, dataLabelsX: h, strokeWidth: d, dataLabelsConfig: p, barDataLabelsConfig: g, barTotalDataLabelsConfig: f, offX: x, offY: m } = t;
		...
		if (e.labelData.seriesGroups.forEach((F) => {
			...
			const T = new P(this.barCtx.w).getTextRects(this.getStackedTotalDataLabel({
				realIndex: o,
				j: a
			}), p.fontSize);
	...
	drawCalculatedDataLabels({ x: t, y: e, val: s, i, j: a, textRects: o, barHeight: r, barWidth: n, dataLabelsConfig: l }) {
		const c = this.w;
		let h = "rotate(0)";
		...
		if (l.enabled && !x) {
			...
			if (c.config.plotOptions.bar.dataLabels.orientation === "vertical" && (y === "top" && (u ? l.textAnchor = "end" : l.textAnchor = "start"), y === "center" && (l.textAnchor = "middle"), y === "bottom" && (u ? l.textAnchor = "end" : l.textAnchor = "start")), this.barCtx.isRangeBar && this.barCtx.barOptions.dataLabels.hideOverflowingLabels) n < p.getTextRects(m, parseFloat(l.style.fontSize)).width && (m = "");
	...
	drawTotalDataLabels({ x: t, y: e, val: s, realIndex: i, textAnchor: a, barTotalDataLabelsConfig: o }) {
...
var $s = class {
	...
	initVariables(t) {
		...
		for (let s = 0; s < t.length; s++) if (t[s].length > 0 && (this.barCtx.seriesLen = this.barCtx.seriesLen + 1, this.barCtx.totalItems += t[s].length), e.axisFlags.isXNumeric) for (let i = 0; i < t[s].length; i++) e.seriesData.seriesX[s][i] > e.globals.minX && e.seriesData.seriesX[s][i] < e.globals.maxX && this.barCtx.visibleItems++;
		else this.barCtx.visibleItems = e.globals.dataPoints;
		this.arrBorderRadius = this.createBorderRadiusArr(e.seriesData.series), L.isSafari() && (this.arrBorderRadius = this.arrBorderRadius.map((s) => s.map((i) => "none"))), this.barCtx.seriesLen === 0 && (this.barCtx.seriesLen = 1), this.barCtx.zeroSerieses = [], e.globals.comboCharts || this.checkZeroSeries({ series: t });
	}
	initialPositions(t) {
		const e = this.w;
		...
		if (e.config.plotOptions.bar.rangeBarGroupRows && (d = 1), this.barCtx.isHorizontal) a = e.layout.gridHeight / h, r = a / d, e.axisFlags.isXNumeric && (a = e.layout.gridHeight / this.barCtx.totalItems, r = a / this.barCtx.seriesLen), r = r * parseInt(this.barCtx.barOptions.barHeight, 10) / 100, String(this.barCtx.barOptions.barHeight).indexOf("%") === -1 && (r = parseInt(this.barCtx.barOptions.barHeight, 10)), c = this.barCtx.baseLineInvertedY + e.globals.padHorizontal + (this.barCtx.isReversed ? e.layout.gridWidth : 0) - (this.barCtx.isReversed ? this.barCtx.baseLineInvertedY * 2 : 0), this.barCtx.isFunnel && (c = e.layout.gridWidth / 2), i = (a - r * this.barCtx.seriesLen) / 2;
			...
			if (o = e.layout.gridWidth / this.barCtx.visibleItems, e.config.xaxis.convertedCatToNumeric && (o = e.layout.gridWidth / e.globals.dataPoints), n = o / d * parseInt(this.barCtx.barOptions.columnWidth, 10) / 100, e.axisFlags.isXNumeric) {
				...
				e.globals.minXDiff && e.globals.minXDiff !== .5 && e.globals.minXDiff / p > 0 && (o = e.globals.minXDiff / p), n = o / d * parseInt(this.barCtx.barOptions.columnWidth, 10) / 100, n < 1 && (n = 1);
			}
	...
	createBorderRadiusArr(t) {
		var e;
		const s = this.w, i = !this.w.config.chart.stacked || s.config.plotOptions.bar.borderRadius <= 0, a = t.length, o = ((e = t[0]) == null ? void 0 : e.length) | 0, r = Array.from({ length: a }, () => Array(o).fill(i ? "top" : "none"));
		if (i) return r;
		...
		for (let l = 0; l < o; l++) {
			...
			if (c.length > 0 && h.length === 0) if (c.length === 1) r[c[0]][l] = n === "bar" && o === 1 ? "top" : "both";
			else {
				const p = c[0], g = c[c.length - 1];
				for (const f of c) f === p ? r[f][l] = n === "bar" && o === 1 ? "top" : "bottom" : f === g ? r[f][l] = "top" : r[f][l] = "none";
			}
			else if (h.length > 0 && c.length === 0) if (h.length === 1) r[h[0]][l] = "both";
			else {
				const p = Math.max(...h), g = Math.min(...h);
				for (const f of h) f === p ? r[f][l] = "bottom" : f === g ? r[f][l] = "top" : r[f][l] = "none";
			}
			else if (c.length > 0 && h.length > 0) {
				const p = c[c.length - 1];
				for (const f of c) f === p ? r[f][l] = "top" : r[f][l] = "none";
				const g = Math.max(...h);
				for (const f of h) f === g ? r[f][l] = "bottom" : r[f][l] = "none";
			} else if (d === 1) {
				const p = c[0] || h[0];
				r[p][l] = "both";
			}
	...
	barBackground({ j: t, i: e, x1: s, x2: i, y1: a, y2: o, elSeries: r }) {
		...
		if (this.barCtx.barOptions.colors.backgroundBarColors.length > 0 && h === e) {
			...
			const d = this.barCtx.barOptions.colors.backgroundBarColors[t], p = l.drawRect(typeof s < "u" ? s : 0, typeof a < "u" ? a : 0, typeof i < "u" ? i : n.layout.gridWidth, typeof o < "u" ? o : n.layout.gridHeight, this.barCtx.barOptions.colors.backgroundBarRadius, d, this.barCtx.barOptions.colors.backgroundBarOpacity);
			r.add(p), p.node.classList.add("apexcharts-backgroundBar");
		}
	}
	getColumnPaths({ barWidth: t, barXPosition: e, y1: s, y2: i, strokeWidth: a, isReversed: o, series: r, seriesGroup: n, realIndex: l, i: c, j: h, w: d }) {
		var p;
		...
		const C = g.line(y, s);
		if (d.globals.previousPaths.length > 0 && (v = this.barCtx.getPreviousPath(l, h, !1)), w = w + g.line(u, i) + g.line(y, i) + C + (d.config.plotOptions.bar.borderRadiusApplication === "around" || this.arrBorderRadius[l][h] === "both" ? " Z" : " z"), v = v + g.line(u, s) + C + C + C + C + C + g.line(u, s) + (d.config.plotOptions.bar.borderRadiusApplication === "around" || this.arrBorderRadius[l][h] === "both" ? " Z" : " z"), this.arrBorderRadius[l][h] !== "none" && (w = g.roundPathCorners(w, d.config.plotOptions.bar.borderRadius)), d.config.chart.stacked) {
			let D = this.barCtx;
	...
	getBarpaths({ barYPosition: t, barHeight: e, x1: s, x2: i, strokeWidth: a, isReversed: o, series: r, seriesGroup: n, realIndex: l, i: c, j: h, w: d }) {
		var p;
		const g = new P(this.barCtx.w);
		a = Array.isArray(a) ? a[l] : a, a || (a = 0);
		let f = t, x = e;
		...
		const C = g.line(s, y);
		if (w = w + g.line(i, u) + g.line(i, y) + C + (d.config.plotOptions.bar.borderRadiusApplication === "around" || this.arrBorderRadius[l][h] === "both" ? " Z" : " z"), v = v + g.line(s, u) + C + C + C + C + C + g.line(s, u) + (d.config.plotOptions.bar.borderRadiusApplication === "around" || this.arrBorderRadius[l][h] === "both" ? " Z" : " z"), this.arrBorderRadius[l][h] !== "none" && (w = g.roundPathCorners(w, d.config.plotOptions.bar.borderRadius)), d.config.chart.stacked) {
			let D = this.barCtx;
	...
	checkZeroSeries({ series: t }) {
		...
		for (let s = 0; s < t.length; s++) {
			let i = 0;
			for (let a = 0; a < t[e.globals.maxValsInArrayIndex].length; a++) i += t[s][a];
	...
	getGoalValues(t, e, s, i, a, o) {
		...
		if (r.seriesData.seriesGoals[i] && r.seriesData.seriesGoals[i][a] && Array.isArray(r.seriesData.seriesGoals[i][a]) && r.seriesData.seriesGoals[i][a].forEach((c) => {
			...
			l(r.rangeData.seriesRangeStart[i][a], h), l(r.rangeData.seriesRangeEnd[i][a], O(I({}, h), { strokeColor: Array.isArray(c[i]) ? c[i][1] : c[i] }));
		}
		return n;
	}
	drawGoalLine({ barXPosition: t, barYPosition: e, goalX: s, goalY: i, barWidth: a, barHeight: o }) {
		const r = new P(this.barCtx.w), n = r.group({ className: "apexcharts-bar-goals-groups" });
		n.node.classList.add("apexcharts-element-hidden"), this.barCtx.w.globals.delayedElements.push({ el: n.node }), n.attr("clip-path", `url(#gridRectMarkerMask${this.barCtx.w.globals.cuid})`);
		let l = null;
		return this.barCtx.isHorizontal ? Array.isArray(s) && s.forEach((c) => {
			if (c.x >= -1 && c.x <= r.w.layout.gridWidth + 1) {
				...
				l = r.drawLine(c.x, d - h * 2, c.x, d, c.attrs.strokeColor ? c.attrs.strokeColor : void 0, c.attrs.strokeDashArray, c.attrs.strokeWidth ? c.attrs.strokeWidth : 2, c.attrs.strokeLineCap), n.add(l);
			}
			...
			if (c.y >= -1 && c.y <= r.w.layout.gridHeight + 1) {
				...
				l = r.drawLine(d - h * 2, c.y, d, c.y, c.attrs.strokeColor ? c.attrs.strokeColor : void 0, c.attrs.strokeDashArray, c.attrs.strokeHeight ? c.attrs.strokeHeight : 2, c.attrs.strokeLineCap), n.add(l);
			}
		}), n;
	}
	drawBarShadow({ prevPaths: t, currPaths: e, color: s, realIndex: i, j: a }) {
		...
		return g.drawPath({
			d: x,
			fill: f.shadeColor(.5, L.rgb2hex(s)),
			stroke: "none",
			strokeWidth: 0,
			fillOpacity: 1,
	...
	getGroupIndex(t) {
		const e = this.w, s = e.labelData.seriesGroups.findIndex((o) => o.indexOf(e.seriesData.seriesNames[t]) > -1), i = this.barCtx.columnGroupIndices;
		let a = i.indexOf(s);
		return a < 0 && (i.push(s), a = i.length - 1), {
			groupIndex: s,
			columnGroupIndex: a
		};
	}
...
var ft = class {
	constructor(t, e, s) {
		...
		this.stackedSeriesTotals = o.getStackedSeriesTotals(this.w.config.series.map((r, n) => a.indexOf(n) === -1 ? n : -1).filter((r) => r !== -1)), this.barHelpers = new $s(this);
	}
	...
	**/
	draw(t, e) {
		...
		const r = a.group({ class: "apexcharts-bar-series apexcharts-plot-series" });
		...
		for (let n = 0, l = 0; n < t.length; n++, l++) {
			...
			const u = this.barHelpers.initialPositions(g), { y, yDivision: b, zeroW: w, x: v, xDivision: C, zeroH: D } = u;
			...
			i.globals.delayedElements.push({ el: F.node }), F.node.classList.add("apexcharts-element-hidden");
			const T = a.group({ class: "apexcharts-bar-goals-markers" }), M = a.group({ class: "apexcharts-bar-shadows" });
			i.globals.delayedElements.push({ el: M.node }), M.node.classList.add("apexcharts-element-hidden");
	...
	renderSeries({ realIndex: t, pathFill: e, lineFill: s, j: i, i: a, columnGroupIndex: o, pathFrom: r, pathTo: n, strokeWidth: l, elSeries: c, x: h, y: d, y1: p, y2: g, series: f, barHeight: x, barWidth: m, barXPosition: u, barYPosition: y, elDataLabelsWrap: b, elGoalsMarkers: w, elBarShadows: v, visibleSeries: C, type: D, classes: S }) {
		const k = this.w, F = new P(this.w, this.ctx);
		let T = !1;
		if (c._bindingsDelegated || (c._bindingsDelegated = !0, F.setupEventDelegation(c, `.apexcharts-${D}-area`)), !s) {
			let R = function(z) {
	...
	drawBarPaths({ indexes: t, barHeight: e, strokeWidth: s, zeroW: i, x: a, y: o, yDivision: r, elSeries: n }) {
		const l = this.w, c = t.i, h = t.j;
		let d;
		if (l.axisFlags.isXNumeric) o = (l.seriesData.seriesX[c][h] - l.globals.minX) / this.invertedXRatio - e, d = o + e * this.visibleI;
		else if (l.config.plotOptions.bar.hideZeroBarsWhenGrouped) {
			const { nonZeroColumns: g, zeroEncounters: f } = this.barHelpers.getZeroValueEncounters({
				i: c,
				j: h
			});
			g > 0 && (e = this.seriesLen * e / g), d = o + e * this.visibleI, d -= e * f;
		} else d = o + e * this.visibleI;
	...
	drawColumnPaths({ indexes: t, x: e, y: s, xDivision: i, barWidth: a, zeroH: o, strokeWidth: r, elSeries: n }) {
		...
		if (l.axisFlags.isXNumeric) {
			...
			const { nonZeroColumns: m, zeroEncounters: u } = this.barHelpers.getZeroValueEncounters({
				i: d,
				j: p
			});
			m > 0 && (a = this.seriesLen * a / m), f = e + a * this.visibleI, f -= a * u;
		} else f = e + a * this.visibleI;
		s = this.barHelpers.getYForValue(this.series[d][p], o, h);
		...
		return l.axisFlags.isXNumeric || (e = e + i), this.barHelpers.barBackground({
			...
			x: e,
			y: s,
			goalY: this.barHelpers.getGoalValues("y", null, o, d, p, h),
			barXPosition: f,
			barWidth: a
		};
	}
...
var Vs = class extends ft {
	draw(t, e) {
		...
		const a = this.graphics.group({ class: "apexcharts-bar-series apexcharts-plot-series" });
		let o = 0, r = 0;
		for (let n = 0, l = 0; n < t.length; n++, l++) {
			...
			const m = this.graphics.group({
				class: "apexcharts-datalabels",
				"data:realIndex": c
			}), u = this.graphics.group({ class: "apexcharts-bar-goals-markers" }), y = this.initialPositions(o, r, void 0, void 0, void 0, void 0, f), { xDivision: b, yDivision: w, zeroH: v, zeroW: C } = y;
	...
	drawStackedBarPaths({ indexes: t, barHeight: e, strokeWidth: s, zeroW: i, x: a, y: o, columnGroupIndex: r, seriesGroup: n, yDivision: l, elSeries: c }) {
		const h = this.w, d = o + r * e;
		let p;
		const g = t.i, f = t.j, x = t.realIndex, m = t.translationsIndex;
		let u = 0;
		for (let w = 0; w < this.groupCtx.prevXF.length; w++) u = u + this.groupCtx.prevXF[w][f];
		let y = g;
		...
		return this.barHelpers.barBackground({
			...
			pathTo: b.pathTo,
			pathFrom: b.pathFrom,
			goalX: this.barHelpers.getGoalValues("x", i, null, g, f, m),
			barXPosition: p,
			barYPosition: d,
			x: a,
			y: o
	...
	drawStackedColumnPaths({ indexes: t, x: e, y: s, xDivision: i, barWidth: a, zeroH: o, columnGroupIndex: r, seriesGroup: n, elSeries: l }) {
		var c, h, d, p;
		...
		const b = e + r * a;
		let w, v = 0;
		for (let S = 0; S < this.groupCtx.prevYF.length; S++) v = v + (isNaN(this.groupCtx.prevYF[S][x]) ? 0 : this.groupCtx.prevYF[S][x]);
		let C = f;
		if (n && (C = n.indexOf(g.seriesData.seriesNames[u])), C > 0 && !g.axisFlags.isXNumeric || C > 0 && g.axisFlags.isXNumeric && g.seriesData.seriesX[u - 1][x] === g.seriesData.seriesX[u][x]) {
			...
			if (this.groupCtx.prevY[C - 1] !== void 0 && this.groupCtx.prevY[C - 1].length) {
				for (let T = 1; T < F; T++) if (!isNaN((c = this.groupCtx.prevY[C - T]) == null ? void 0 : c[x])) {
					k = this.groupCtx.prevY[C - T][x];
					break;
				}
			}
			for (let T = 1; T < F; T++) if (((h = this.groupCtx.prevYVal[C - T]) == null ? void 0 : h[x]) < 0) {
...
var se = class extends ft {
	draw(t, e, s) {
		...
		const n = a.group({ class: `apexcharts-${o}-series apexcharts-plot-series` });
		for (let l = 0; l < t.length; l++) {
			...
			const { y, barHeight: b, yDivision: w, zeroW: v, x: C, barWidth: D, xDivision: S, zeroH: k } = this.barHelpers.initialPositions(g);
			h = y, c = C, p.push(c + D / 2);
			const F = a.group({
				class: "apexcharts-datalabels",
				"data:realIndex": g
			}), T = a.group({ class: "apexcharts-bar-goals-markers" });
	...
	drawVerticalBoxPaths({ indexes: t, x: e, xDivision: s, barWidth: i, zeroH: a, strokeWidth: o }) {
		...
		if (r.globals.previousPaths.length > 0 && (F = this.getPreviousPath(p, c, !0)), this.isOHLC) {
			...
			k = [n.move(T, y) + n.line(T, b) + n.move(T, M) + n.line(S, M) + n.move(T, E) + n.line(S + i, E)];
		} else this.isBoxPlot ? k = [n.move(S, v) + n.line(S + i / 2, v) + n.line(S + i / 2, y) + n.line(S + i / 4, y) + n.line(S + i - i / 4, y) + n.line(S + i / 2, y) + n.line(S + i / 2, v) + n.line(S + i, v) + n.line(S + i, D) + n.line(S, D) + n.line(S, v + o / 2), n.move(S, D) + n.line(S + i, D) + n.line(S + i, C) + n.line(S + i / 2, C) + n.line(S + i / 2, b) + n.line(S + i - i / 4, b) + n.line(S + i / 4, b) + n.line(S + i / 2, b) + n.line(S + i / 2, C) + n.line(S, C) + n.line(S, D) + "z"] : k = [n.move(S, C) + n.line(S + i / 2, C) + n.line(S + i / 2, y) + n.line(S + i / 2, C) + n.line(S + i, C) + n.line(S + i, v) + n.line(S + i / 2, v) + n.line(S + i / 2, b) + n.line(S + i / 2, v) + n.line(S, v) + n.line(S, C - o / 2)];
		return F = F + n.move(S, v), r.axisFlags.isXNumeric || (e = e + s), {
			pathTo: k,
			pathFrom: F,
			x: e,
			y: C,
			goalY: this.barHelpers.getGoalValues("y", null, a, l, c, t.translationsIndex),
			barXPosition: S,
			color: w
		};
	}
	drawHorizontalBoxPaths({ indexes: t, y: e, yDivision: s, barHeight: i, zeroW: a, strokeWidth: o }) {
		...
		const S = [n.move(b, C) + n.line(b, C + i / 2) + n.line(u, C + i / 2) + n.line(u, C + i / 2 - i / 4) + n.line(u, C + i / 2 + i / 4) + n.line(u, C + i / 2) + n.line(b, C + i / 2) + n.line(b, C + i) + n.line(v, C + i) + n.line(v, C) + n.line(b + o / 2, C), n.move(v, C) + n.line(v, C + i) + n.line(w, C + i) + n.line(w, C + i / 2) + n.line(y, C + i / 2) + n.line(y, C + i - i / 4) + n.line(y, C + i / 4) + n.line(y, C + i / 2) + n.line(w, C + i / 2) + n.line(w, C) + n.line(v, C) + "z"];
...
var Se = class {
	...
	getShadeColor(t, e, s, i) {
		...
		let l = n.color;
		const c = new L();
		if (a.config.plotOptions[t].enableShades) if (this.w.config.theme.mode === "dark") {
			const h = c.shadeColor(o * -1, n.color);
			l = L.hexToRgba(L.isColorHex(h) ? h : L.rgb2hex(h), a.config.fill.opacity);
		} else {
...
var Us = class {
	...
	draw(t) {
		const e = this.w, s = new P(this.w, this.ctx), i = s.group({ class: "apexcharts-heatmap" });
		...
		for (let h = n ? 0 : l.length - 1; n ? h < l.length : h >= 0; n ? h++ : h--) {
			...
			for (let x = 0; x < e.globals.dataPoints; x++) {
				...
				if (f >= l[h].length) break;
				const m = this.helpers.getShadeColor(e.config.chart.type, h, f, this.negRange);
				let u = m.color;
				...
				if (e.globals.dataChanged) {
					...
					if (this.dynamicAnim.enabled && e.globals.shouldAnimate) {
						...
						k || (k = "rgba(255, 255, 255, 0)"), this.animateHeatColor(w, L.isColorHex(k) ? k : L.rgb2hex(k), L.isColorHex(u) ? u : L.rgb2hex(u), S);
					}
				}
...
var qs = class {
	...
	calculatePoints({ series: t, realIndex: e, x: s, y: i, i: a, j: o, prevY: r }) {
		...
		return n.axisFlags.isXNumeric && (h = (n.seriesData.seriesX[e][0] - n.globals.minX) / this.lineCtx.xRatio + n.config.markers.offsetX), o === 0 && (l.push(h), c.push(L.isNumber(t[a][0]) ? r + n.config.markers.offsetY : null)), l.push(s + n.config.markers.offsetX), c.push(L.isNumber(t[a][o + 1]) ? i + n.config.markers.offsetY : null), {
			x: l,
			y: c
		};
	}
	...
	determineFirstPrevY({ i: t, realIndex: e, series: s, prevY: i, lineYPosition: a, translationsIndex: o }) {
		var r, n, l;
		...
		if (typeof ((l = s[t]) == null ? void 0 : l[0]) < "u") h ? t > 0 ? a = this.lineCtx.prevSeriesY[t - 1][0] : a = this.lineCtx.zeroY : a = this.lineCtx.zeroY, i = a - s[t][0] / this.lineCtx.yRatio[o] + (this.lineCtx.isReversed ? s[t][0] / this.lineCtx.yRatio[o] : 0) * 2;
		else if (h && t > 0 && typeof s[t][0] > "u") {
			for (let d = t - 1; d >= 0; d--) if (s[d][0] !== null && typeof s[d][0] < "u") {
...
var Zs = (A) => {
	...
	let a, o, r, n;
	for (let l = 0; l < e; l++) r = Ot(A[l], A[l + 1]), Math.abs(r) < s ? t[l] = t[l + 1] = 0 : (a = t[l] / r, o = t[l + 1] / r, n = a * a + o * o, n > 9 && (n = r * 3 / Math.sqrt(n), t[l] = n * a, t[l + 1] = n * o));
	for (let l = 0; l <= e; l++) n = (A[Math.min(e, l + 1)][0] - A[Math.max(0, l - 1)][0]) / (6 * (1 + t[l] * t[l])), i.push([n || 0, t[l] * n || 0]);
	return i;
}, js = (A) => {
	let t = "";
	...
	slice(A, t, e) {
		...
		if (t) {
			if (e - t > 1 && s[1].length < 6) {
				const i = s[0].length;
				s[1] = [s[0][i - 2] * 2 - s[0][i - 4], s[0][i - 1] * 2 - s[0][i - 3]].concat(s[1]);
			}
			s[0] = s[0].slice(-2);
		}
		return s;
...
function Ks(A) {
	...
	for (let o = A.length - 1; a < o; a++) e = s, s = A[a + 1], t[a] = (i + (i = Ot(e, s))) * .5;
	return t[a] = i, t;
}
var mt = class {
	constructor(t, e, s, i) {
	...
	draw(t, e, s, i) {
		var a;
		const o = this.w, r = new P(this.w), n = o.globals.comboCharts ? e : o.config.chart.type, l = r.group({ class: `apexcharts-${n}-series apexcharts-plot-series` }), c = new V(this.w);
		...
		for (let d = 0; d < t.length; d++) {
			...
			if (n === "rangeArea") {
				...
				for (let J = 0; J < W; J++) H.linePaths[J] = $.linePaths[J + W] + H.linePaths[J];
		...
		if (typeof ((a = o.config.series[0]) == null ? void 0 : a.zIndex) < "u" && h.sort((d, p) => Number(d.node.getAttribute("zIndex")) - Number(p.node.getAttribute("zIndex"))), o.config.chart.stacked) for (let d = h.length - 1; d >= 0; d--) l.add(h[d]);
		else for (let d = 0; d < h.length; d++) l.add(h[d]);
		return l;
	}
	...
	_calculatePathsFrom({ type: t, series: e, i: s, realIndex: i, translationsIndex: a, prevX: o, prevY: r, prevY2: n }) {
		const l = this.w, c = new P(this.w);
		let h, d, p, g;
		if (e[s][0] === null) {
			for (let f = 0; f < e[s].length; f++) if (e[s][f] !== null) {
				o = this.xDivision * f, r = this.zeroY - e[s][f] / this.yRatio[a], h = c.move(o, r), d = c.move(o, this.areaBottomY);
				break;
			}
		} else h = c.move(o, r), t === "rangeArea" && (h = c.move(o, n) + c.line(o, r)), d = c.move(o, this.areaBottomY) + c.line(o, r);
	...
	_handlePaths({ type: t, realIndex: e, i: s, paths: i }) {
		...
		if (n.count > 0 && t !== "rangeArea") {
			const c = a.globals.seriesXvalues[e][a.globals.seriesXvalues[e].length - n.count - 1], h = o.drawRect(c, 0, a.layout.gridWidth, a.layout.gridHeight, 0);
			a.dom.elForecastMask.appendChild(h.node);
			const d = o.drawRect(0, 0, c, a.layout.gridHeight, 0);
		...
		if (t === "area") {
			const c = r.fillPath({ seriesNumber: e });
		...
		if (a.config.stroke.show && !this.pointsChart) {
			let c = null;
			if (t === "line") c = r.fillPath({
				seriesNumber: e,
				i: s
			});
			else if (a.config.stroke.fill.type === "solid") c = a.globals.stroke.colors[e];
			else {
			...
			for (let h = 0; h < i.linePaths.length; h++) {
				...
				if (this.elSeries.add(g), g.attr("fill-rule", "evenodd"), n.count > 0 && t !== "rangeArea") {
					...
					f.node.setAttribute("stroke-dasharray", n.dashArray), n.strokeWidth && f.node.setAttribute("stroke-width", n.strokeWidth), this.elSeries.add(f), f.attr("clip-path", `url(#forecastMask${a.globals.cuid})`), g.attr("clip-path", `url(#nonForecastMask${a.globals.cuid})`);
				}
			}
		}
	}
	_iterateOverDataPoints({ type: t, series: e, iterations: s, realIndex: i, translationsIndex: a, i: o, x: r, y: n, pX: l, pY: c, pathsFrom: h, linePaths: d, areaPaths: p, seriesIndex: g, lineYPosition: f, xArrj: x, yArrj: m, y2Arrj: u, isRangeStart: y, seriesRangeEnd: b }) {
		var w, v;
		...
		for (let W = 0; W < s && e[o].length !== 0; W++) {
			...
			if (z) if (o > 0 && C.globals.collapsedSeries.length < C.config.series.length - 1) {
				const Ct = (Le) => {
					for (let ct = Le; ct > 0; ct--) if (C.globals.collapsedSeriesIndices.indexOf((g == null ? void 0 : g[ct]) || ct) > -1) ct--;
					else return ct;
					return 0;
				};
	...
	_createPaths({ type: t, series: e, i: s, j: i, x: a, y: o, xArrj: r, yArrj: n, y2: l, y2Arrj: c, pX: h, pY: d, pathState: p, segmentStartX: g, linePath: f, areaPath: x, linePaths: m, areaPaths: u, curve: y, isRangeStart: b }) {
		...
		switch (y) {
			case "monotoneCubic": {
				const S = b ? n : c, k = (M, E) => M.map((X, R) => [X, E[R]]).filter((X) => X[1] !== null), F = (M) => {
					...
					const X = F(M), R = [];
					for (let Y = 0, z = 0; Y < X.length; z += X[Y++]) R[Y] = ie.slice(E, z, z + X[Y]);
					return R;
				};
				switch (p) {
					case 0:
						if (S[i + 1] === null) break;
						p = 1;
					case 1: if (!(C ? r.length === e[s].length : i === e[s].length - 2)) break;
					case 2: {
						...
						if (T(E, R).forEach((_) => {
							...
							H += _.length;
							const J = H - 1;
							D ? f = w.move(X[W][0], X[W][1]) + $ : C ? f = w.move(Y[W][0], Y[W][1]) + w.line(X[W][0], X[W][1]) + $ + w.line(Y[J][0], Y[J][1]) : (f = w.move(X[W][0], X[W][1]) + $, x = f + w.line(X[J][0], v) + w.line(X[W][0], v) + "z", u.push(x)), m.push(f);
						}), C && z > 1 && !D) {
			...
			case "smooth": {
				...
				if (e[s][i] === null) p = 0;
					...
					case 1:
						if (e[s][i + 1] === null) D ? f += w.line(h, l) : f += w.move(h, d), x += w.line(h, v) + w.line(g, v) + "z", m.push(f), u.push(x), p = -1;
						else {
							const k = w.curve(h + S, d, a - S, o, a, o);
...
var kt = class {
	constructor(t, e) {
		this.ctx = e, this.w = t, this.chartType = this.w.config.chart.type, this.initialAnim = this.w.config.chart.animations.enabled, this.dynamicAnim = this.initialAnim && this.w.config.chart.animations.dynamicAnimation.enabled, this.animBeginArr = [0], this.animDur = 0, this.donutDataLabels = this.w.config.plotOptions.pie.donut.labels, this.lineColorArr = t.globals.stroke.colors !== void 0 ? t.globals.stroke.colors : t.globals.colors, this.defaultSize = Math.min(t.layout.gridWidth, t.layout.gridHeight), this.centerY = this.defaultSize / 2, this.centerX = t.layout.gridWidth / 2, t.config.chart.type === "radialBar" ? this.fullAngle = 360 : this.fullAngle = Math.abs(t.config.plotOptions.pie.endAngle - t.config.plotOptions.pie.startAngle), this.initialAngle = t.config.plotOptions.pie.startAngle % this.fullAngle, t.globals.radialSize = this.defaultSize / 2.05 - t.config.stroke.width - (t.config.chart.sparkline.enabled ? 0 : t.config.chart.dropShadow.blur), this.donutSize = t.globals.radialSize * parseInt(t.config.plotOptions.pie.donut.size, 10) / 100;
	...
	draw(t) {
		const e = this, s = this.w, i = new P(this.w), a = i.group({ class: "apexcharts-pie" });
		if (s.globals.noData) return a;
		let o = 0;
		for (let c = 0; c < t.length; c++) o += L.negToZero(t[c]);
		const r = [], n = i.group();
		...
		if (s.globals.dataChanged) {
			let c = 0;
			for (let d = 0; d < s.globals.previousPaths.length; d++) c += L.negToZero(s.globals.previousPaths[d]);
			let h;
			for (let d = 0; d < s.globals.previousPaths.length; d++) h = this.fullAngle * L.negToZero(s.globals.previousPaths[d]) / c, this.prevSectorAngleArr.push(h);
		}
		...
		if (this.sliceLabels.forEach((c) => {
			l.add(c);
		}), n.attr({ transform: `translate(${this.translateX}, ${this.translateY}) scale(${s.config.plotOptions.pie.customScale})` }), n.add(l), a.add(n), this.donutDataLabels.show) {
			const c = this.renderInnerDataLabels(this.dataLabelsGroup, this.donutDataLabels, {
	...
	drawArcs(t, e) {
		const s = this.w, i = new Z(this.w), a = new P(this.w), o = new st(this.w), r = a.group({ class: "apexcharts-slices" });
		...
		for (let d = 0; d < t.length; d++) {
			...
			this.chartType === "pie" || this.chartType === "polarArea" ? (u = L.polarToCartesian(this.centerX, this.centerY, s.globals.radialSize / 1.25 + s.config.plotOptions.pie.dataLabels.offset, y), b = L.polarToCartesian(this.centerX, this.centerY, s.globals.radialSize / 2, y)) : this.chartType === "donut" && (u = L.polarToCartesian(this.centerX, this.centerY, (s.globals.radialSize + this.donutSize) / 2 + s.config.plotOptions.pie.dataLabels.offset, y), b = L.polarToCartesian(this.centerX, this.centerY, (s.globals.radialSize + this.donutSize) / 2, y)), P.setAttrs(m.node, {
				"data:angle": g,
				"data:startAngle": n,
			...
			if (this.initialAnim && !s.globals.resized && !s.globals.dataChanged ? (w = g / this.fullAngle * s.config.chart.animations.speed, w === 0 && (w = 1), this.animDur = w + this.animDur, this.animBeginArr.push(this.animDur)) : this.animBeginArr.push(0), this.dynamicAnim && s.globals.dataChanged ? this.animatePaths(m, {
				...
				if (g !== 0 && s.config.plotOptions.pie.dataLabels.minAngleToShowLabel < t[d]) {
					...
					const k = s.globals.dataLabels.style.colors[d], F = a.group({ class: "apexcharts-datalabels" }), T = a.drawText({
						x: v,
						y: C,
						text: D,
						textAnchor: "middle",
	...
	addListeners(t, e) {
		...
		t.node.addEventListener("mouseenter", s.pathMouseEnter.bind(this, t)), t.node.addEventListener("mouseleave", s.pathMouseLeave.bind(this, t)), t.node.addEventListener("mouseleave", this.revertDataLabelsInner.bind(this, t.node, e)), t.node.addEventListener("mousedown", s.pathMouseDown.bind(this, t)), this.donutDataLabels.total.showAlways || (t.node.addEventListener("mouseenter", this.printDataLabelsInner.bind(this, t.node, e)), t.node.addEventListener("mousedown", this.printDataLabelsInner.bind(this, t.node, e)));
	}
	animatePaths(t, e) {
	...
	animateArc(t, e, s, i, a, o) {
		...
		n.globals.dataChanged && o.shouldSetPrevPaths && o.prevEndAngle && (h = r.getPiePath({
			...
			(r.chartType === "pie" || r.chartType === "donut" || r.chartType === "polarArea") && this.animate(n.config.chart.animations.dynamicAnimation.speed).attr({ "stroke-width": r.strokeWidth }), o.i === n.config.series.length - 1 && l.animationCompleted(t);
		}).during((f) => {
			d = g + (i - g) * f, o.animateStartingPos && (d = a + (i - a) * f, p = e - a + (s - (e - a)) * f), h = r.getPiePath({
				...
				angle: d,
				size: c
			}), t.node.setAttribute("data:pathOrig", h), t.attr({ d: h });
		}) : (h = r.getPiePath({
			me: r,
			startAngle: p,
			angle: i,
	...
	pieClicked(t) {
		const e = this.w, s = this, i = s.sliceSizes[t] + (e.config.plotOptions.pie.expandOnClick ? 4 : 0), a = e.dom.Paper.findOne(`.apexcharts-${s.chartType.toLowerCase()}-slice-${t}`);
		if (a.attr("data:pieClicked") === "true") {
			...
			const l = e.dom.baseEl.getElementsByClassName("apexcharts-pie-area");
	...
	getPiePath({ me: t, startAngle: e, angle: s, size: i }) {
		...
		const c = Math.PI * (l - 90) / 180, h = t.centerX + i * Math.cos(n), d = t.centerY + i * Math.sin(n), p = t.centerX + i * Math.cos(c), g = t.centerY + i * Math.sin(c), f = L.polarToCartesian(t.centerX, t.centerY, t.donutSize, l), x = L.polarToCartesian(t.centerX, t.centerY, t.donutSize, r), m = s > 180 ? 1 : 0, u = [
			"M",
			h,
			d,
			"A",
		...
		return t.chartType === "donut" ? a = [
			...
			d,
			"z"
		].join(" ") : t.chartType === "pie" || t.chartType === "polarArea" ? a = [
			...u,
			"L",
			t.centerX,
			t.centerY,
			...
			h,
			d
		].join(" ") : a = [...u].join(" "), o.roundPathCorners(a, this.strokeWidth * 2);
	}
	drawPolarElements(t) {
		...
		for (let p = 0; p < c - 1; p++) {
			...
			if (g.attr({
				...
				const f = a.drawYAxisTexts(this.centerX, this.centerY - h + parseInt(e.config.yaxis[0].labels.style.fontSize, 10) / 2, p, l[p]);
				r.add(f);
			}
			o.add(g), h = h - d;
		}
	...
	renderInnerDataLabels(t, e, s) {
		...
		let f = "", x = "";
		if (o ? (l = e.total.color, h = e.total.fontSize, d = e.total.fontFamily, p = e.total.fontWeight, x = this.donutDataLabels.total.label ? e.total.label : "", f = e.total.formatter(i)) : i.seriesData.series.length === 1 && (f = g(i.seriesData.series[0], i), x = i.seriesData.seriesNames[0]), x && (x = e.name.formatter(x, e.total.show, i)), e.name.show) {
			const m = a.drawText({
				x: r,
	...
	*/
	printInnerLabels(t, e, s, i) {
		...
		const r = a.dom.baseEl.querySelector(".apexcharts-datalabel-label"), n = a.dom.baseEl.querySelector(".apexcharts-datalabel-value"), l = t.value.formatter;
	...
	printDataLabelsInner(t, e) {
		...
		const o = s.dom.baseEl.querySelector(".apexcharts-datalabels-group");
	...
	drawSpokes(t) {
		...
		for (let r = 0; r < e.seriesData.series.length; r++) a.push(L.polarToCartesian(this.centerX, this.centerY, e.globals.radialSize, e.config.plotOptions.pie.startAngle + o * r));
		a.forEach((r, n) => {
			const l = s.drawLine(r.x, r.y, this.centerX, this.centerY, Array.isArray(i.connectorColors) ? i.connectorColors[n] : i.connectorColors);
			t.add(l);
		});
	}
	revertDataLabelsInner() {
		...
		if (this.donutDataLabels.show) {
			const e = t.dom.Paper.findOne(".apexcharts-datalabels-group"), s = this.renderInnerDataLabels(e, this.donutDataLabels, {
				hollowSize: this.donutSize,
				centerX: this.centerX,
				centerY: this.centerY,
				opacity: this.donutDataLabels.show
...
var Js = class {
	...
	draw(t) {
		...
		let h = [], d = null, p = null;
		if (this.yaxisLabels = this.graphics.group({ class: "apexcharts-yaxis" }), t.forEach((g, f) => {
			...
			}), h = this.getDataPointsPos(this.dataRadius[f], this.angleArr[f]);
			const u = this.createPaths(h, {
				x: 0,
				y: 0
			});
			d = this.graphics.group({ class: "apexcharts-series-markers-wrap apexcharts-element-hidden" }), p = this.graphics.group({
				class: "apexcharts-datalabels",
				"data:realIndex": f
			...
			for (let w = 0; w < u.linePathsTo.length; w++) {
				...
				m.add(v);
				const C = s.fillPath({ seriesNumber: f }), D = this.graphics.renderPaths(O(I({}, y), {
				...
				if (e.config.chart.dropShadow.enabled) {
					...
					S.dropShadow(D, Object.assign({}, k, { noUserSpaceOnUse: !0 }), f);
				}
				m.add(D);
			}
			g.forEach((w, v) => {
				const D = new xt(this.w, this.ctx).getMarkerConfig({
					...
					seriesIndex: f,
					dataPointIndex: v
				}), S = this.graphics.drawMarker(h[v].x, h[v].y, D);
				S.attr("rel", v), S.attr("j", v), S.attr("index", f), S.node.setAttribute("default-marker-size", D.pSize);
				const k = this.graphics.group({ class: "apexcharts-series-markers" });
		...
		}), this.drawPolygons({ parent: c }), e.config.xaxis.labels.show) {
	...
	drawPolygons(t) {
		...
		r.forEach((h, d) => {
			...
			p.forEach((f, x) => {
				if (d === 0) {
					const m = this.graphics.drawLine(f.x, f.y, 0, 0, Array.isArray(this.polygons.connectorColors) ? this.polygons.connectorColors[x] : this.polygons.connectorColors);
					c.push(m);
				}
			...
			const p = this.polygons.strokeColors, g = this.polygons.strokeWidth, f = this.graphics.drawPolygon(h, Array.isArray(p) ? p[d] : p, Array.isArray(g) ? g[d] : g, e.globals.radarPolygons.fill.colors[d]);
			s.add(f);
		}), c.forEach((h) => {
			s.add(h);
	...
	drawXAxisTexts() {
		const t = this.w, e = t.config.xaxis.labels, s = this.graphics.group({ class: "apexcharts-xaxis" }), i = L.getPolygonPos(this.size, this.dataPointsLen);
			...
			if (i[o]) {
				...
				n.plotDataLabelsText({
					...
					if (typeof t.config.chart.events.xAxisLabelClick == "function") {
						const p = Object.assign({}, t, { labelIndex: o });
...
var Qs = class extends kt {
	...
	draw(t) {
		...
		if (e.globals.radialSize = n - n * d, this.radialDataLabels.value.show) {
			const p = Math.max(this.radialDataLabels.value.offsetY, this.radialDataLabels.name.offsetY);
	...
	drawTracks(t) {
		const e = this.w, s = new P(this.w), i = s.group({ class: "apexcharts-tracks" }), a = new Z(this.w), o = new st(this.w), r = this.getStrokeWidth(t);
		t.size = t.size - r / 2;
		for (let n = 0; n < t.series.length; n++) {
			const l = s.group({ class: "apexcharts-radialbar-track apexcharts-track" });
			i.add(l), l.attr({ rel: n + 1 }), t.size = t.size - r - this.margin;
			...
			Math.abs(p) + Math.abs(d) >= 360 && (p = 360 - Math.abs(this.startAngle) - .1);
			const g = s.drawPath({
				d: "",
				stroke: h,
	...
	drawArcs(t) {
		...
		if (this.radialDataLabels.show) {
			const f = e.dom.Paper.findOne(".apexcharts-datalabels-group");
		...
		for (let f = g ? t.series.length - 1 : 0; g ? f >= 0 : f < t.series.length; g ? f-- : f++) {
			...
			e.globals.dataChanged && (y = this.startAngle, v = Math.round(this.totalAngle * L.negToZero(e.globals.previousPaths[f]) / 100) + y), Math.abs(w) + Math.abs(u) > 360 && (w = w - .01), Math.abs(v) + Math.abs(y) > 360 && (v = v - .01);
			const S = w - u, k = Array.isArray(e.config.stroke.dashArray) ? e.config.stroke.dashArray[f] : e.config.stroke.dashArray, F = s.drawPath({
				...
				strokeDashArray: k
			}), T = u + S / 2, M = L.polarToCartesian(t.centerX, t.centerY, t.size, T);
			...
			if (a.setSelectionFilter(F, 0, f), this.addListeners(F, this.radialDataLabels), x.add(F), F.attr({
				...
				const X = L.polarToCartesian(t.centerX, t.centerY, t.size, u), R = this.barLabels.formatter(e.seriesData.seriesNames[f], {
					seriesIndex: f,
					w: e
				...
				$.on("click", this.onBarLabelClick), $.attr({ rel: f + 1 }), u !== 0 && $.attr({
	...
	drawHollowImage(t, e, s, i) {
		...
		if (a.config.plotOptions.radialBar.hollow.imageClipped) o.clippedImgArea({
			width: s,
			height: s,
			image: n,
			...
			if (l === void 0 && c === void 0) {
				const h = a.dom.Paper.image(n, function(d) {
					this.move(t.centerX - d.width / 2 + a.config.plotOptions.radialBar.hollow.imageOffsetX, t.centerY - d.height / 2 + a.config.plotOptions.radialBar.hollow.imageOffsetY);
				});
				e.add(h);
			} else {
				const h = a.dom.Paper.image(n, function() {
					this.move(t.centerX - l / 2 + a.config.plotOptions.radialBar.hollow.imageOffsetX, t.centerY - c / 2 + a.config.plotOptions.radialBar.hollow.imageOffsetY), this.size(l, c);
				});
				e.add(h);
			}
		}
	...
	onBarLabelClick(t) {
		...
		s && s(i.seriesData.seriesNames[e], {
			w: i,
			seriesIndex: e
		});
	}
...
var ti = class extends ft {
	draw(t, e) {
		...
		const a = i.group({ class: "apexcharts-rangebar-series apexcharts-plot-series" });
		for (let o = 0; o < t.length; o++) {
			...
			const p = this.barHelpers.initialPositions(l), { y: g, zeroW: f, x, xDivision: m, yDivision: u, zeroH: y } = p;
			let b = p.barWidth, w = p.barHeight;
			n = g, r = x;
			const v = i.group({
				class: "apexcharts-datalabels",
				"data:realIndex": l
			}), C = i.group({ class: "apexcharts-rangebar-goals-markers" });
			for (let D = 0; D < s.globals.dataPoints; D++) {
				...
				if (s.config.plotOptions.bar.rangeBarGroupRows && (R = 1), typeof s.config.series[o].data[D] > "u") break;
				if (this.isHorizontal) {
					E = n + w * this.visibleI;
					const H = (u - w * R) / 2;
	...
	detectOverlappingBars({ i: t, j: e, barYPosition: s, barXPosition: i, srty: a, srtx: o, barHeight: r, barWidth: n, yDivision: l, xDivision: c, initPositions: h }) {
		const d = this.w;
		let p = [];
		const g = d.config.series[t].data[e].rangeName, f = d.config.series[t].data[e].x, x = Array.isArray(f) ? f.join(" ") : f, m = d.labelData.labels.map((y) => Array.isArray(y) ? y.join(" ") : y).indexOf(x), u = d.rangeData.seriesRange[t].findIndex((y) => y.x === x && y.overlaps.length > 0);
	...
	drawRangeColumnPaths({ indexes: t, x: e, xDivision: s, barWidth: i, barXPosition: a, zeroH: o }) {
		...
		if (!r.axisFlags.isXNumeric) e = e + s;
		else {
		...
		return {
			...
			goalY: this.barHelpers.getGoalValues("y", null, o, n, l, h),
			barXPosition: a
		};
	}
	...
	drawRangeBarPaths({ indexes: t, y: e, y1: s, y2: i, yDivision: a, barHeight: o, barYPosition: r, zeroW: n }) {
...
function ae(A, t) {
	let e = 0;
	for (let a = 0; a < A.length; a++) e += A[a];
	const s = t / e, i = new Array(A.length);
	for (let a = 0; a < A.length; a++) i[a] = A[a] * s;
	return i;
}
function oe(A, t, e, s) {
	const i = s * s, a = e * e;
	return Math.max(i * t / a, a / (i * A));
}
function ei(A, t, e, s, i, a) {
	if (A === 0) return !0;
	return oe(t, e, s, a) >= oe(Math.min(t, i), Math.max(e, i), s + i, a);
}
...
function ne(A, t, e, s, i) {
	...
	if (o === 0) return a;
	const r = new Array(o);
	let n = 0, l = 0, c = Infinity, h = -Infinity, d = 0;
	for (; d < o;) {
		const p = Math.min(s, i), g = A[d];
		if (ei(n, c, h, l, g, p)) r[n] = g, n++, l += g, g < c && (c = g), g > h && (h = g), d++;
		else {
...
function si(A, t, e) {
	...
	for (let r = 0; r < s; r++) {
		let n = 0;
		const l = A[r];
		for (let c = 0; c < l.length; c++) n += l[c];
		i[r] = n;
	}
	const a = ne(ae(i, t * e), 0, 0, t, e), o = new Array(s);
	for (let r = 0; r < s; r++) {
		const n = a[r], l = n[0], c = n[1], h = n[2] - l, d = n[3] - c;
		o[r] = ne(ae(A[r], h * d), l, c, h, d);
	}
	return o;
}
var ii = { generate: si };
var ai = class {
	...
	draw(t) {
		const e = this.w, s = new P(this.w, this.ctx), i = new st(this.w), a = s.group({ class: "apexcharts-treemap" });
		...
		return t.forEach((n) => {
		...
		}), ii.generate(o, e.layout.gridWidth, e.layout.gridHeight).forEach((n, l) => {
			const c = s.group({
			...
			const h = s.group({ class: "apexcharts-data-labels" }), d = {
				xMin: Infinity,
				yMin: Infinity,
				xMax: -Infinity,
				yMax: -Infinity
			...
			n.forEach((g, f) => {
				...
				const b = this.helpers.getShadeColor(e.config.chart.type, l, f, this.negRange), w = b.color, v = i.fillPath({
					color: w,
					seriesNumber: l,
					dataPointIndex: f
				}), C = s.drawRect(x, m, u - x, y - m, e.config.plotOptions.treemap.borderRadius, "#fff", 1, this.strokeWidth, e.config.plotOptions.treemap.useFillColorAsStroke ? w : e.globals.stroke.colors[l]);
				C.attr({
					cx: x,
					cy: m,
					index: l,
				...
				e.config.plotOptions.treemap.dataLabels.format === "truncate" && (k = parseInt(e.config.dataLabels.style.fontSize, 10), F = this.truncateLabels(F, k, x, m, u, y));
				let T = null;
				e.seriesData.series[l][f] && (T = this.helpers.calculateDataLabels({
					...
					fontSize: k,
					series: t
				})), e.config.dataLabels.enabled && T && this.rotateToFitLabel(T, k, F, x, m, u, y), c.add(C), T !== null && c.add(T);
			});
			const p = e.config.plotOptions.treemap.seriesTitle;
			if (e.config.series.length > 1 && p && p.show) {
				const g = e.config.series[l].name || "";
				if (g && d.xMin < Infinity && d.yMin < Infinity) {
					const { offsetX: f, offsetY: x, borderColor: m, borderWidth: u, borderRadius: y, style: b } = p, w = b.color || e.config.chart.foreColor, v = {
						left: b.padding.left,
						right: b.padding.right,
						top: b.padding.top,
						bottom: b.padding.bottom
					}, C = s.getTextRects(g, b.fontSize, b.fontFamily), D = C.width + v.left + v.right, S = C.height + v.top + v.bottom, k = d.xMin + (f || 0), F = d.yMin + (x || 0), T = s.drawRect(k, F, D, S, y, b.background, 1, u, m), M = s.drawText({
						x: k + v.left,
						y: F + v.top + C.height * .75,
						text: g,
						fontSize: b.fontSize,
	...
	getFontSize(t) {
		...
		function s(r) {
			let n, l = 0;
			if (Array.isArray(r[0])) for (n = 0; n < r.length; n++) l += s(r[n]);
			else for (n = 0; n < r.length; n++) l += r[n].length;
			return l;
		}
		function i(r) {
			let n, l = 0;
			if (Array.isArray(r[0])) for (n = 0; n < r.length; n++) l += i(r[n]);
			else for (n = 0; n < r.length; n++) l += 1;
			return l;
		}
		const a = s(this.labels) / i(this.labels);
		function o(r, n) {
			const l = r * n;
			return Math.min(Math.pow(l, .5) / a, parseInt(e.config.dataLabels.style.fontSize, 10));
		}
		return o(t[2] - t[0], t[3] - t[1]);
	}
	rotateToFitLabel(t, e, s, i, a, o, r) {
		...
		if (l.width + this.w.config.stroke.width + 5 > o - i && l.width <= r - a) {
			...
			t.node.setAttribute("transform", `rotate(-90 ${c.x} ${c.y}) translate(${l.height / 3})`);
		}
	}
	...
	*/
	static renderToString(t) {
		return Ut(this, arguments, function* (e, s = {}) {
			B.isSSR() && N.init();
			const { width: i = 400, height: a = 300, scale: o = 1 } = s, l = new K(this._createVirtualElement(i, a), O(I({}, e), { chart: O(I({}, e.chart), {
				width: i,
				height: a,
				toolbar: { show: !1 },
			...
			try {
				...
				throw l.destroy(), /* @__PURE__ */ new Error(`SSR rendering failed: ${c.message}`);
			}
		});
	}
	/**
...
${a}
	...
	*/
	static _extractSVGString(t, e = 1) {
		const s = t.w;
		if (!s || !s.dom || !s.dom.Paper) throw new Error("Chart not properly initialized");
...
var pt = class {
	...
	*/
	static hydrate(t, e = {}) {
		if (!B.isBrowser()) throw new Error("Hydration can only be performed in browser environment");
		if (!t) throw new Error("Element is required for hydration");
		if (!t.hasAttribute("data-apexcharts-hydrate")) throw new Error("Element does not have data-apexcharts-hydrate attribute");
		const s = t.getAttribute("data-apexcharts-config");
		if (!s) throw new Error("Element is missing data-apexcharts-config attribute");
		...
		return t.style.width = `${n.width}px`, t.style.height = `${n.height}px`, t.innerHTML = "", t.removeAttribute("data-apexcharts-hydrate"), t.removeAttribute("data-apexcharts-config"), r.render().then(() => {
			...
			const l = new CustomEvent("apexcharts:hydrated", { detail: {
				chart: r,
				ssrContent: o
			} });
			t.dispatchEvent(l);
	...
	*/
	static hydrateAll(t = "[data-apexcharts-hydrate]", e = {}) {
		...
		if (s.length === 0) return console.warn(`No elements found matching selector: ${t}`), [];
		const i = [];
		return s.forEach((a) => {
			try {
	...
	*/
	static _mergeConfigs(t, e) {
		var s, i, a;
		const o = I(I({}, t), e);
		return (t.chart || e.chart) && (o.chart = I(I({}, t.chart), e.chart), (o.chart.animations === void 0 || o.chart.animations.enabled === !1) && (o.chart.animations = O(I({}, o.chart.animations || {}), { enabled: !0 })), ((s = e.chart) == null ? void 0 : s.toolbar) === void 0 && ((a = (i = t.chart) == null ? void 0 : i.toolbar) == null ? void 0 : a.show) === !1 && (o.chart.toolbar = O(I({}, o.chart.toolbar || {}), { show: !0 }))), o;
	}
};
